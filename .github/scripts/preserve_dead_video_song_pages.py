...
legacy_delete = "await db.collection('song_pages').deleteMany({ videoId: deadId });"
...
health_delete = "await db.collection('song_pages').deleteMany({ videoId: vid });"
...
# Targeted guards: only the two destructive song-page deletions above must disappear.
if legacy_delete in catchall.read_text(encoding="utf-8"):
    raise SystemExit("Legacy YouTube cleanup still deletes permanent song pages")
if health_delete in health.read_text(encoding="utf-8"):
    raise SystemExit("Health cleanup still deletes permanent song pages")
