...
for name in VERCEL_TOKEN PREVIEW_URL PREVIEW_DEPLOYMENT_ID EXPECTED_PREVIEW_SOURCE_COMMIT EXPECTED_ROUTE_BLOB EXPECTED_PAGE_BLOB EXPECTED_NEXT_CONFIG_BLOB HARDENED_BRIDGE_BLOB PROTOCOL_BLOB WORKER_BLOB SCHEDULER_BLOB AUDIO_URL AUDIO_BLOB_SHA PROFESSIONAL_REFERENCE_SHA256 GITHUB_SHA; do
  require_env "$name"
done
...
summary={
 'schemaVersion':2,'gate':'v143-final-rhythm-one-shot-existing-preview','sourceCommit':os.environ['GITHUB_SHA'],
 'previewDeploymentId':os.environ['PREVIEW_DEPLOYMENT_ID'],'previewDeploymentUrl':os.environ['PREVIEW_URL'],
 'previewSourceCommit':os.environ['EXPECTED_PREVIEW_SOURCE_COMMIT'],
 'routeBlob':os.environ['EXPECTED_ROUTE_BLOB'],'pageBlob':os.environ['EXPECTED_PAGE_BLOB'],
 'nextConfigBlob':os.environ['EXPECTED_NEXT_CONFIG_BLOB'],'bridgeBlob':os.environ['HARDENED_BRIDGE_BLOB'],
 'protocolBlob':os.environ['PROTOCOL_BLOB'],'workerBlob':os.environ['WORKER_BLOB'],'schedulerBlob':os.environ['SCHEDULER_BLOB'],
 ...
 'deploymentProtectionBypassSecretCreated':False,'temporaryShareLinkCreated':False,
...
import json, os, sys
from pathlib import Path
p=Path(os.environ.get('OUT','debug/v143-contextual-prune/final-rhythm-one-shot'))/'summary.json'
...
export OUT PREVIEW_URL PREVIEW_DEPLOYMENT_ID EXPECTED_PREVIEW_SOURCE_COMMIT
write_initial_summary
JOB_TOKEN=''
ACK_DONE=0

ack_same_job() {
  if [ -z "$JOB_TOKEN" ] || [ "$ACK_DONE" -eq 1 ]; then return 0; fi
  ...
  [[ "$ACK_STATUS" =~ ^[0-9]{3}$ ]] || ACK_STATUS=0
  export ACK_STATUS ACK_RC ACK_SECONDS
  set +e
  python3 - <<'PY'
import json,os
from pathlib import Path
...
try:s['ackRequestSeconds']=float(os.environ.get('ACK_SECONDS') or 0)
except Exception:s['ackRequestSeconds']=0
s['acknowledged']=j.get('status')=='acknowledged'; s['transientResultCleared']=j.get('resultCleared') is True
p.write_text(json.dumps(s,indent=2,sort_keys=True)+'\n')
raise SystemExit(0 if s['ackStatus']==200 and s['acknowledged'] and s['transientResultCleared'] else 1)
PY
  ACK_PARSE_RC=$?
  set -e
  if [ "$ACK_RC" -eq 0 ] && [ "$ACK_PARSE_RC" -eq 0 ]; then ACK_DONE=1; return 0; fi
  echo 'ACK/cleanup failed for the same job; never replace the model start.' >&2
  return 1
}
...
cleanup_on_exit() {
  local original_rc=$?
  rm -f "$REF_TMP"
  if [ -n "$JOB_TOKEN" ] && [ "$ACK_DONE" -ne 1 ]; then
    set +e
    ack_same_job
    set -e
  fi
...
test "$(git hash-object next.config.js)" = "$EXPECTED_NEXT_CONFIG_BLOB"
...
# Reuse the already-built immutable Preview. No deployment, promotion, protection change or share link.
...
patch_summary 'previewIdentityVerified=true' 'previewReady=true'

# Model-free route reachability using the repo's already-proven protected-Preview transport.
PREFLIGHT_STATUS="$(
  vercel curl /api/analyze-audio-tab \
    --deployment "$PREVIEW_URL" \
    -- \
...
from pathlib import Path
out=Path(os.environ['OUT'])
try:d=json.loads((out/'preflight-response.json').read_text())
except Exception:d={}
...
if [ "$PREFLIGHT_PARSE_RC" -ne 0 ]; then
  patch_summary 'terminalState=preflight-failed' 'error=Protected Preview route preflight failed before model start.'
  echo 'STOP before model start: protected Preview route preflight failed.' >&2
  exit 3
fi
...
patch_summary 'modelBearingStartRequestCount=1' 'liveBudgetConsumed=true' "startStatus=$START_STATUS" "startCurlExitCode=$START_RC" "startRequestSeconds=${START_SECONDS:-0}"
export START_STATUS
set +e
python3 - <<'PY'
import json,os
...
print('startAccepted='+str(ok).lower())
raise SystemExit(0 if ok else 1)
PY
START_PARSE_RC=$?
set -e
if [ "$START_RC" -ne 0 ] || [ "$START_PARSE_RC" -ne 0 ]; then
  patch_summary 'startAccepted=false' 'terminalState=start-response-unusable' 'error=Single authorized model-bearing start did not yield a usable async token.'
  echo 'STOP: live budget consumed; no replacement start is authorized.' >&2
  exit 4
fi
...
# Validate terminal structured result and construct the exact freeze input without opening the reference.
python3 - <<'PY'
import json,os
from pathlib import Path
...
# Scorer-only professional reference access begins only after frozen PDF identity is exact.
...
# ACK the exact same job. The EXIT trap provides the same cleanup if a later validation stage fails.
ack_same_job
ACK_DONE=1

# Remove raw/token/full-event JSON. Keep bounded evidence, hashes/manifests and PDFs only.
rm -f "$OUT/preflight-response.json" "$OUT/start-request.json" "$OUT/start-response.json" "$OUT/status-request.json" "$OUT/status-response.json" "$OUT/ack-request.json" "$OUT/ack-response.json" "$OUT/job-token.txt" "$OUT/freeze-input.json" "$OUT/inspect.log"
rm -f "$FREEZE/rhythm-frozen-analysis.json" "$PDF_DIR/pdf-event-evidence.json"
rm -rf "$FINAL_DIR"

if [ "$SCORE_RC" -ne 0 ]; then
  echo 'The single professional score completed but did not pass the 0.99 final gate. The live run is consumed; no retry is authorized.' >&2
  exit "$SCORE_RC"
fi

echo 'V143 final Rhythm one-shot completed: one live start, one professional score, exact same-result PDF, same-job ACK cleanup.'
