...
Continue only on `songsterr-fresh-pipeline-v1`. Guitar-TECHS is closed outcome C before correctness; its official successful audit artifact remains unexpired. V6 method/scoring remain frozen and no replacement-holdout correctness has been exposed. The latest reviewed lead is the SJSU Patil 2025 thesis corpus: despite its reported 75,579-song / 5,134+ hour scale, current authoritative public evidence does not establish permissive performance-audio rights, independent performed note-level onset+pitch truth, an immutable qualifying real-guitar/isolation population, or an untouched external split, so it is rejected as a current V6 holdout before media access. Purpose-built independent-sensor capture remains the strongest design route but is design-only and requires explicit user authorization before contact/spending/recording. No real media, Basic Pitch/V6 correctness, Modal, Vercel heavy-GPU or L4 work is authorized by this checkpoint.