...
Updated: 2026-09-17 America/Toronto — successor metadata search 8 recorded (DoMP/DoPP).
...
1. `I authorize a model run when your ready` was pre-PRE willingness only.
2. `Record pre with my authorization` authorized recording EGSet12 PRE `2a2ed0e4b009f8cd96ba0bc18b384441f53ce87a` only.
3. A later `Please continue 💚` authorized that exact frozen EGSet12 one-shot.
4. That authorization was consumed by authoritative run `35176277018`, job `105058572244`, attempt 1.
5. Later `Please continue 💚` / `Please continue searching 🙏` instructions authorize continuation of metadata/provenance research only. Any alternate-corpus correctness execution requires a new exact PRE and fresh post-freeze authorization.
...
- all `S,E,O,K` true -> `corroborated`;
- resolved required predicate rejection -> `rejected`;
- unresolved/missing/malformed/nonfinite required predicate -> `insufficient` / abstention.
...
- run `35176277018`, job `105058572244`, attempt 1;
- artifact `10478985810`, digest `sha256:da71074013e67c09b7ae85bcafcb914ff2858bf9abb6941a27c769e403c8a53c`;
- status **`BLOCKED_UNTOUCHED_LINEAGE_PROVENANCE / REAL_EVALUATION_NOT_EXECUTED`**.

Prior exposure includes commit `9c0ad09436f74b6168043a2e779b25fb3922199b`. The run stopped before runtime setup, corpus download, Basic Pitch, qualification, annotation parsing or scoring. It produced no EGSet12 correctness evidence and must not be rerun/rescued under the consumed PRE.

## GAPS — PROVENANCE FAILED / NO RERUN TO FORCE PASS

Audit/result:
- workflow/head `80eeaa05c0a063bb193a315b0c2e1aa81041e093`;
- run `35177384414`, job `105061966125`;
- artifact `10479606030`, digest `sha256:b9d54aa6e967db6929c3765e2928022eebc36abb07a090e9e742b6a0e6eb426d`;
- result file `docs/checkpoints/SONGSTERR_FRESH_GAPS_PROVENANCE_AUDIT_RESULT.md`;
- result commit `e4eb3530e4a13e060135e92adddadfbe812a7f03`;
- status **`FAIL_PRIOR_LINEAGE_EXPOSURE / NO_CORPUS_MEDIA_OPENED`**.

Material prior-lineage hits include commits `9a20dcde71f954d4a1704dfca2985e91366f8cc6` and `e110d90c5f8761a4c1fd27af06fc018bc2dec24a`. Do not narrow/repeat the audit merely to force a clean result.

## SUCCESSOR-CORPUS SEARCH LEDGER

### Search 1

File `docs/checkpoints/SONGSTERR_FRESH_SUCCESSOR_CORPUS_METADATA_SEARCH_2026-09-16.md`, commit `9e92faee8b650449d67042732f05f00739f0778a`.

Rejected/excluded: GAPS, GuitarDuets, Jackson Lightfoot/Dhiren Wijesinghe, EG-IPT, EGDB variants, GuitarSet, IDMT-SMT-Guitar, FLGD, Guitar-TECHS, EGFxSet and GOAT under current scope/provenance/reference requirements.

### Search 2

File `docs/checkpoints/SONGSTERR_FRESH_SUCCESSOR_CORPUS_METADATA_SEARCH_CONTINUED_2026-09-16.md`, commit `35e9f786935dddd6e9960b7b98c96497e129112d`.

- GuitarJam — real CC0 electric-guitar DI, no aligned note-birth/MIDI reference established.
- AG-PT-set / Zenodo `10159492` — technically strong onset + `pitch_midi` truth, but prior lineage exposure at `e114eab...`, `225aaf...`, `2c7becc...`; not untouched.
- EG-Solo — historically exposed and exact-source rights/use unresolved.
- G&N/TENT — historically surfaced; source-use basis unresolved.
- `magcil/guitar_style_dataset` / Zenodo `10075352` — real guitar technique recordings, no verified synchronized note-birth truth for each performance.

### Search 3

File `docs/checkpoints/SONGSTERR_FRESH_SUCCESSOR_CORPUS_METADATA_SEARCH_CONTINUED_2_2026-09-16.md`, commit `54e3008a79eeec42093b007595f6b23e28d66cde`.

- GIHME — approximately ten hours of real hexaphonic-guitar improvisations with rich annotations; initial history searches clean, but the cited public GitHub repository still exposes only a placeholder README and no stable corpus release. **Promising but not PRE-ready.**
...
- GIHME status is now **`REJECT_REFERENCE_NOT_INDEPENDENT_AND_NO_STABLE_PUBLIC_CORPUS_RELEASE`** for this correctness boundary.

### Search 7

File `docs/checkpoints/SONGSTERR_FRESH_SUCCESSOR_CORPUS_METADATA_SEARCH_CONTINUED_6_2026-09-17.md`, commit `2a639cdea2e6a4402932dd83dbc46854c236e8ad`.

- GM thesis metadata clarifies that the Ultimate Guitar/Guitar Pro transcription was not simply used unchanged: it was imported into GarageBand and aligned against the actual recording, with the thesis explicitly describing synchronization/matching of timing and content and correction of discrepancies/misalignments. Six per-string MIDI files were then derived from that aligned transcription, and note start time, duration and integer pitch generate the ground-truth labels.
- No AMT/pitch-estimation system is documented as generating GM's labels. Therefore GM is **not rejected solely on reference independence or as completely unaligned score-only truth**.
- Fresh searches across Sapienza IRIS, CISUC/MIR, author/public pages, GitHub, Zenodo, Figshare, OSF and Mendeley Data surfaced no authoritative public GM Dataset package, immutable release/version/file manifest, or dataset-specific license/research-use grant. The article's CC BY-NC-ND license is not a dataset license; the thesis is © 2024 Simone Chieppa, all rights reserved.
- GM status: **`UNRESOLVED_STABLE_PUBLIC_RELEASE_AND_DATASET_RIGHTS / REFERENCE_ALIGNMENT_POTENTIALLY_ACCEPTABLE_FROM_METADATA / NOT_PRE_READY`**.

### Search 8 — latest

File `docs/checkpoints/SONGSTERR_FRESH_SUCCESSOR_CORPUS_METADATA_SEARCH_CONTINUED_7_2026-09-17.md`, commit `7e2083ebf158e064688378d18cea5b37b0e5d6b7`.

- DoMP / Dataset of Monophonic Patterns, Zenodo `10.5281/zenodo.10818617`, version `1.0.0` — primary/public metadata establishes live electric-guitar participation plus MIDI/audio pattern representations. The guitar MIDI comes from a Fishman TriplePlay MIDI tracker; the paper documents tracker limitations/errors. Under the frozen independent-reference requirement, MIDI created by a pitch-tracking conversion system is not authoritative independent note-birth truth. **Status: `REJECT_REFERENCE_NOT_INDEPENDENT_FOR_NOTE_CORRECTNESS`.** No payload opened.
- DoPP / Dataset of Polyphonic Patterns, DOI `10.5281/zenodo.14497998` — metadata establishes real polyphonic guitar audio/pattern repetitions, but not deterministic exact performed-note onset + integer-MIDI-pitch truth for every note. Pattern identity/equivalence is insufficient for this correctness boundary. **Status: `REJECT_NO_VERIFIED_EXACT_NOTE_BIRTH_PLUS_MIDI_PITCH_REFERENCE`.** No payload opened.
- Commit-message searches alone are not accepted as a full-history lineage proof. No payload access was justified because both leads already fail prospective reference gates.
- GM watch found no new authoritative public package/version/file manifest or dataset-specific rights basis; GM remains unresolved and not PRE-ready.
...
Do not rerun/reconstruct merely to change historical outcomes:

- EGSet12 `35176277018` / `105058572244`;
- GAPS audit `35177384414` / `105061966125`;
...
GIHME is rejected for this correctness boundary. DoMP is also rejected for correctness-reference use because its guitar MIDI is generated by a MIDI/pitch-tracking system rather than an independent direct-performance reference. DoPP has real guitar audio but does not establish exact per-note onset+MIDI truth from the metadata reviewed.

GM remains the strongest unresolved clean lead. Its thesis supports actual-audio alignment and timing/content correction without an AMT-generated label source, but GM still lacks an authoritative public dataset release/version/file manifest and dataset-specific rights basis, so it is not PRE-ready.

## NEXT RESEARCH BOUNDARY

Continue **metadata/provenance-only discovery**. GM may be revisited only if a stable public release or direct authoritative data-access record appears. Prioritize direct fret/string/contact/optical sensing, instrumented-fretboard/HCI datasets, manually reconciled score-following corpora, per-string sensing, and 2025–2026 institutional research-data supplements.
...
2. Latest successor-search checkpoint: `SONGSTERR_FRESH_SUCCESSOR_CORPUS_METADATA_SEARCH_CONTINUED_7_2026-09-17.md`, commit `7e2083ebf158e064688378d18cea5b37b0e5d6b7`.
3. No successor corpus selected; no successor PRE exists; no successor run authorized.
4. GIHME is rejected for this correctness boundary: labels originate in automatic Aubio YIN-FFT pitch extraction before manual verification, and no stable public corpus release exists.
5. DoMP is rejected for this correctness boundary because the guitar MIDI reference is generated by a Fishman TriplePlay MIDI/pitch-tracking device; no payload was opened.
6. DoPP is rejected for this correctness boundary because reviewed metadata does not establish exact per-note performed onset+MIDI-pitch truth; no payload was opened.
7. GM Dataset remains the strongest unresolved clean lead. Its thesis supports actual-audio alignment and timing/content correction without AMT-generated labels, but no authoritative public dataset package/version/license has been found.
8. MMIP guitar MIDI must not be used as independent correctness truth for this Basic-Pitch evaluation.
...
1. **Broaden discovery:** prioritize 2025–2026 institutional research-data releases involving direct fret/string/contact/optical sensing, synchronized performance analysis, guitar HCI, score-following datasets, per-string sensing and instrument-performance corpora. Do not limit discovery to mainstream AMT benchmark names already exhausted above.
2. **GM watch:** if an authoritative GM Dataset release, repository, DOI/data record, immutable file manifest or explicit dataset license appears, evaluate that metadata prospectively. Do not infer access/rights from the article or thesis license.
...
4. **Reference-independence gate:** require deterministic note-birth truth tied to the actual recorded performance. Reject references generated by Basic Pitch, NeuralNote, Aubio/YIN, Fishman TriplePlay pitch/MIDI tracking, or other AMT/pitch-estimation systems and score references that are not demonstrably synchronized/reconciled to the performed notes.
5. **PRE readiness gate:** a candidate becomes PRE-ready only after stable source/version/file identity, research-use rights, population/signal-path suitability, deterministic onset+MIDI annotation semantics and clean lineage provenance are all established from metadata/provenance inspection.
6. **If one candidate clears all gates:** record the corpus-selection evidence, freeze a new prospective real-evaluation PRE using unchanged `S AND E AND O AND K`, update this canonical checkpoint to the exact PRE commit, then **STOP for fresh post-freeze user authorization** before downloading/opening evaluation media or running Basic Pitch/qualification/scoring.
...
- Do not reopen GIHME as an independent correctness corpus unless the frozen reference-independence requirement is explicitly changed.
- Do not use MMIP guitar MIDI, DoMP guitar MIDI, or other AMT/pitch-tracker-derived pseudo-ground-truth as authoritative correctness truth.
- Do not treat DoPP pattern labels as exact per-note correctness truth without authoritative exact performed-note onset+pitch annotations.
- Do not treat GM as PRE-ready unless an authoritative stable public release/version/file manifest and dataset-use rights are established.
