...
Continue only on `songsterr-fresh-pipeline-v1`. Guitar-TECHS is closed outcome C before correctness; its official successful audit artifact remains live/unexpired with GitHub digest `sha256:d6e4395f815ce51e1ae83ebdd5c770ca6cd485bb7e90e150dc0e7f7944bf4125`, and the previously independently verified archive digest and merged JSON SHA remain unchanged. V6 method/scoring remain frozen and no replacement-holdout correctness has been exposed. Latest material lead reviewed: the 2025 GM Dataset from Chieppa et al. uses real acoustic/clean-electric performances but derives its labels from pre-existing Ultimate Guitar score/MIDI material that was DAW-aligned to the recordings; it therefore fails the independent performed-event reference gate, also lacks a clear permissive corpus performance-audio rights grant and is only 18 recordings / ~25.5 minutes. Dedicated checkpoint `docs/checkpoints/SONGSTERR_FRESH_GM_DATASET_METADATA_REFERENCE_REVIEW_2026-09-14.md`, commit `5585e2f14c35809ab19c194a2ff21e05a5f3049e`. Purpose-built independent-sensor capture remains the strongest design route but is design-only and requires explicit user authorization before contact/spending/recording. No real candidate media, Basic Pitch/V6 correctness, Modal, Vercel heavy-GPU or L4 work is authorized by this checkpoint.