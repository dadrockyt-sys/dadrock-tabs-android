...
- **M-M Guitar / Perez-Carrillo multimodal guitar — REJECTED: AUDIO-DERIVED ONSETS + INSUFFICIENT POPULATION + RIGHTS/AVAILABILITY NOT ESTABLISHED.** Primary methodology detects note onsets from the recorded audio and takes pitch from the musical score before combining those signals with motion geometry to infer string/fret/pluck controls. Later work again states that pitch/onsets are extracted from audio, so the performed timing is not an independent sensor truth. The corpus is only about ten one-minute monophonic fragments by two guitarists, and later surveys describe it as not publicly available. Dedicated review `docs/checkpoints/SONGSTERR_FRESH_MM_GUITAR_METADATA_REFERENCE_REVIEW_2026-09-14.md`, commit `c338d05c6c321a2aebe7ca72d1f8b803bcfb36e1`.
...
- **TART 2026 — NO NEW ADMISSIBLE HOLDOUT.** Newly posted 2026-09-10 work evaluates GuitarSet, EGDB, and derived Noisy GuitarSet/Noisy EGDB benchmarks. The noisy benchmarks are augmentations of already-governed corpora, not new independent real-guitar performances, so they cannot supply new untouched evidence or cure the underlying provenance/rights gates. Dedicated delta review `docs/checkpoints/SONGSTERR_FRESH_TART_2026_CORPUS_DELTA_REVIEW_2026-09-14.md`, commit `4a6d8e9b9860d2693e72fc0b4bb0af0404692959`.
- **PolyMap — NOT ADMISSIBLE ON CURRENT PUBLIC EVIDENCE / METADATA-ONLY LEAD CLOSED FOR NOW.** ETH Zürich's 2026 PolyMap work establishes a real 64-channel eight-string electric-guitar pickup/capture prototype, but the authoritative public material reviewed does not establish a released external corpus with an immutable independent performed note-level onset+pitch reference, corpus-level performance-audio rights for product validation, sufficient released population, or immutable package identities. No media was acquired. Dedicated review `docs/checkpoints/SONGSTERR_FRESH_POLYMAP_METADATA_HOLDOUT_REVIEW_2026-09-14.md`, commit `bd5379d5384e565abb89f81b1bd8288be41a13e0`.
...
V6 method/scoring remain frozen and no replacement-holdout correctness has been exposed. Guitar-TECHS is closed outcome C before correctness. AG-PT-set, GAPS, François Leduc and M-M Guitar are rejected because precise performance timing is materially reconstructed from evaluated audio/model activations (M-M Guitar additionally uses score pitch and is too small/unavailable); licensing changes cannot cure reference provenance. GIHME has been freshly rechecked and still has no public corpus package behind its ten-hour dataset description. DoMP has now been checked as a potentially stronger live-performance/MIDI-tracker lead, but its authoritative Zenodo release contains only the small MIDI archive and no released evaluated audio. PolyMap has been checked as a new 2026 hardware/capture lead; public ETH/paper evidence establishes the 64-channel prototype but not an admissible released external corpus, so it remains metadata-only with no media acquisition. The newly posted TART 2026 paper adds only evaluations/augmentations of already-governed GuitarSet and EGDB and does not create a new independent holdout. Public/institutional/commercial corpus searches remain near exhausted; the 2026-09-14 recent primary-source delta sweep found no new corpus clearing all five pre-media gates. Purpose-built independent-sensor capture is the strongest remaining design route, but remains design-only and requires explicit user authorization before contact/spending/recording.
