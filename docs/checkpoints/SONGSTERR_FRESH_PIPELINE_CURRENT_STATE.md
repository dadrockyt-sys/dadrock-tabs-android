...
Reverified 2026-09-14: job remains completed/successful; artifact remains live/unexpired; GitHub-reported archive digest `sha256:d6e4395f815ce51e1ae83ebdd5c770ca6cd485bb7e90e150dc0e7f7944bf4125`; run head `f3c9d4a88740146918c34a3538c565f21079f3bf`.
...
- **GAPS — REJECTED.** Performance timing reconstructed from evaluated audio using DTW/transcription-model activations/manual realignment; third-party recording rights also unresolved.
- **François Leduc Guitar Dataset (FLGD) — REJECTED.** Performance timing aligned/fine-aligned using transcription-model activations from evaluated audio. Dedicated checkpoint `docs/checkpoints/SONGSTERR_FRESH_FRANCOIS_LEDUC_DATASET_METADATA_REFERENCE_REVIEW_2026-09-14.md`, commit `fa99798f4c2b08842f534533ea51d84ae9b2a967`.
...
- **Five guitar dataset — REJECTED BEFORE MEDIA ACCESS: NO INDEPENDENT PERFORMED NOTE-LEVEL REFERENCE.** Zenodo `4988354` contains 30 underlying real-guitar performances recorded simultaneously through DI/mobile/computer setups (90 WAV views). Example Freesound copies are CC0, but the authoritative dataset release exposes audio/tempo/song metadata rather than an independent performed onset+pitch event stream. Simultaneous recording views cannot count as independent performances. Dedicated checkpoint `docs/checkpoints/SONGSTERR_FRESH_FIVE_GUITAR_DATASET_METADATA_REVIEW_2026-09-14.md`, commit `b4911312dd3a38d8578b0b97ef47b48b26b85e61`.
...
Continue only on `songsterr-fresh-pipeline-v1`. Guitar-TECHS is closed outcome C before correctness; its official successful audit artifact remains unexpired. V6 method/scoring remain frozen and no replacement-holdout correctness has been exposed. The Five guitar dataset is the latest reviewed lead and is rejected before media access because the authoritative release provides real simultaneous audio views but no independent performed note-level onset+pitch reference. Purpose-built independent-sensor capture remains the strongest design route but is design-only and requires explicit user authorization before contact/spending/recording. No real media, Basic Pitch/V6 correctness, Modal, Vercel heavy-GPU or L4 work is authorized by this checkpoint.