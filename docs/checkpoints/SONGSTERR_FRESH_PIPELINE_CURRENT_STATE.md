...
- **GAPS — REJECTED.** Performance timing reconstructed from evaluated audio using DTW/transcription-model activations/manual realignment; third-party recording rights unresolved.
...
Continue only on `songsterr-fresh-pipeline-v1`. Guitar-TECHS is closed outcome C before correctness; its official successful audit artifact remains unexpired. V6 method/scoring remain frozen and no replacement-holdout correctness has been exposed. The latest reviewed lead is FretboardFlow: although it is a real expert-performed hexaphonic guitar dataset, its own public README says its released MIDI is quantized and not millisecond-accurate, while the ISMIR paper describes automated hexaphonic transcription via the GuitarSet/KAMIR pipeline. It therefore fails the frozen independent high-resolution onset-reference gate before media access; current public repository evidence also does not establish permissive performance-audio rights or a released audio package. Purpose-built independent-sensor capture remains the strongest design route but is design-only and requires explicit user authorization before contact/spending/recording. No real media, Basic Pitch/V6 correctness, Modal, Vercel heavy-GPU or L4 work is authorized by this checkpoint.