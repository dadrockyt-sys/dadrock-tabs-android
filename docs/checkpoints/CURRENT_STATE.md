...
- The workflow job (`98679621613`) contains only checkout, generator reassembly/execution, and commit/self-seal steps. The workflow YAML has no download/copy/upload step for notation images and uses `fetch-depth: 1`.
- Historical parent-tree probes at `a52ddd668934a15c6a1e6769333221c51941667b` and `fb1f7555f991e7679472d5efed213c98eb238e16` contain the staged generator parts but no `1000120332...` Lead JPG path and no `84.jpg...105.jpg` rendered-copy path. Repository issue search for these identities/song reference also returned no attachment-bearing issue.
- Crucially, staging commit `57467c42968d15651eb5ca691987eaaffbace4ab` (`research: stage Lead preservation generator 6/7`) preserves the generator text stating that the user had **re-provided all 22 Lead pages in the active chat**, that the active-chat copies were platform-rendered/re-encoded, and that screenshot bytes were **not committed**. This confirms the visual pages were conversation-side source material, not a repository artifact later deleted by the self-sealing workflow.
...
- Repository-side recovery is therefore close to exhausted: the remaining plausible source is the prior conversation/upload substrate (or an external local copy), not reconstruction from Git objects. Hashes/visual-order data remain authentication/transcription evidence only and cannot reconstruct exact page images or authorize exact Lead attack timing inference.
- Constraints unchanged: scoring 0; frozen candidate untouched; no GPU; no `main`/Production change; do not commit notation images or derived raster copies.

## Exact next steps
1. **DONE / FROZEN:** Rhythm row-identity equivalence audit; preserve both frozen Rhythm files unchanged.
2. Lead source recovery now primarily requires locating the prior conversation/upload copies or an external original copy matching the receipt fingerprints. Continue only non-destructive repository/history checks if a new concrete breadcrumb appears; do not substitute Bass pages and do not invent timing from visual-order JSON.
