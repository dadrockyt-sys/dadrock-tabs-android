...
- Aug 24 SHA `d2db25365232a1a39354eb58570e6450fa790c23` (`Force Demucs CPU for exact cold determinism`) dual-band run `32685200097` also has only a live behavior-log artifact `9505600616`; its normalized input remains `ab64e7...`, it still fails both direct/cascade cross-worker exactness, and workers 1/3 agree while worker 2 differs. No stems were retained.
- Aug 24 SHA `b1520a8eab185f6bf2abd9b8838693b3bfbae77a` (`Pin Demucs CPU thread topology`) dual-band run `32685824573` has live behavior-log artifact `9505790190`; artifact type is log-only, not source-stem storage.
- Aug 24 SHA `c8f18da8a2f6c5017ddb8935b993b2a0429cf453` (`Use dedicated Demucs shift RNG`) dual-band run `32686195439` has live behavior-log artifact `9505920380`; artifact type is log-only, not source-stem storage.
- Aug 24 SHA `7713f13f18d23917d71813b87b5cfa793ea1d488` (`Pin Demucs CPU numerical dispatch in research wrapper`) is important: Modal-smoke run `32687907974` succeeded historically, but its retained artifact list is **empty**. Its paired dual-band run `32687907958` retains only behavior-log artifact `9506637919`; the log still reports `directStemPcmExactAcrossWorkers=false` and `cascadeStemPcmExactAcrossWorkers=false`, normalized input `ab64e7...`, workers 1/3 direct/cascade file SHA `06d2356b3eddf7f4e2b84489fff223cdf2d3508d7f8bd3b74cae6a005e9d92b8` / `4a52f9078d40f7d48bd00408610ccd3d93b6ba33fdf7139af5f368add30a4d9c`, worker 2 `b6cc6404c0b262673b6217ece869ded150c780b88f0e517f9662e6e5fa80b35f` / `1de2354f564e83d33dfdc38799d5cff7b383928e4242c3f16f4c1ab29fa4a848`. Therefore even this historically successful smoke did not preserve exact full-source view bytes.
- Therefore the preferred path remains locating any preserved workflow artifact containing the exact BS-RoFormer guitar view (or byte-identical source arrays), especially later Aug 24 determinism runs after `7713f13...`; direct Demucs repeatability can be tested CPU-only if useful.
