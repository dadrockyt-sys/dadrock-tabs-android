...
- Implemented the exact preregistered 36-rule contextual Guitar family: onset `{0.50,0.65}`, candidate/max-active template ratio `{0.75,1.00,1.25}`, active-state `{allow_active,inactive_only}`, interval policy `{none,exclude_harmonic_octave,chord_interval}`, top-1/site, `(step,midi)` dedupe, polyphony cap 6.
...
- Reference-blind generator: `validation/v167_single_song_calibration/build_contextual_guitar_recovery_variants_v167.py`. Initial preregistration blob `23065c56b5b08d1d9d59fe37a01dfa95f6c6627d`; identity-only checksum correction commit `193238d822660ee3d28b7ca890622d0256791422`, current blob `0dd0f9c7560a2caef9b11ef1caea34a4bbf48fcb`. Rule family and selection ordering are unchanged by that correction.
- Base frozen-family grader: `validation/v167_single_song_calibration/score_contextual_guitar_recovery_variants_v167.py`, blob `adfd512f53e8839b295129c0768d484b5af09bc7`, commit `95adfb70254bbf3be13233d99ed844e57bb31297`.
- Safe grader identity adapter: `validation/v167_single_song_calibration/score_contextual_guitar_recovery_variants_v167_rearm.py`, blob `8555101a684f59f7dcd910d7c6f7694e968a68b1`, commit `af57ace48426ae3faf61c61a6f5cae24ba66fe57`. It changes only the stale I003 SHA256 guard before delegating to the already-preregistered grader; scoring and winner-selection logic are unchanged.
- Predeclared top-1/site ordering remains: candidate/max-active template-score ratio descending, template rank descending, template score descending, onset support descending, activity support descending, MIDI ascending.
- First one-shot arm commit `82c1e0e375d8be6d1c0a974d311723381bfdf91c`; run `33256979705`, job `99112324623` terminated in the immutable preregistration verification step. Generation, grading, and freeze steps were skipped, so **zero contextual candidates were generated and zero professional reference/scorer reads occurred**.
- The failed guard established a stale historical checkpoint checksum: Git blob `758f8762632e916306aed9b036a6483af9431dc0` is unchanged, but the runner independently SHA256-hashed the frozen I003 bytes as `f15c6f40dd4b8479c2dfb7eab039cff98a23b45eb796265ffad08c5a8ae37115`. The previously written `...c575bf709673` SHA256 was incorrect metadata and is corrected below; I003 content itself was not modified.
- Next substep: re-arm the one-shot CPU workflow with only the corrected frozen-I003 identity, exact current generator/adapter/checkpoint blobs, the same 37 predeclared complete variants, and the same winner policy. Freeze all candidate hashes before any reference/scorer read; self-remove after report/receipt commit; do not create I004 inside the sweep.
