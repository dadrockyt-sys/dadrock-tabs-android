...
def event(event_index: int, *, measure: int = 1, step: int = 4, string_index: int = 1,
          fret: int = 1, midi: int = 60, techniques: list[str] | None = None, **extra) -> dict:
    row = {
        "eventIndex": event_index, "measure": measure, "step": step,
        "stringIndex": string_index, "fret": fret, "midi": midi,
        "durationSteps": 1, "techniques": [] if techniques is None else list(techniques),
    }
    row.update(extra)
    return row
...
def reference(*, measure: int, step: int) -> dict:
    return {"measure": measure, "step": step, "stringIndex": 1, "fret": 1,
            "midi": 60, "durationSteps": 1, "techniques": []}
...
class GeneratedOnlyTriadPrunePolicyTests(unittest.TestCase):
    def test_construction_requires_generated_only_exact_triad_onset(self) -> None:
        ...
        corrections = generated_only_triad_corrections(generated, [reference(measure=2, step=4)])
        self.assertEqual([[row["eventIndex"] for row in onset] for onset in corrections], [[0, 1, 2]])
    ...
    def test_construction_and_ranking_are_deterministic_under_reversal(self) -> None:
        ...
        forward = rank_fit_generated_only_triad_prune_rules(generated, [], minimum_false_positive_support=3)
        reverse = rank_fit_generated_only_triad_prune_rules(list(reversed(generated)), [], minimum_false_positive_support=3)
    ...
    def test_rank_support_cap_and_rule_shape_are_fixed(self) -> None:
        ...
        rules = rank_fit_generated_only_triad_prune_rules(
            generated, [], minimum_false_positive_support=3, maximum_candidates=2
        )
    ...
    def test_runtime_match_requires_exact_triad_context_and_all_three_identities(self) -> None:
        ...
        self.assertTrue(onset_matches_generated_only_triad_prune_rule(
            source, "stepQuarter::0", 3, 0, 1, 0, 2, 2
        ))
        self.assertFalse(onset_matches_generated_only_triad_prune_rule(
            source, "stepQuarter::1", 1, 0, 2, 2, 3, 0
        ))
        self.assertFalse(onset_matches_generated_only_triad_prune_rule(
            source, "stepQuarter::0", 1, 0, 2, 2, 3, 1
        ))
        self.assertFalse(onset_matches_generated_only_triad_prune_rule(
            source[:2], "stepQuarter::0", 1, 0, 2, 2, 3, 0
        ))
        four = source + [event(3, measure=1, step=4, string_index=4, fret=15, midi=60)]
        self.assertFalse(onset_matches_generated_only_triad_prune_rule(
            four, "stepQuarter::0", 1, 0, 2, 2, 3, 0
        ))
    ...
    def test_runtime_prunes_three_events_atomically_and_preserves_survivors(self) -> None:
        source = [
            *triad(0, measure=1),
            ...
            event(4, measure=2, step=8, string_index=2, fret=5, midi=60, custom="keep-b"),
        ]
        out = apply_generated_only_triad_prune_rule(source, "stepQuarter::0", 1, 0, 2, 2, 3, 0)
    ...
    def test_runtime_refuses_measure_erasure_linked_and_referenced_triads(self) -> None:
        last_measure = [*triad(0, measure=1), event(3, measure=2, step=8, string_index=2, fret=5, midi=60)]
        self.assertEqual(
            apply_generated_only_triad_prune_rule(last_measure, "stepQuarter::0", 1, 0, 2, 2, 3, 0),
            last_measure,
        )
        linked = [*triad(0, measure=1, bendTargetMidi=64), event(3, measure=1, step=8, string_index=4, fret=15, midi=60)]
        self.assertEqual(
            apply_generated_only_triad_prune_rule(linked, "stepQuarter::0", 1, 0, 2, 2, 3, 0),
            linked,
        )
        referenced = [*triad(0, measure=1), event(3, measure=1, step=8, string_index=4, fret=15, midi=60, legatoTargetEventIndex=0)]
        self.assertEqual(
            apply_generated_only_triad_prune_rule(referenced, "stepQuarter::0", 1, 0, 2, 2, 3, 0),
            referenced,
        )

    def test_invalid_context_string_or_pitch_class_is_rejected(self) -> None:
        source = [*triad(0, measure=1), event(3, measure=1, step=8, string_index=4, fret=15, midi=60)]
        with self.assertRaises(ValueError):
            apply_generated_only_triad_prune_rule(source, "pitchClass::0", 1, 0, 2, 2, 3, 0)
        with self.assertRaises(ValueError):
            apply_generated_only_triad_prune_rule(source, "stepQuarter::0", 9, 0, 2, 2, 3, 0)
        with self.assertRaises(ValueError):
            apply_generated_only_triad_prune_rule(source, "stepQuarter::0", 1, 0, 2, 12, 3, 0)
