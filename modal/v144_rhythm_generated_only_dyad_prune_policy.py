...
def _structural_onset_signatures(measure: int, step: int) -> tuple[str, ...]:
    ...
    if not result:
        raise ValueError("generated-only dyad prune requires structural onset signatures")
    if any(value.startswith(("pitchClass::", "register::", "registerStep::")) for value in result):
        raise ValueError("generated-only dyad structural context unexpectedly contains pitch-derived data")
    return result
...
def _validate_note_identity(string_index: int, pitch_class: int) -> tuple[int, int]:
    ...
    if string_value not in OPEN_MIDI_BY_STRING_INDEX:
        raise ValueError("generated-only dyad source string is invalid")
    if not 0 <= pitch_value <= 11:
        raise ValueError("generated-only dyad source pitch class must be in [0,11]")
    return string_value, pitch_value


def _normalize_note_identities(
...
def _validate_rule(
    ...
    if not context_value.startswith(_ALLOWED_CONTEXT_PREFIXES):
        raise ValueError("generated-only dyad prune requires one structural onset context signature")
    return context_value, _normalize_note_identities(
        first_string_index,
        first_pitch_class,
        second_string_index,
        second_pitch_class,
...
def _dyad_is_runtime_prune_eligible(
    events: Sequence[Mapping[str, Any]], onset_events: Sequence[Mapping[str, Any]]
) -> bool:
    ...
    if any(int(event["measure"]) != measure for event in onset_events):
        return False
    if sum(1 for row in events if int(row["measure"]) == measure) <= 2:
        return False
    return True


def _onset_note_identities(
    ...
    try:
        identities = tuple(
            sorted((int(event["stringIndex"]), int(event["midi"]) % 12) for event in onset_events)
        )
    except (KeyError, TypeError, ValueError):
        return None
...
def generated_only_dyad_corrections(
    fit_generated: Sequence[Mapping[str, Any]],
    fit_reference: Sequence[Mapping[str, Any]],
) -> list[list[dict[str, Any]]]:
    """Return FIT generated-only exact-two-note onset false positives.

    Construction uses FIT labels only to identify onsets with exactly two generated
    events and zero reference notes. Runtime application never receives reference rows.
    """
    ...
    for onset in sorted(generated_by_onset):
        ...
        if len(generated) != 2 or len(reference) != 0:
            continue
...
def onset_matches_generated_only_dyad_prune_rule(
    onset_events: Sequence[Mapping[str, Any]],
    context_signature_value: str,
    first_string_index: int,
    first_pitch_class: int,
    ...
    if context_value not in _structural_onset_signatures(measure, step):
        return False
    return _onset_note_identities(onset_events) == expected_identities


def rank_fit_generated_only_dyad_prune_rules(
    fit_generated: Sequence[Mapping[str, Any]],
    fit_reference: Sequence[Mapping[str, Any]],
    *,
    ...
    """Construct/rank atomic generated-only dyad prune rules from FIT labels only."""
    ...
    correction_support: Counter[
        tuple[str, tuple[tuple[int, int], tuple[int, int]]]
    ] = Counter()
    for dyad in generated_only_dyad_corrections(fit_generated, fit_reference):
        identities = _onset_note_identities(dyad)
        if identities is None:
            raise ValueError("generated-only dyad correction lost exact dyad identity")
        measure = int(dyad[0]["measure"])
        step = int(dyad[0]["step"])
        for context_value in _structural_onset_signatures(measure, step):
            correction_support[(context_value, identities)] += 1

    generated_by_onset = _group_by_onset(fit_generated)
    candidates: list[dict[str, Any]] = []
    for (context_value, identities), support in correction_support.items():
        ...
        for onset in sorted(generated_by_onset):
            ...
            if not onset_matches_generated_only_dyad_prune_rule(
                ...
                continue
            if any(not _source_position_is_valid(event) or has_pitch_linkage(event) for event in events):
                continue
            eligible_support += 1
        if eligible_support < support or eligible_support <= 0:
            raise ValueError("generated-only dyad prune eligible support is inconsistent")
        candidates.append(
            {
...
def apply_generated_only_dyad_prune_rule(
    ...
    """Reference-free atomic prune of eligible exact-two-note generated onsets."""
    ...
    for onset in sorted(grouped_indices):
        ...
        if not onset_matches_generated_only_dyad_prune_rule(
            ...
            continue
        if not _dyad_is_runtime_prune_eligible(source, onset_events):
            continue
