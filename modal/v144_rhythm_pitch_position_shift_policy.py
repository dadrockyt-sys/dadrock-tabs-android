...
from v144_rhythm_pitch_shift_policy import (
...
def _rule_signatures(row: Mapping[str, Any]) -> tuple[tuple[str, str], ...]:
    ...
    if len(pitch_classes) != 1:
        raise ValueError("joint pitch-position construction requires exactly one pitch-class signature")
...
def _target_position(
    event: Mapping[str, Any], semitone_shift: int, string_shift: int
) -> tuple[int, int, int] | None:
    if has_pitch_linkage(event):
        return None
    string_index, _, midi = _validate_event_position(event)
    target_string = string_index + int(string_shift)
    if target_string not in OPEN_MIDI_BY_STRING_INDEX:
        return None
    target_midi = midi + int(semitone_shift)
...
def event_matches_pitch_position_rule(
    event: Mapping[str, Any], signatures: Sequence[str]
) -> bool:
    if len(signatures) != 2:
        raise ValueError("a V144 joint pitch-position rule requires exactly two signatures")
    normalized = tuple(sorted(str(value) for value in signatures))
    if len(set(normalized)) != 2:
        raise ValueError("joint pitch-position signatures must be distinct")
    if sum(value.startswith("pitchClass::") for value in normalized) != 1:
        raise ValueError("joint pitch-position rule requires exactly one source pitch-class signature")
    available = set(context_signature(event))
    return all(value in available for value in normalized)


def same_onset_pitch_position_pairs(
    ...
    *,
    ...
    """Return fit-only same-onset substitutions requiring pitch + string changes.

    Pairing inherits the already-fixed exact-pitch-removal/minimum-MIDI-distance
    deterministic policy. This materially new family then keeps only pairs whose
    pitch delta and string delta are both non-zero and within conservative bounds.
    Runtime application receives no reference row.
    """
    ...
    for generated_row, reference_row in same_onset_substitution_pairs(
        ...
        pitch_shift = int(reference_row["midi"]) - int(generated_row["midi"])
        string_shift = int(reference_row["stringIndex"]) - int(generated_row["stringIndex"])
        if pitch_shift == 0 or string_shift == 0:
            continue
        if abs(string_shift) > maximum_string:
            continue
        target = _target_position(generated_row, pitch_shift, string_shift)
        if target is None:
            continue
        target_string, target_fret, target_midi = target
...
def rank_fit_pitch_position_rules(
    ...
    *,
    ...
    """Construct/rank joint pitch+adjacent-string rules from fit labels only."""
    ...
    for generated_row, reference_row in same_onset_pitch_position_pairs(
        fit_generated,
        fit_reference,
        maximum_abs_semitone_shift=maximum_pitch,
        ...
        for signatures in _rule_signatures(generated_row):
            correction_support[(signatures, semitone_shift, string_shift)] += 1

    candidates: list[dict[str, Any]] = []
    for (signatures, semitone_shift, string_shift), support in correction_support.items():
        if support < minimum_support:
            continue
        eligible_support = sum(
            1
            for row in fit_generated
            if event_matches_pitch_position_rule(row, signatures)
            and _target_position(row, semitone_shift, string_shift) is not None
        )
        if eligible_support < support or eligible_support <= 0:
            raise ValueError(
                "invalid pitch-position eligible support "
                f"signatures={signatures!r} pitch={semitone_shift} string={string_shift}"
            )
        candidates.append(
            {
...
def apply_pitch_position_rule(
    ...
    *,
    ...
    """Apply a reference-free pitch correction plus adjacent-string revoice."""
    pitch_shift = int(semitone_shift)
    position_shift = int(string_shift)
    maximum_pitch = int(maximum_abs_semitone_shift)
    maximum_string = int(maximum_abs_string_shift)
    if pitch_shift == 0 or abs(pitch_shift) > maximum_pitch or maximum_pitch < 1 or maximum_pitch > 12:
        raise ValueError("semitone_shift must be non-zero and within [1, 12]")
    if position_shift == 0 or abs(position_shift) > maximum_string or maximum_string != 1:
    ...
    for event in events:
        row = dict(event)
        if event_matches_pitch_position_rule(row, signatures):
            target = _target_position(row, pitch_shift, position_shift)
