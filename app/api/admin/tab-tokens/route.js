...
function makeToken() {
  ...
  return `DRT-${value.slice(0, 4)}-${value.slice(4, 8)}-${value.slice(8, 12)}`;
}
...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, stats, tokens: tokens.map(serializeToken) });
  } catch (error) {
...
export async function POST(request) {
  ...
  try {
    ...
    if (action === 'generate') {
      ...
      return NextResponse.json({ success: true, tokens: documents.map((token) => ({ ...token, _id: undefined })) });
    }
    ...
    console.error('Token admin error:', error);
    return NextResponse.json({ error: 'Unable to update tokens.' }, { status: 500 });
  }
}
