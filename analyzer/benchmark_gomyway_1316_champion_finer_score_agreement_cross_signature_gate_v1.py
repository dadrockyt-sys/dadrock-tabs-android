...
import profile_gomyway_1316_champion_finer_cross_signatures_v1 as finer

p1316 = finer.p1316
v2 = finer.v2
v3 = finer.v3
recall = finer.recall
profile = finer.profile

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1316-champion-finer-score-agreement-cross-signature-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1316-champion-finer-score-agreement-cross-signature-gate-v1-manifest.json"
...
TARGET_SIGNATURES = {
    "step0_midi52_score20_plus_both10",
    "step12_midi47_score8_13_single_or_weak",
    "step12_midi50_score8_13_single_or_weak",
    "step12_midi59_score8_13_single_or_weak",
    "step8_midi50_score8_13_single_or_weak",
}

RULES = [
    {"name": "drop_step0_midi52_score20_plus_both10", "drop": {"step0_midi52_score20_plus_both10"}},
    {"name": "drop_step12_midi47_score8_13_single_or_weak", "drop": {"step12_midi47_score8_13_single_or_weak"}},
    {"name": "drop_step12_midi50_score8_13_single_or_weak", "drop": {"step12_midi50_score8_13_single_or_weak"}},
    {"name": "drop_step12_midi59_score8_13_single_or_weak", "drop": {"step12_midi59_score8_13_single_or_weak"}},
    {"name": "drop_step8_midi50_score8_13_single_or_weak", "drop": {"step8_midi50_score8_13_single_or_weak"}},
    {"name": "drop_all_five_profiled_zero_precision_score_agreement_signatures", "drop": TARGET_SIGNATURES},
]
...
def signature(token, winner_scores, alt_scores) -> str:
    ...
    return f"step{step}_midi{pitch}_{score_band}_{agreement}"
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.16 champion for finer score/agreement gate...", flush=True)
    ...
    champion_actual = (
        int(champion_score["matched"]), int(champion_score["missing"]), int(champion_score["extra"]),
    )
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected 13.16 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got "
            f"{champion_actual}/{champion_score['pitchF1']}"
        )

    evaluator = p1272.prune1163.evaluate
    results = {}
    for rule in RULES:
        kept = prune_residual(fourteenth_kept, winner_scores, alt_scores, rule)
        ...
        print(
            ...
            f"extra={s['extra']} extraReduction={audit['extraReduction']} "
            f"matchedChange={audit['matchedChange']} cv={audit['crossValidationPassed']} "
            f"sections={audit['sectionStabilityPassed']} shifted={audit['shiftedWindowStabilityPassed']} "
            f"accepted={audit['acceptedOverChampion']}",
            flush=True,
        )
    ...
    if accepted:
        ...
        winner_name, winner = "retain_13_16_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.16 finer score/agreement cross-signature benchmark")

    ws = winner["fullScore"]
    validated = winner_name != "retain_13_16_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.16-finer-score-agreement-cross-signature-gate",
        ...
        "recommendedNextAction": (
            "profile-finer-cross-signatures-from-validated-winner"
            if validated else "retain-13.16-and-test-next-finer-cross-signature-family"
        ),
    }
    manifest = {
        "schemaVersion": 1,
    ...
    print("GOMYWAY 13.16 CHAMPION FINER SCORE-AGREEMENT CROSS-SIGNATURE GATE V1 COMPLETE")
    print("Passed: True")
