...
OUTPUT_PATH = PUBLIC / "gomyway-3676-patch-v10-negative-tail-hybrid-selector-tiebreak-v2.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-patch-v10-negative-tail-hybrid-selector-tiebreak-v2-manifest.json"
EXPECTED = (272, 595, 341)
EXPECTED_F1 = 36.76
OUTER_FOLDS = 5
INNER_FOLDS = 3
NEG_Q = 0.975
MIN_MEAN_LIFT_ADVANTAGE = 2.0
...
def inner_section_comparison(
    ...
    baseline_mean = float(np.mean(baseline_lifts)) if baseline_lifts else 0.0
    neg_mean = float(np.mean(neg_lifts)) if neg_lifts else 0.0
    return {
        "baselineSectionPassCount": baseline_passes,
        "negativeTailSectionPassCount": neg_passes,
        "baselineMeanLift": round(baseline_mean, 3),
        "negativeTailMeanLift": round(neg_mean, 3),
        "negativeTailMeanLiftAdvantage": round(neg_mean - baseline_mean, 3),
        "innerRows": rows,
    }


def selector_uses_negative_tail(inner: dict[str, Any]) -> bool:
    neg_pass = int(inner["negativeTailSectionPassCount"])
    base_pass = int(inner["baselineSectionPassCount"])
    if neg_pass > base_pass:
        return True
    if neg_pass < base_pass:
        return False
    return float(inner["negativeTailMeanLiftAdvantage"]) >= MIN_MEAN_LIFT_ADVANTAGE
...
def evaluate_scheme(
    ...
    for fold in range(OUTER_FOLDS):
        ...
        use_neg = selector_uses_negative_tail(inner)
        print(
            "    heartbeat negative-tail selector-v2 "
            ...
            f"meanLiftAdv={inner['negativeTailMeanLiftAdvantage']} "
...
def main() -> None:
    ...
    print("Starting V10 negative-tail hybrid selector mean-lift tie-break diagnostic", flush=True)
    print(
        "Predeclared rule: prefer negQ0.975 if inner section pass count is higher; "
        f"if tied, require >= {MIN_MEAN_LIFT_ADVANTAGE:.1f} points higher inner mean lift.",
        flush=True,
    )
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during negative-tail selector tie-break diagnostic")

    output = {
        "schemaVersion": 2,
        "profileType": "36.76-patch-v10-negative-tail-hybrid-selector-tiebreak",
        "baselinePitchF1": EXPECTED_F1,
        "baselineMatchedMissingExtra": list(EXPECTED),
        "negativeTailQuantile": NEG_Q,
        "minimumMeanLiftAdvantagePoints": MIN_MEAN_LIFT_ADVANTAGE,
        "selectorRule": "prefer-negQ0.975-on-higher-inner-section-pass-count; on-tie-require-2pt-inner-mean-lift-advantage",
        ...
        "nextTarget": "strict-v10-negative-tail-tiebreak-nested-cv-v11" if ready else "retire-negative-tail-selector-tiebreak-and-pivot-residual-strategy",
        "normal": normal,
        "section": section,
    ...
    print("GOMYWAY 36.76 PATCH V10 NEGATIVE-TAIL HYBRID SELECTOR TIEBREAK V2 COMPLETE")
