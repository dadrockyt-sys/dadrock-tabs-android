...
app = modal.App("dadrock-v143-bass-technique-canary")
...
@app.function(
    image=technique_canary_image,
    gpu="L4",
    timeout=1200,
    memory=8192,
)
def analyze_approved_bass_techniques(source_audio: bytes) -> dict[str, Any]:
    """Enrich authenticated Bass events with conservative two-view techniques."""
    if not source_audio:
        raise ValueError("Bass technique canary source audio is empty")
    if len(source_audio) > MAX_CANARY_AUDIO_BYTES:
        raise ValueError("Bass technique canary audio cannot exceed 50 MB")
    ...
    with tempfile.TemporaryDirectory(prefix="dadrock-bass-technique-") as tmp:
        root = Path(tmp)
        ...
        enrichment = enrich_bass_events_with_techniques(
            base_events,
            stem_paths=[direct, cascade],
        )
        enriched_events = list(enrichment["events"])
        technique_diagnostics = dict(enrichment["diagnostics"])

    return {
        "schemaVersion": 1,
        "mode": "isolated-reference-free-bass-technique-evidence-canary",
        ...
        "techniqueEvidenceGenerated": int(
            technique_diagnostics.get("techniqueEventCount") or 0
        ) > 0,
        "allInitialTechniqueFamiliesProven": bool(
            technique_diagnostics.get("allInitialTechniqueFamiliesProven")
        ),
...
def run(
    ...
    output_path: str = ".bass-technique/raw-bass-technique.json",
) -> None:
    source = Path(audio_path)
    if not source.is_file() or source.stat().st_size <= 0:
        raise RuntimeError(f"Approved Bass technique audio missing: {source}")
    if source.as_posix() != APPROVED_FIXTURE:
        raise RuntimeError(
            "Bass technique canary is locked to the approved fixture: "
            f"{APPROVED_FIXTURE}"
        )
    if source.stat().st_size > MAX_CANARY_AUDIO_BYTES:
        raise RuntimeError("Approved Bass technique audio exceeds 50 MB")

    result = analyze_approved_bass_techniques.remote(source.read_bytes())
    ...
    diagnostics = result.get("techniqueDiagnostics") or {}
    print("=== BASS TECHNIQUE CANARY COMPLETE ===")
    print(f"rawOutput={output}")
    print(f"eventCount={result.get('eventCount')}")
    print(f"techniqueEventCount={diagnostics.get('techniqueEventCount')}")
    print(f"provenTechniqueFamilies={diagnostics.get('provenTechniqueFamilies')}")
    print("harmonicEvidenceImplemented=false")
