...
import profile_gomyway_1305_pruned_recall_champion_residual_additions_precision_v1 as p1305

v2 = p1305.v2
v3 = p1305.v3
recall = p1305.recall
profile = p1305.profile
prune1305 = p1305.prune1305

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1305-champion-eleventh-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1305-champion-eleventh-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1597)
EXPECTED_F1 = 13.05
WINNER_NAME = "dual6_quarter_40_64"

ELEVENTH_ZERO_BUCKETS = {
    (57, "13_16"),
    (55, "10_13"),
    (55, "13_16"),
    (55, "16_20"),
...
RULES = [
    {"name": "drop_eleventh_zero_midi57_13_16", "drop": [[57, "13_16"]]},
    {"name": "drop_eleventh_zero_midi55_mid_strengths", "drop": [[55, "10_13"], [55, "13_16"], [55, "16_20"]]},
    {"name": "drop_all_eleventh_profiled_zero_precision_pitch_strength", "drop": sorted([list(x) for x in ELEVENTH_ZERO_BUCKETS])},
]
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.05 pruned recall champion...", flush=True)
    ...
    core_score = p1305.grade(core, reference)
    ...
    tenth_kept = prune1305.prune_residual(ninth_kept, winner_scores, alt_scores, p1305.TENTH_PRUNE)

    champion = recall.precond.merge_with_cap(core, tenth_kept)
    champion_score = p1305.grade(champion, reference)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 13.05 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    results = {}
    evaluator = p1272.prune1163.evaluate
    for rule in RULES:
        eleventh_kept = prune_residual(tenth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, eleventh_kept)
        audit = evaluator(candidate, champion, reference, champion_score)
        results[rule["name"]] = {"rule": rule, "keptProposedAdditions": int(sum(eleventh_kept.values())), **audit}
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_13_05_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.05 eleventh pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_13_05_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.05-recall-champion-eleventh-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-eleventh-pruned-champion" if validated else "retain-13.05-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.05 CHAMPION ELEVENTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
