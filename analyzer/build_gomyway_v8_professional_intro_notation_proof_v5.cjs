...
const PDF_PATH = path.join(ROOT, 'public', 'gomyway-v8-professional-intro-notation-proof-v5.pdf');
const MANIFEST_PATH = path.join(ROOT, 'public', 'gomyway-v8-professional-intro-notation-proof-v5-manifest.json');
...
function drawPolyline(page, points, thickness = 1.4) {
...
function drawFilledArrowHead(page, tipX, tipY, direction) {
  const size = 5.5;
  if (direction === 'up') {
    page.drawSvgPath(`M ${tipX} ${tipY} L ${tipX - size} ${tipY - 8} L ${tipX + size} ${tipY - 8} Z`, { color: INK });
  } else {
    page.drawSvgPath(`M ${tipX} ${tipY} L ${tipX - size} ${tipY + 8} L ${tipX + size} ${tipY + 8} Z`, { color: INK });
  }
}
...
function drawProfessionalBendRelease(page, fonts, x, y) {
  const bendStart = { x: x + 3, y: y + 7 };
  const bendTop = { x: x + 11, y: y + 48 };
  const releaseTop = { x: x + 13, y: y + 47 };
  const releaseEnd = { x: x + 39, y: y + 24 };
  ...
  drawPolyline(page, quadraticPoints(bendStart, { x: x + 13, y: y + 17 }, bendTop, 24), 1.7);
  drawFilledArrowHead(page, bendTop.x, bendTop.y + 1, 'up');

  drawPolyline(page, quadraticPoints(releaseTop, { x: x + 37, y: y + 48 }, releaseEnd, 24), 1.7);
  drawFilledArrowHead(page, releaseEnd.x, releaseEnd.y - 1, 'down');
}

function drawVisibleVibrato(page, x1, x2, y) {
  const points = [];
  const width = Math.max(1, x2 - x1);
  const cycles = Math.max(2, Math.round(width / 12));
  const samples = cycles * 12;
  for (let index = 0; index <= samples; index += 1) {
    const ratio = index / samples;
    points.push({
      x: x1 + ratio * width,
      y: y + Math.sin(ratio * cycles * Math.PI * 2) * 2.4,
    });
  }
  drawPolyline(page, points, 1.35);
}
...
async function main() {
  ...
  page.drawText('Jimmy PAIge V8 — Professional Intro Notation Proof V5', { x: 220, y: 553, size: 11, font: fonts.bold, color: INK });
  page.drawText('TRAINING ONLY • standard 4/4 • curved bend/release • visible vibrato • clean tablature', { x: 210, y: 536, size: 7.5, font: fonts.body, color: rgb(0.32, 0.32, 0.32) });
  ...
  const manifest = {
    ...
    curvedBendReleaseArrowsRendered: bends,
  ...
  console.log('JIMMY PAIGE V8 PROFESSIONAL INTRO NOTATION PROOF V5');
  console.log('Passed:', manifest.passed);
  console.log('Events:', events.length);
  console.log('4/4 time signature rendered: true');
  console.log('Curved bend/release arrows rendered:', bends);
  console.log('Visible vibrato connectors rendered: 6');
