...
OUTPUT_PATH = PUBLIC / "gomyway-zero-precision-pruned-919-champion-residual-extras-profile-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-zero-precision-pruned-919-champion-residual-extras-profile-v1-manifest.json"
...
def main() -> None:
    ...
    print("Rebuilding validated 9.19 champion and profiling residual extras...", flush=True)
    ...
    prediction = precond.merge_with_cap(base_champion, additions_919)
    ...
    if actual != expected:
        raise RuntimeError(f"Expected validated 9.19 score {expected}, got {actual}")
    if abs(float(full["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected validated 9.19 F1 {EXPECTED_F1}, got {full['pitchF1']}")

    extras = prediction - reference
    matched = prediction & reference
    specialist_extras = Counter({token: count for token, count in extras.items() if token in additions_919})
    specialist_matches = Counter({token: count for token, count in matched.items() if token in additions_919})
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during 9.19 residual profile.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.19-winner13-recurrent-subgate-pruned-champion-residual-extra-profile",
        "championScore": full,
        "winner919Rule": {
            "name": gate919.RULE_NAME,
        ...
        "recommendedNextAction": "design-next-precision-prune-from-validated-9.19-residual-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY ZERO-PRECISION PRUNED 9.19 CHAMPION RESIDUAL EXTRAS PROFILE V1 COMPLETE")
    print("Passed: True")
