...
def capture_reserve_97_113_reference_free(source_audio: bytes, suffix: str = ".m4a") -> dict[str, Any]:
    """Capture raw attacks + whole-onset spectra for measures 97-113.
    ...
    with tempfile.TemporaryDirectory(prefix="v143-reserve-97-113-") as temp_dir:
        root = Path(temp_dir)
        source = root / f"uploaded{safe_suffix}"
        normalized = root / "normalized.wav"
        source.write_bytes(source_audio)
        if source.stat().st_size <= 0:
            raise RuntimeError("Reserve 97-113 source audio is empty")
        ...
        if not onset_groups:
            raise RuntimeError("No Reserve 97-113 physical onset groups produced")
        ...
        for index, group in enumerate(onset_groups, start=1):
            ...
            if index % 250 == 0:
                print(f"Reserve 97-113 onset spectra {index}/{len(onset_groups)}")
        ...
        return {
            "cacheVersion": 1,
            "scope": "reserve-97-113-measures-97-113-reference-free",
            "section": {"name": "Reserve 97-113", "startMeasure": FIRST_MEASURE, "endMeasure": LAST_MEASURE},
            "timing": {
...
@app.local_entrypoint(name="capture_reserve_97_113")
def capture_reserve_97_113(audio_path: str = str(DEFAULT_AUDIO_PATH)) -> None:
    ...
    print("=== V143 RESERVE 97-113 REFERENCE-FREE CAPTURE ===")
    print("Measures: 97-113")
    ...
    result = capture_reserve_97_113_reference_free.remote(payload, source.suffix)
