...
OUTPUT_PATH = PUBLIC / "gomyway-927-champion-step0-midi57-recurrent-and-both10-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-927-champion-step0-midi57-recurrent-and-both10-gate-v1-manifest.json"
...
EXPECTED_BASELINE = (102, 765, 1232)
EXPECTED_F1 = 9.27
NEW_RULE = {
    "name": "drop_step0_midi57_recurrent_and_both10",
    "selector": {"step": 0, "pitch": 57, "reason": "recurrent_and_both10"},
}
...
def residual_prune(additions, winner_scores, alt_scores, adaptive_base):
    ...
    for token, count in list(additions.items()):
        ...
        if step == 0 and pitch == 57 and reason == "recurrent_and_both10":
...
def main() -> None:
    ...
    print("Rebuilding validated 9.27 champion and step0/midi57 recurrent-and-both10 candidate...", flush=True)
    ...
    baseline_prediction = precond.merge_with_cap(base_champion, additions_927)
    ...
    if baseline_tuple != EXPECTED_BASELINE or abs(float(baseline_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected validated 9.27 baseline {EXPECTED_BASELINE}/{EXPECTED_F1}, got {baseline_tuple}/{baseline_score['pitchF1']}"
        )

    filtered = residual_prune(additions_927, winner_scores, alt_scores, adaptive_base)
    ...
    result["additionsDroppedFrom927"] = sum(additions_927.values()) - sum(filtered.values())
    validated_new_champion = bool(result["acceptedOverChampion"])

    after = sha256(CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 9.27 residual benchmark.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.27-champion-step0-midi57-recurrent-and-both10-gate",
    ...
    print("GOMYWAY 9.27 CHAMPION STEP0 MIDI57 RECURRENT_AND_BOTH10 GATE V1 COMPLETE")
    print("Passed: True")
