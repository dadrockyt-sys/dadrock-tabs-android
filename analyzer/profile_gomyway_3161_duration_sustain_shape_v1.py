...
import profile_gomyway_2769_dual_stem_duration_sustain_shape_v1 as duration
...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-3161-duration-sustain-shape-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-3161-duration-sustain-shape-v1-manifest.json"
...
def main() -> None:
    ...
    def record(tok: tuple[int, int, int], count: int, truth: str) -> None:
        ...
        wf = duration.duration_features(winner_audio, winner_sr, center)
        af = duration.duration_features(alt_audio, alt_sr, center)
        signatures = sorted(duration.signatures_for(wf, af))
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 31.61 duration/sustain-shape profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-31.61-dual-stem-duration-sustain-shape",
    ...
    print("GOMYWAY 31.61 DURATION SUSTAIN SHAPE V1 COMPLETE")
    print("Passed: True")
