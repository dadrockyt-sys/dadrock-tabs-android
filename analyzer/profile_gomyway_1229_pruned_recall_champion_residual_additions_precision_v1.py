...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1229-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1229-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
...
def main() -> None:
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 12.29 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {actual}/{champion_score['pitchF1']}")
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 12.29 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-12.29-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "recommendedNextAction": "design-next-zero-or-near-zero-precision-prune-from-validated-12.29-residual-recall-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 12.29 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
