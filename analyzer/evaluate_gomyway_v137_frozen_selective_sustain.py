...
FREEZE_PATH = (
    PUBLIC
    / "training"
    / "v137-selective-sustain-development"
    / "v137-frozen-selector.json"
)
OUT_PATH = (
    PUBLIC
    / "training"
    / "v137-selective-sustain-development"
    / "v137-heldout-evaluation.json"
)
...
if OUT_PATH.exists():
    raise RuntimeError(
        f"V137 held-out evaluation already exists at {OUT_PATH}; "
...
if not SPEC_PATH.is_file():
    raise RuntimeError(f"Frozen sustain spec missing: {SPEC_PATH}")
if not FREEZE_PATH.is_file():
    raise RuntimeError(f"Frozen V137 selector missing: {FREEZE_PATH}")

spec = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
freeze = json.loads(FREEZE_PATH.read_text(encoding="utf-8"))

if int(spec.get("interactionCount", -1)) != 15:
    raise RuntimeError("Expected 15 frozen sustain interactions")
if len(spec.get("interactions") or []) != 15:
    raise RuntimeError("Frozen sustain interaction list is not length 15")
if spec.get("tuningAfterFreezeAllowed") is not False:
    raise RuntimeError("Sustain spec is not frozen")
if spec.get("professionalMidtermAnswersUsed") is not False:
    raise RuntimeError("Sustain spec is contaminated by professional answers")

if int(freeze.get("schemaVersion", -1)) != 137:
    raise RuntimeError("V137 frozen selector schema mismatch")
if freeze.get("frozenBeforeHeldoutEvaluation") is not True:
    raise RuntimeError("V137 selector was not frozen before held-out evaluation")
if freeze.get("heldoutV134FoldScoredDuringDevelopment") is not False:
    raise RuntimeError("V137 development already scored V134 held-out folds")
if freeze.get("v136dEvaluationLossesUsed") is not False:
    raise RuntimeError("V137 selector used rejected V136D evaluation losses")
if freeze.get("professionalMidtermAnswersUsed") is not False:
    raise RuntimeError("V137 selector used professional/midterm answers")
if freeze.get("reserve11mod16Inspected") is not False:
    raise RuntimeError("Untouched 11-mod-16 reserve was inspected")

selected_rows = freeze.get("selected") or []
if len(selected_rows) != 3:
    raise RuntimeError(
        f"Expected exactly 3 frozen V137 selections, got {len(selected_rows)}"
    )
...
if len(selected_keys) != 3:
    raise RuntimeError("Frozen V137 selections are not unique")
...
for item in spec["interactions"]:
    ...
    if left not in name_to_idx or right not in name_to_idx:
        raise RuntimeError(
            f"Frozen sustain source column missing: {left} / {right}"
        )
    new_cols.append(
...
if sustain_x.shape[1] != 15:
    raise RuntimeError(
        f"Expected 15 sustain columns, got {sustain_x.shape[1]}"
    )
if not np.isfinite(sustain_x).all():
    raise RuntimeError("Non-finite sustain interaction values")
...
print("=== V137 FROZEN SELECTIVE SUSTAIN HELD-OUT EVALUATION ===")
...
print("=== V137 ONE-SHOT HELD-OUT SCORING ===", flush=True)
...
for index, row in enumerate(carriers, 1):
    ...
    if key in selected_keys:
        if rep != "full_phase":
            raise RuntimeError(
                f"Frozen V137 selection no longer resolves to full_phase: "
                f"phase={phase} fold={fold} rep={rep}"
            )
        matrix = full_phase_sustain
        applied.append({"phase": phase, "fold": fold})
    else:
        matrix = matrices[rep]

    result = evaluate_one(row, matrix)
    v137_results.append(result)

    if index % 40 == 0:
        print(f"V137 heartbeat {index}/320", flush=True)

if len(applied) != 3:
    raise RuntimeError(
        f"Expected exactly 3 V137 interventions, applied {len(applied)}"
    )

v137_passes = sum(int(item["pass"]) for item in v137_results)

gains = 0
losses = 0
changed = []

for base_result, candidate_result in zip(base_results, v137_results):
        ...
        changed.append(
            {
                ...
                "v137Lift": candidate_result["lift"],
...
if candidate_before != candidate_after:
    raise RuntimeError(
        "Protected 949-event candidate changed during V137 evaluation"
    )

promotion_candidate = bool(
    v137_passes > EXPECTED_BASE_PASSES
    and gains >= 1
    and losses == 0
)

out = {
    "experiment": "V137-frozen-selective-sustain-heldout-evaluation",
    "baseline": {
        "name": "V134",
        "passes": base_passes,
...
print()
print("=== V137 HELD-OUT EVALUATION COMPLETE ===")
print(f"V134 baseline: {base_passes}/320")
print(f"V137 selective sustain: {v137_passes}/320")
print(f"Gains/losses vs V134: +{gains}/-{losses} net={gains - losses:+d}")
print("Frozen V137 interventions applied:", len(applied))
