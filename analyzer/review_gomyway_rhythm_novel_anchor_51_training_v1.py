...
TRAINING_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-51-training-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-51-training-review-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-51-training-review-v1-manifest.json"
...
def main() -> None:
    ...
    if training.get("passed") is not True:
        raise RuntimeError("Novel anchor 51 training V1 is not green.")
    if training.get("measureNumber") != TARGET_MEASURE:
        raise RuntimeError("Novel anchor training target changed unexpectedly.")
    if training.get("readyForNovelAnchor51TrainingReview") is not True:
    ...
    if training.get("anchor36CoverageGainsPreserved") != EXPECTED_ANCHOR36_GAINS:
    ...
    if rhythm.get("supported") is not True:
        raise RuntimeError("Measure 51 rhythm evidence is not supported.")
    ...
    if rhythm.get("stepMultiplicities") != EXPECTED_MULTIPLICITIES:
        raise RuntimeError("Measure 51 step multiplicities changed unexpectedly.")
    if int(rhythm.get("unquantizedEventCount", -1)) != 0:
        raise RuntimeError("Measure 51 contains unexpected unquantized events.")

    if chord.get("observationAvailable") is not False:
        raise RuntimeError("Measure 51 unexpectedly has chord-shape evidence.")
    if chord.get("multiNoteSteps") != []:
        raise RuntimeError("Measure 51 unexpectedly contains multi-note steps.")
    if timing.get("observationAvailable") is not False:
        raise RuntimeError("Measure 51 unexpectedly has timing evidence.")
    if technique.get("observationAvailable") is not False:
        raise RuntimeError("Measure 51 unexpectedly has technique evidence.")
    if int(technique.get("explicitTechniqueEventCount", -1)) != 0:
        raise RuntimeError("Measure 51 unexpectedly contains explicit technique events.")
    ...
    recommended = (
        "register-gomyway-rhythm-novel-anchor-51-read-only-v1"
        if passed
        else "review-gomyway-rhythm-novel-anchor-51-training-v1"
    )
    ...
    output = {
        ...
        "readOnlyReview": True,
        "readyForNovelAnchor51ReadOnlyRegistration": passed,
    ...
    print("GOMYWAY RHYTHM NOVEL ANCHOR 51 TRAINING REVIEW V1 COMPLETE")
    print("Passed:", passed)
    ...
    print("Ready for novel anchor 51 read-only registration:", passed)
