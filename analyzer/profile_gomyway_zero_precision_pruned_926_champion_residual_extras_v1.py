...
OUTPUT_PATH = PUBLIC / "gomyway-zero-precision-pruned-926-champion-residual-extras-profile-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-zero-precision-pruned-926-champion-residual-extras-profile-v1-manifest.json"
...
def main() -> None:
    ...
    print("Rebuilding validated 9.26 champion and profiling residual extras...", flush=True)
    ...
    prediction = precond.merge_with_cap(base_champion, additions_926)
    ...
    if actual != EXPECTED or abs(float(full["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected validated 9.26 {EXPECTED}/{EXPECTED_F1}, got {actual}/{full['pitchF1']}")

    specialist_extras = Counter({t: c for t, c in (prediction - reference).items() if t in additions_926})
    specialist_matches = Counter({t: c for t, c in (prediction & reference).items() if t in additions_926})
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 9.26 residual profile.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.26-deep-zero-precision-pruned-champion-residual-profile",
        "championScore": full,
        "winner926Rule": WINNER_926_RULE,
        ...
        "recommendedNextAction": "design-next-precision-prune-from-validated-9.26-residual-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY ZERO-PRECISION PRUNED 9.26 CHAMPION RESIDUAL EXTRAS PROFILE V1 COMPLETE")
    print("Passed: True")
    ...
    for label, key, limit in [
        ...
        ("Section/step/pitch precision", "sectionStepPitch", 30),
    ]:
        print(label + ":")
