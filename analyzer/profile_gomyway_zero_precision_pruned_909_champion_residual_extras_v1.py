...
OUTPUT_PATH = PUBLIC / "gomyway-zero-precision-pruned-909-champion-residual-extras-profile-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-zero-precision-pruned-909-champion-residual-extras-profile-v1-manifest.json"
...
def main() -> None:
    ...
    print("Rebuilding validated 9.09 zero-precision-pruned champion...", flush=True)
    ...
    winner_additions = crossgate.cross_signature_prune(residual_additions, winner_scores, alt_scores, adaptive_base, ZERO_PRECISION_RULE)
    prediction = precond.merge_with_cap(base_champion, winner_additions)
    ...
    if actual != expected:
        raise RuntimeError(f"Expected validated 9.09 score {expected}, got {actual}")
    if abs(float(full["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected validated 9.09 F1 {EXPECTED_F1}, got {full['pitchF1']}")

    extras = prediction - reference
    matched = prediction & reference
    specialist_extras = Counter({token: count for token, count in extras.items() if token in winner_additions})
    specialist_matches = Counter({token: count for token, count in matched.items() if token in winner_additions})
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during 9.09 residual profile.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.09-zero-precision-pruned-champion-residual-extra-profile",
        "championScore": full,
        "safeCrossSignatureRule": SAFE_CROSS_RULE,
        "zeroPrecisionWinnerRule": ZERO_PRECISION_RULE,
        ...
        "recommendedNextAction": "design-next-precision-prune-from-validated-9.09-residual-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY ZERO-PRECISION PRUNED 9.09 CHAMPION RESIDUAL EXTRAS PROFILE V1 COMPLETE")
    print("Passed: True")
