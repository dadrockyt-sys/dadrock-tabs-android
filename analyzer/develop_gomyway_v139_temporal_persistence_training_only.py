...
SPEC_PATH = (
    PUBLIC
    / "training"
    / "v139-temporal-persistence-development"
    / "v139-persistence-spec.json"
)
OUT_PATH = (
    PUBLIC
    / "training"
    / "v139-temporal-persistence-development"
    / "v139-training-only-selector.json"
)
...
if spec.get("heldoutV134ScoringAllowedDuringDevelopment") is not False:
    raise RuntimeError("V139 spec allows held-out V134 scoring during development")
...
if PHASE_OFFSETS != [0.0, 0.125]:
    raise RuntimeError(f"Unexpected frozen V139 phase offsets: {PHASE_OFFSETS}")
        ...
        inner_rows.append(
            {
                ...
                "v134Pass": base["pass"],
                "v139Pass": challenger["pass"],
                "v134Lift": base["lift"],
                "v139Lift": challenger["lift"],
                "liftDelta": round(challenger["lift"] - base["lift"], 6),
            }
        )

    gains = sum(
        int(item["v139Pass"] and not item["v134Pass"])
        for item in inner_rows
    )
    losses = sum(
        int(item["v134Pass"] and not item["v139Pass"])
        for item in inner_rows
    )
...
def training_only_decision(row: dict) -> dict:
    ...
    challenger_matrix = np.concatenate([base_matrix, persistence_x], axis=1)
    ...
    views = [
        evaluate_view(
            row, base_matrix, challenger_matrix, outer_train, offset
        )
    ...
    return {
        ...
        "reason": (
            "replicated frozen V139 training-only dominance"
            if selected
            else "replicated V139 dominance rule not met"
        ),
    }


print("=== V139 TEMPORAL PERSISTENCE TRAINING-ONLY DEVELOPMENT ===")
print("Frozen feature recipe loaded before scoring: True")
print("Temporal persistence features:", persistence_x.shape[1])
...
if candidate_before != candidate_after:
    raise RuntimeError(
        "Protected 949-event candidate changed during V139 development"
    )

out = {
    "experiment": "V139-temporal-persistence-training-only-development",
    "baselineChampion": {
        "name": "V134",
        "passes": 311,
        "total": 320,
    ...
    "frozenPersistenceSpec": {
        "path": str(SPEC_PATH.relative_to(ROOT)),
        "sha256": sha256(SPEC_PATH),
        "featureCount": int(persistence_x.shape[1]),
        "featureNames": persistence_names,
    },
    "selector": {
        "rule": rule,
...
print()
print("=== V139 DEVELOPMENT COMPLETE ===")
print(f"Training-only persistence selections: {len(selected)}/320")
