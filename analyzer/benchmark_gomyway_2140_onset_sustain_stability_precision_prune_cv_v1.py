...
PUBLIC = ROOT / "public"
PROFILE_PATH = PUBLIC / "gomyway-2140-dual-stem-harmonic-onset-sustain-stability-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-2140-onset-sustain-stability-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2140-onset-sustain-stability-precision-prune-cv-v1-manifest.json"
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    early_b = stability.retention_bucket(float(row["minEarlyRetention"]))
    sustain_b = stability.retention_bucket(float(row["minSustainRetention"]))
    sve_b = stability.retention_bucket(float(row["minSustainVsEarly"]))
    decay_b = stability.decay_bucket(float(row["minDecayMargin"]))
    diff_b = stability.diff_bucket(float(row["sustainRetentionStemDifference"]))
    return {
        f"earlyRetention::{early_b}",
        f"sustainRetention::{sustain_b}",
        f"sustainVsEarly::{sve_b}",
        f"decayMargin::{decay_b}",
        f"stemRetentionDiff::{diff_b}",
        f"retentionCross::{early_b}|{sustain_b}|{diff_b}",
        f"decayCross::{sustain_b}|{decay_b}|{diff_b}",
        f"fullStabilityCross::{early_b}|{sustain_b}|{sve_b}|{decay_b}|{diff_b}",
    }
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("21.40 onset-sustain profile has no rows")
    ...
    if not zero_rows:
        raise RuntimeError("21.40 onset-sustain profile has no zero-precision signatures to benchmark")
    ...
    champion = stability.reconstruct_2140(grid, winner_audio, winner_sr, alt_audio, alt_sr)
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 21.40 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"onsetsustain2140_exact_{idx:02d}", make_exact_predicate(signature)))
    ...
    for n in (3, 5, 10):
        if len(exact_signatures) >= n:
            variants.append((f"onsetsustain2140_union_top{n}", union_pred(exact_signatures[:n])))
    variants.append(("onsetsustain2140_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2140": accepted_over_2140,
        }
        results[name] = result
        print(
            ...
            f"sections={result['sectionStabilityPassed']} shifted={result['shiftedWindowStabilityPassed']} accepted={accepted_over_2140}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2140:
    ...
    if accepted:
        ...
        winner_name = "retain_21_40_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "extraReduction": 0,
            ...
            "acceptedOver2140": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 21.40 onset-sustain precision prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-21.40-onset-sustain-stability-precision-prune-with-heldout-cv",
        "baseline2140Score": baseline,
    ...
    manifest = {
        ...
        "baseline2140PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 21.40 ONSET-SUSTAIN STABILITY PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
