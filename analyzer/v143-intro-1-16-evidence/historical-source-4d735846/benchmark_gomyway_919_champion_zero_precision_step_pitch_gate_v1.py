...
OUTPUT_PATH = PUBLIC / "gomyway-919-champion-zero-precision-step-pitch-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-919-champion-zero-precision-step-pitch-gate-v1-manifest.json"
...
RULES: list[dict[str, Any]] = [
    {"name": "drop_step0_midi55", "dropStepPitch": [{"step": 0, "pitch": 55}]},
    {"name": "drop_step0_midi62", "dropStepPitch": [{"step": 0, "pitch": 62}]},
    {"name": "drop_step10_midi60", "dropStepPitch": [{"step": 10, "pitch": 60}]},
    {"name": "drop_step10_midi62", "dropStepPitch": [{"step": 10, "pitch": 62}]},
    {"name": "drop_step10_midi63", "dropStepPitch": [{"step": 10, "pitch": 63}]},
    {"name": "drop_step4_midi63", "dropStepPitch": [{"step": 4, "pitch": 63}]},
    {
        "name": "drop_midi62_zero_precision_pair",
        "dropStepPitch": [{"step": 0, "pitch": 62}, {"step": 10, "pitch": 62}],
    },
    {
        "name": "drop_midi63_zero_precision_pair",
        "dropStepPitch": [{"step": 10, "pitch": 63}, {"step": 4, "pitch": 63}],
    },
    {
        "name": "drop_all_profiled_zero_precision_step_pitch_919",
        "dropStepPitch": [
            {"step": 0, "pitch": 55},
            {"step": 0, "pitch": 62},
            {"step": 10, "pitch": 60},
...
def main() -> None:
    ...
    print("Rebuilding validated 9.19 champion and zero-precision step-pitch candidates...", flush=True)
    ...
    additions_919 = gate919.refined_subgate(additions_916, winner_scores, alt_scores, adaptive_base)

    baseline_prediction = precond.merge_with_cap(base_champion, additions_919)
    ...
    if actual != expected:
        raise RuntimeError(f"Expected validated 9.19 baseline {expected}, got {actual}")
    ...
    for rule in RULES:
        filtered_additions = crossgate.cross_signature_prune(additions_919, winner_scores, alt_scores, adaptive_base, rule)
        ...
        result["rule"] = rule
        result["additionsDroppedFrom919"] = sum(additions_919.values()) - sum(filtered_additions.values())
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during 9.19 step-pitch benchmark.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.19-champion-zero-precision-step-pitch-gate",
    ...
    print("GOMYWAY 9.19 CHAMPION ZERO-PRECISION STEP-PITCH GATE V1 COMPLETE")
    print("Passed: True")
