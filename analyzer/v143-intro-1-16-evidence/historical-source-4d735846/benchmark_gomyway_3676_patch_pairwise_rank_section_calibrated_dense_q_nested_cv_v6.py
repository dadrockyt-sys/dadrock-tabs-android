...
OUTPUT_PATH = PUBLIC / "gomyway-3676-patch-pairwise-rank-section-calibrated-dense-q-nested-cv-v6.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-patch-pairwise-rank-section-calibrated-dense-q-nested-cv-v6-manifest.json"
EXPECTED = (272, 595, 341)
EXPECTED_F1 = 36.76
OUTER_FOLDS = 5
PAIR_RADII = list(v2.PAIR_RADII)
LAMBDAS = list(v2.LAMBDAS)
# V5 used [0.05, 0.075, 0.10, 0.15].
# V6 changes only the legal training-only operating-point grid.
TAIL_QUANTILES = [0.025, 0.05, 0.075, 0.10, 0.125, 0.15, 0.20]
    ...
    for radius in PAIR_RADII:
        for lam in LAMBDAS:
            ...
            if done == 1 or done % 3 == 0 or done == total:
                print(f"    heartbeat dense-q section-calibration model search {done}/{total}", flush=True)
...
def evaluate_scheme(x: np.ndarray, y: np.ndarray, measures: np.ndarray, feature_names: list[str], name: str, fold_fn: Callable[[int], int]) -> tuple[bool, list[dict[str, Any]]]:
    ...
    for fold in range(OUTER_FOLDS):
        ...
        row = {
            ...
            "operatingPointCalibration": "training-only-inner-contiguous-section-priority-dense-q-grid",
...
def main() -> None:
    ...
    print("Starting V6 dense-q training-only section-aware operating-point calibration", flush=True)
    print("Ranker/sampler/features/radius/lambda unchanged from V5; only q grid expanded", flush=True)
    print("Dense q grid:", TAIL_QUANTILES, flush=True)
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during V6 dense-q section-aware calibration CV")

    output = {
        "schemaVersion": 6,
        "profileType": "36.76-patch-pairwise-rank-section-calibrated-dense-q-nested-cv-exploratory",
        "baselinePitchF1": EXPECTED_F1,
        "baselineMatchedMissingExtra": list(EXPECTED),
        "pairSampler": "deterministic-measure-stratified-v2",
        "operatingPointCalibration": "training-only-inner-contiguous-section-priority-dense-q-grid",
        "tailQuantileGrid": TAIL_QUANTILES,
        ...
        "pairwiseRankPatchV6Generalizes": generalizes,
        "normal": normal,
        "section": section,
    ...
    print("GOMYWAY 36.76 PATCH PAIRWISE RANK SECTION-CALIBRATED DENSE-Q NESTED CV V6 COMPLETE")
    ...
    print("Pairwise-rank patch V6 generalizes:", generalizes)
