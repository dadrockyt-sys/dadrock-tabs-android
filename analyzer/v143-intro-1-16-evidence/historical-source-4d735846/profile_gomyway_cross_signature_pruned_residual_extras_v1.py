...
OUTPUT_PATH = PUBLIC / "gomyway-cross-signature-pruned-residual-extras-profile-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-cross-signature-pruned-residual-extras-profile-v1-manifest.json"
...
def main() -> None:
    ...
    print("Rebuilding frozen 9.04 champion and validated one-extra cross-signature prune...", flush=True)
    ...
    full = grade(residual_prediction, reference)

    expected = (EXPECTED_MATCHED, EXPECTED_MISSING, EXPECTED_EXTRA)
    actual = (int(full["matched"]), int(full["missing"]), int(full["extra"]))
    if actual != expected:
        raise RuntimeError(f"Expected validated residual score {expected}, got {actual}")
    if abs(float(full["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected residual F1 {EXPECTED_F1}, got {full['pitchF1']}")

    extras = residual_prediction - reference
    matched = residual_prediction & reference
    specialist_extras = Counter({token: count for token, count in extras.items() if token in residual_additions})
    specialist_matches = Counter({token: count for token, count in matched.items() if token in residual_additions})
    ...
    dimensions = {
        ...
        "scoreAgreement": (Counter(), Counter()),
        "stepScore": (Counter(), Counter()),
    }
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during residual extras profile.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-cross-signature-pruned-residual-extra-profile",
        "residualScore": full,
        ...
        "recommendedNextAction": "design-second-orthogonal-safe-prune-from-1287-extra-residual-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
        ...
        "residualPitchF1": full["pitchF1"],
        "residualExtra": full["extra"],
        "productionPromotionAllowed": False,
    }
    OUTPUT_PATH.write_text(json.dumps(output, indent=2) + "\n", encoding="utf-8")
    MANIFEST_PATH.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")

    print("GOMYWAY CROSS-SIGNATURE PRUNED RESIDUAL EXTRAS PROFILE V1 COMPLETE")
    print("Passed: True")
    print("Residual pitch F1:", full["pitchF1"])
    print("Residual matched/missing/extra:", full["matched"], "/", full["missing"], "/", full["extra"])
    print("Safe cross-signature rule:", SAFE_CROSS_RULE["name"])
