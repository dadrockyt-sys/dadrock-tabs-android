...
OUTPUT_PATH = PUBLIC / "gomyway-916-champion-recurrent-step8-or-midi57-subgate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-916-champion-recurrent-step8-or-midi57-subgate-v1-manifest.json"
...
EXPECTED_F1 = 9.16
RULE_NAME = "drop_score13_16_both_ge10_recurrent_step8_or_midi57"
...
def main() -> None:
    ...
    print("Rebuilding validated 9.16 champion and provenance-derived detector subgate...", flush=True)
    ...
    if baseline_tuple != EXPECTED_BASELINE or abs(float(baseline_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected validated 9.16 baseline {EXPECTED_BASELINE} / {EXPECTED_F1}, "
            f"got {baseline_tuple} / {baseline_score['pitchF1']}"
        )

    candidate_additions = provenance_subgate(additions_916, winner_scores, alt_scores, adaptive_base)
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during provenance subgate benchmark.")

    validated_new_champion = bool(result["acceptedOverChampion"])
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.16-provenance-derived-detector-subgate",
    ...
    print("GOMYWAY 9.16 CHAMPION RECURRENT STEP8 OR MIDI57 SUBGATE V1 COMPLETE")
    print("Passed: True")
