...
import profile_gomyway_1312_pruned_recall_champion_residual_additions_precision_v1 as p1312
import benchmark_gomyway_1312_champion_thirteenth_zero_precision_pitch_strength_prune_v1 as prune1315

v2 = p1312.v2
v3 = p1312.v3
recall = p1312.recall
profile = p1312.profile

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1315-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1315-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1576)
EXPECTED_F1 = 13.15
WINNER_NAME = "dual6_quarter_40_64"
THIRTEENTH_PRUNE = next(
    r for r in prune1315.RULES
    if r["name"] == "drop_all_thirteenth_profiled_zero_precision_pitch_strength"
)
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.15 pruned recall champion...", flush=True)
    ...
    for (measure, step), center in grid.items():
        if 17 <= measure <= 113:
            for pitch in range(recall.PITCH_MIN, recall.PITCH_MAX + 1):
                ...
                winner_scores[token] = recall.spectral.spectral_score(
                    winner_audio, winner_sr, float(center), pitch
                )
                alt_scores[token] = recall.spectral.spectral_score(
                    alt_audio, alt_sr, float(center), pitch
                )
    ...
    eleventh_kept = p1308.prune1308.prune_residual(
        tenth_kept, winner_scores, alt_scores, p1308.ELEVENTH_PRUNE
    )
    twelfth_kept = p1312.prune1312.prune_residual(
        eleventh_kept, winner_scores, alt_scores, p1312.TWELFTH_PRUNE
    )
    thirteenth_kept = prune1315.prune_residual(
        twelfth_kept, winner_scores, alt_scores, THIRTEENTH_PRUNE
    )

    champion = recall.precond.merge_with_cap(core, thirteenth_kept)
    champion_score = grade(champion, reference)
    actual = (
        champion_score["matched"],
        champion_score["missing"],
        champion_score["extra"],
    )
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected 13.15 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, "
    ...
    for token, count in effective.items():
        ...
        f = count - t
        strength = profile.bucket(
            min(winner_scores.get(token, 0.0), alt_scores.get(token, 0.0))
        )
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.15 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.15-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "precisionTables": {
            name: rows(table) for name, table in tables.items()
        },
        ...
        "recommendedNextAction": "design-next-zero-or-near-zero-precision-prune-from-validated-13.15-residual-recall-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.15 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
    ...
    print(
        "Effective residual true / false:",
        sum(effective_true.values()),
        "/",
        sum((effective - effective_true).values()),
    )
    for title, name in [
        ("Strength precision", "strength"),
        ("Step precision", "step"),
        ("Pitch precision", "pitch"),
        ("Top step/pitch precision", "stepPitch"),
        ("Step/strength precision", "stepStrength"),
        ("Pitch/strength precision", "pitchStrength"),
    ]:
        print(title + ":")
        for row in output["precisionTables"][name][:25]:
            print(
                f"  {row['bucket']}: true={row['true']} false={row['false']} "
