...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1163-champion-second-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1163-champion-second-zero-precision-pitch-strength-prune-v1-manifest.json"

EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (161, 706, 1740)
EXPECTED_F1 = 11.63
...
RULES = [
    {"name": "drop_second_zero_low_strength", "drop": sorted([list(x) for x in SECOND_ZERO_BUCKETS if x[1] in {"6_8", "8_10"}])},
    {"name": "drop_second_zero_high_strength", "drop": sorted([list(x) for x in SECOND_ZERO_BUCKETS if x[1] not in {"6_8", "8_10"}])},
    {"name": "drop_all_second_profiled_zero_precision_pitch_strength", "drop": sorted([list(x) for x in SECOND_ZERO_BUCKETS])},
]
...
def prune_residual(proposed: Counter[tuple[int, int, int]], winner_scores, alt_scores, rule):
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 11.63 pruned recall champion...", flush=True)
    ...
    champion = recall.precond.merge_with_cap(core, first_kept)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 11.63 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    results = {}
    for rule in RULES:
        second_kept = prune_residual(first_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, second_kept)
        audit = prune1163.evaluate(candidate, champion, reference, champion_score)
        results[rule["name"]] = {"rule": rule, "keptProposedAdditions": int(sum(second_kept.values())), **audit}
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_11_63_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 11.63 second pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_11_63_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-11.63-recall-champion-second-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-second-pruned-champion" if validated else "retain-11.63-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 11.63 CHAMPION SECOND ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
