...
import benchmark_gomyway_1285_champion_tenth_zero_precision_pitch_strength_prune_v1 as prune1305
...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1305-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1305-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1597)
EXPECTED_F1 = 13.05
WINNER_NAME = "dual6_quarter_40_64"
TENTH_PRUNE = next(r for r in prune1305.RULES if r["name"] == "drop_all_tenth_profiled_zero_precision_pitch_strength")
...
def main() -> None:
    ...
    tenth_kept = prune1305.prune_residual(ninth_kept, winner_scores, alt_scores, TENTH_PRUNE)
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 13.05 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {actual}/{champion_score['pitchF1']}")
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.05 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.05-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "recommendedNextAction": "design-next-zero-or-near-zero-precision-prune-from-validated-13.05-residual-recall-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.05 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
