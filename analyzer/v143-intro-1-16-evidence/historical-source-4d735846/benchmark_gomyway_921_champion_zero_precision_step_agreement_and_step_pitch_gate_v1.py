...
OUTPUT_PATH = PUBLIC / "gomyway-921-champion-zero-precision-step-agreement-and-step-pitch-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-921-champion-zero-precision-step-agreement-and-step-pitch-gate-v1-manifest.json"
...
EXPECTED_BASELINE = (102, 765, 1246)
EXPECTED_F1 = 9.21

RULES: list[dict[str, Any]] = [
    {
        "name": "drop_step12_single_stem_or_weak_second",
        "dropStepAgreement": [{"step": 12, "agreement": "single_stem_or_weak_second"}],
    },
    {
        "name": "drop_step8_midi53_55",
        "dropStepPitch": [{"step": 8, "pitch": 53}, {"step": 8, "pitch": 55}],
    },
    {
        "name": "drop_step12_single_weak_plus_step8_midi53_55",
        "dropStepAgreement": [{"step": 12, "agreement": "single_stem_or_weak_second"}],
        "dropStepPitch": [{"step": 8, "pitch": 53}, {"step": 8, "pitch": 55}],
    },
]
...
def main() -> None:
    ...
    print("Rebuilding validated 9.21 champion and next zero-precision candidates...", flush=True)
    ...
    additions_921 = crossgate.cross_signature_prune(additions_919, winner_scores, alt_scores, adaptive_base, WINNER_921_RULE)

    baseline_prediction = precond.merge_with_cap(base_champion, additions_921)
    ...
    if baseline_tuple != EXPECTED_BASELINE or abs(float(baseline_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected validated 9.21 baseline {EXPECTED_BASELINE} / {EXPECTED_F1}, got {baseline_tuple} / {baseline_score['pitchF1']}")

    results: dict[str, Any] = {}
    for rule in RULES:
        filtered = crossgate.cross_signature_prune(additions_921, winner_scores, alt_scores, adaptive_base, rule)
        ...
        result["rule"] = rule
        result["additionsDroppedFrom921"] = sum(additions_921.values()) - sum(filtered.values())
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during 9.21 zero-precision benchmark.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.21-champion-zero-precision-step-agreement-and-step-pitch-gate",
    ...
    print("GOMYWAY 9.21 CHAMPION ZERO-PRECISION STEP-AGREEMENT/STEP-PITCH GATE V1 COMPLETE")
    print("Passed: True")
