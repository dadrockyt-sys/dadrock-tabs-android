...
OUTPUT_PATH = PUBLIC / "gomyway-3676-patch-rhythm24-oof-robust-pass-threshold-consensus-v32.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-patch-rhythm24-oof-robust-pass-threshold-consensus-v32-manifest.json"
...
def widest_passing_threshold(percentiles: np.ndarray, yy: np.ndarray) -> tuple[float | None, dict[str, Any]]:
    ...
    best = None
    best_stats = None
    # Lowest percentile threshold = widest selection. Choose the widest training-only
    # selection that still clears the frozen +5 point pass criterion.
    for threshold in candidates:
        ...
        if true > 0 and lift >= MIN_LIFT:
            ...
            break
    return best, {"base": base, "passing": best_stats}
...
def derive_training_only_cutoff(
    ...
    if not scheme_medians:
        raise RuntimeError("No training-only inner fold produced a passing threshold")
    cutoff = float(np.median(np.asarray(scheme_medians, dtype=np.float64)))
    return cutoff, {
        "architecture": "training-only-cross-boundary-robust-pass-threshold-consensus",
        "criterion": "widest-inner-validation-selection-with-lift-at-least-5",
        "aggregation": "median-within-scheme-then-median-across-schemes",
...
def evaluate_phase(x: np.ndarray, y: np.ndarray, measures: np.ndarray, phase: float) -> tuple[int, list[dict[str, Any]]]:
    ...
    for fold in range(OUTER_FOLDS):
        ...
        test = ids == fold
        train = ~test
        print("    heartbeat V32 frozen V17 representation/model-selection", flush=True)
        ...
        print("    heartbeat V32 training-only robust percentile threshold consensus", flush=True)
        ...
        print(f"  V32 cutoff={cutoff:.6f} held={true}/{false} lift={round(lift,2)} pass={passed}; V28 pass={v28_passed}", flush=True)
    return passes, rows
...
def main() -> None:
    ...
    print("Starting V32 training-only robust pass-threshold consensus challenger", flush=True)
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during V32")

    output = {
        "schemaVersion": 32,
        "profileType": "36.76-rhythm24-training-only-oof-robust-pass-threshold-consensus",
        ...
        "calibrationArchitecture": "cross-boundary-OOF-robust-pass-threshold-consensus",
    ...
    print("GOMYWAY 36.76 RHYTHM24 OOF ROBUST PASS-THRESHOLD CONSENSUS V32 COMPLETE")
    print("V32 folds passed:", total_passes, "/", total_folds)
    print("Minimum V32 phase passes:", min_phase, "/ 5")
