...
app = modal.App("dadrock-gomyway2-lead-technique-handoff-benchmark")
...
def add_lead_technique_handoff(
    ...
    ordered = sorted((dict(event) for event in events), key=voicing.base.event_start)
    release_pairs: list[dict[str, Any]] = []
    palm_muted_indices: list[int] = []

    # A full bend in the supplied reference resolves from fret 14 to fret 12.
    # Preserve any existing bend flag, then attach release metadata to both
    # events when that playable pair appears close together in time.
    for index, event in enumerate(ordered):
        if voicing.event_fret(event) != 14:
            continue
        if not has_bend(event):
            continue

        bend_start = voicing.base.event_start(event)
        for release_index in range(index + 1, min(index + 4, len(ordered))):
            release_event = ordered[release_index]
            release_start = voicing.base.event_start(release_event)
            if release_start - bend_start > 1.0:
                break
...
def analyse_techniques(
    ...
    technique_events, technique_diagnostics = add_lead_technique_handoff(
        voiced_events
    )
    ...
    report = {
        ...
        "passed": (
            ...
            and int(technique_diagnostics.get("palmMutedEventCount") or 0) >= 1
        ...
        "trainingRule": (
            "benchmark-only-technique-handoff; no pitches, frets, V71, V72, "
            "or locked baselines may change"
        ),
    ...
    payload = {
        "report": report,
        "techniqueLeadEvents": technique_events,
    }
    return json.dumps(
        payload,
...
def main(
    ...
    report_output: str = "/tmp/gomyway2-lead-technique-handoff-report.json",
) -> None:
    ...
    Path("/tmp/gomyway2-lead-technique-events.json").write_text(
        json.dumps(
    ...
    print("Saved events: /tmp/gomyway2-lead-technique-events.json")
