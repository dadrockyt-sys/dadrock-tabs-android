...
import profile_gomyway_3118_measure_position_survivors_precision_v1 as survivor
...
PUBLIC = ROOT / "public"
PROFILE_PATH = PUBLIC / "gomyway-3118-measure-register-distribution-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-3118-measure-register-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-3118-measure-register-precision-prune-cv-v1-manifest.json"
EXPECTED = (183, 684, 124)
EXPECTED_F1 = 31.18
EXPECTED_ZERO_SIGNATURES = 8
FOLD_COUNT = 5
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing measure-register signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("31.18 measure-register profile has no rows")
    ...
    if len(zero_rows) != EXPECTED_ZERO_SIGNATURES:
        raise RuntimeError(
            f"Expected exactly {EXPECTED_ZERO_SIGNATURES} measure-register zero-precision signatures, got {len(zero_rows)}"
        )
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision measure-register row: {row}")
    ...
    champion, _ = survivor.reconstruct_3118(
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected frozen 31.18 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}"
        )

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"measureregister3118_exact_{idx:02d}", make_exact_predicate(signature)))

    for n in (2, 3, 5, len(exact_signatures)):
        label = "all" if n == len(exact_signatures) else f"top{n}"
        variants.append((f"measureregister3118_union_{label}_zero_precision", union_pred(exact_signatures[:n])))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver3118": accepted_over_3118,
        }
        results[name] = result
        print(
            ...
            f"accepted={accepted_over_3118}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_3118:
    ...
    if accepted:
        ...
        winner_name = "retain_31_18_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "truePruned": 0,
            ...
            "acceptedOver3118": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 31.18 measure-register precision-prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-31.18-measure-register-precision-prune-with-heldout-cv",
        "baseline3118Score": baseline,
    ...
    manifest = {
        ...
        "baseline3118PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 31.18 MEASURE REGISTER PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
