...
v124 = v134.v124
DEV_DIR = PUBLIC / "training" / "v142-temporal-slope-development"
SPEC_PATH = DEV_DIR / "v142-temporal-slope-spec.json"
OUT_PATH = DEV_DIR / "v142-training-only-selector.json"
...
if not SPEC_PATH.is_file():
    raise RuntimeError(f"Frozen V142 temporal-slope spec missing: {SPEC_PATH}")

spec = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
if int(spec.get("schemaVersion", -1)) != 142:
    raise RuntimeError("V142 temporal-slope spec schema mismatch")
if spec.get("frozenBeforeDevelopmentRun") is not True:
    raise RuntimeError("V142 feature recipe was not frozen before development")
if int(spec.get("featureCount", -1)) != 20:
    raise RuntimeError("Expected exactly 20 frozen V142 temporal-slope features")
if spec.get("heldoutV134ScoringAllowedDuringDevelopment") is not False:
    raise RuntimeError("V142 spec allows held-out V134 scoring during development")
for key in (
    ...
    if spec.get(key) is not False:
        raise RuntimeError(f"V142 frozen safety spec violation: {key}={spec.get(key)}")
if spec.get("qTuningAllowed") is not False:
    raise RuntimeError("V142 spec allows Q tuning")
if spec.get("lambdaTuningAllowed") is not False:
    raise RuntimeError("V142 spec allows lambda tuning")
if spec.get("pairRadiusTuningAllowed") is not False:
    raise RuntimeError("V142 spec allows pair-radius tuning")
...
if PHASE_OFFSETS != [0.0, 0.125]:
    raise RuntimeError(f"Unexpected frozen V142 phase offsets: {PHASE_OFFSETS}")
...
if bands != ["low", "lowMid", "mid", "highMid", "high"]:
    raise RuntimeError(f"Unexpected frozen V142 bands: {bands}")

slope_cols: list[np.ndarray] = []
...
for band in bands:
    ...
    if missing:
        raise RuntimeError(f"Missing V142 source columns for {band}: {missing}")
    ...
    slope_cols.append(r - b)
    slope_names.append(f"v142::{band}::attack_slope")
    slope_cols.append(d30 - r)
    slope_names.append(f"v142::{band}::early_decay_slope")
    slope_cols.append(d60 - d30)
    slope_names.append(f"v142::{band}::late_decay_slope")
    slope_cols.append(p - d60)
    slope_names.append(f"v142::{band}::tail_slope")

slope_x = np.column_stack(slope_cols)
if slope_x.shape[1] != 20:
    raise RuntimeError(f"Expected 20 V142 temporal-slope columns, got {slope_x.shape[1]}")
if not np.isfinite(slope_x).all():
    raise RuntimeError("Non-finite V142 temporal-slope feature values")
        ...
        inner_rows.append(
            {
                ...
                "v134Pass": base["pass"],
                "v142Pass": challenger["pass"],
                "v134Lift": base["lift"],
                "v142Lift": challenger["lift"],
                "liftDelta": round(challenger["lift"] - base["lift"], 6),
            }
        )

    gains = sum(int(item["v142Pass"] and not item["v134Pass"]) for item in inner_rows)
    losses = sum(int(item["v134Pass"] and not item["v142Pass"]) for item in inner_rows)
...
def training_only_decision(row: dict) -> dict:
    ...
    challenger_matrix = np.concatenate([base_matrix, slope_x], axis=1)
    ...
    return {
        ...
        "reason": "replicated frozen V142 training-only dominance" if selected else "replicated V142 dominance rule not met",
    }


print("=== V142 TEMPORAL SLOPE PROGRESSION TRAINING-ONLY DEVELOPMENT ===")
print("Frozen feature recipe loaded before scoring: True")
print("Temporal slope features:", slope_x.shape[1])
...
if candidate_before != candidate_after:
    raise RuntimeError("Protected 949-event candidate changed during V142 development")

out = {
    "experiment": "V142-temporal-slope-training-only-development",
    "baselineChampion": {"name": "V134", "passes": 311, "total": 320, "scorePercent": 97.1875},
    "frozenTemporalSlopeSpec": {
        "path": str(SPEC_PATH.relative_to(ROOT)),
        "sha256": sha256(SPEC_PATH),
        "featureCount": int(slope_x.shape[1]),
        "featureNames": slope_names,
    },
...
print()
print("=== V142 DEVELOPMENT COMPLETE ===")
print(f"Training-only temporal-slope selections: {len(selected)}/320")
