...
OUTPUT_PATH = PUBLIC / "gomyway-step10-agreement-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-step10-agreement-gate-v1-manifest.json"

FOLDS = 5
BLOCKS = temporal.BLOCKS
SHIFTED_WINDOWS = temporal.SHIFTED_WINDOWS
CURRENT_CHAMPION_F1 = 9.01
...
# The frozen 9.01 cross-signature profile showed step10 single-stem/weak-second
# additions at 0 true / 2 false. These are detector-side agreement gates only;
# the professional reference is used strictly for downstream validation.
RULES = [
    {"name": "step10_require_both8", "step": 10, "bothFloor": 8.0},
    {"name": "step10_require_both10", "step": 10, "bothFloor": 10.0},
    {"name": "step10_require_both13", "step": 10, "bothFloor": 13.0},
]
...
def agreement_prune(
    ...
    target_step = int(rule["step"])
    floor = float(rule["bothFloor"])
    for token, count in additions.items():
        _, step, _ = token
        if step != target_step:
            out[token] = count
            continue
        winner_score = float(winner_scores.get(token, 0.0))
        alt_score = float(alt_scores.get(token, 0.0))
        if winner_score >= floor and alt_score >= floor:
            out[token] = count
    return out
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.01 champion and step10 agreement candidates...", flush=True)
    ...
    for rule in RULES:
        filtered_additions = agreement_prune(champion_additions, winner_scores, alt_scores, rule)
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during step10 agreement benchmark.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.01-step10-agreement-precision-gate",
        ...
        "recommendedNextAction": "lock-step10-agreement-pruned-champion" if winner["acceptedOverChampion"] else "retain-9.01-and-profile-score-agreement-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY STEP10 AGREEMENT GATE V1 COMPLETE")
    print("Passed: True")
    ...
    print("Winner accepted over 9.01 champion:", winner["acceptedOverChampion"])
