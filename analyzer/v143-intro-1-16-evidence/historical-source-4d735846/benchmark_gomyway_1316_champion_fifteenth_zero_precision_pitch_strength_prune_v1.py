...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1316-champion-fifteenth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1316-champion-fifteenth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1574)
EXPECTED_F1 = 13.16
WINNER_NAME = "dual6_quarter_40_64"

FIFTEENTH_ZERO_BUCKETS = {(42, "20_plus")}
RULES = [
    {"name": "drop_fifteenth_zero_midi42_20_plus", "drop": [[42, "20_plus"]]},
    {
        "name": "drop_all_fifteenth_profiled_zero_precision_pitch_strength",
        "drop": sorted([list(x) for x in FIFTEENTH_ZERO_BUCKETS]),
    },
]
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.16 pruned recall champion...", flush=True)
    ...
    results = {}
    for rule in RULES:
        fifteenth_kept = prune_residual(fourteenth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, fifteenth_kept)
        audit = evaluator(candidate, champion, reference, champion_score)
        results[rule["name"]] = {"rule": rule, "keptProposedAdditions": int(sum(fifteenth_kept.values())), **audit}
        s = audit["fullScore"]
        print(
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.16 fifteenth pitch-strength prune benchmark.")
    ...
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.16-recall-champion-fifteenth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": (
            "close-broad-zero-precision-loop-and-profile-finer-cross-signatures"
            if validated else "retain-13.16-and-profile-finer-cross-signatures"
        ),
    }
    manifest = {
        "schemaVersion": 1,
    ...
    print("GOMYWAY 13.16 CHAMPION FIFTEENTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
