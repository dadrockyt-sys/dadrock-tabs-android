...
OUTPUT_PATH = PUBLIC / "gomyway-1542-attack-envelope-survivors-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1542-attack-envelope-survivors-precision-v1-manifest.json"
...
def main() -> None:
    ...
    profile_payload = v2.load_json(PROFILE_1499_PATH)
    ...
    seventh_pruned: Counter[tuple[int, int, int]] = Counter()
    survivor_rows: list[dict[str, Any]] = []
    for tok, count in champion_1499.items():
        row = row_by_token.get(tok)
        if row is None:
            continue
        if b1499.pred_a(row) or b1499.pred_b(row) or b1499.pred_c(row) or b1499.pred_d(row):
            seventh_pruned[tok] = count
        else:
            survivor_rows.append(row)

    champion = champion_1499 - seventh_pruned
    ...
    if actual != EXPECTED or abs(float(score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 15.42 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{score['pitchF1']}")
    ...
    for row in survivor_rows:
        ...
        details.append({**row, "survives1542Prune": True, "trueSurvivor": truth == "true"})
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 15.42 attack-envelope survivor profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-15.42-attack-envelope-survivors-precision",
        "champion1542Score": score,
        "validatedAttackSurvivorPruneRule": "attack_survivor_prune_union_a_b_c_d",
        "validatedAttackSurvivorPruneCount": int(sum(seventh_pruned.values())),
        ...
        "recommendedNextAction": "benchmark-only-repeatable-zero-precision-15.42-attack-envelope-survivor-signatures-with-prune-specific-heldout-cv-or-freeze-family-if-exhausted",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 15.42 ATTACK-ENVELOPE SURVIVORS PRECISION V1 COMPLETE")
    print("Passed: True")
    print("Champion pitch F1:", score["pitchF1"])
    print("Champion matched/missing/extra:", score["matched"], "/", score["missing"], "/", score["extra"])
    print("Validated attack survivor prune count:", sum(seventh_pruned.values()))
