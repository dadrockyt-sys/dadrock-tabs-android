...
import profile_gomyway_1272_pruned_recall_champion_residual_additions_precision_v1 as p1272
import benchmark_gomyway_1272_champion_ninth_zero_precision_pitch_strength_prune_v1 as prune1285

v2 = p1272.v2
v3 = p1272.v3
recall = p1272.recall
profile = p1272.profile

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1285-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1285-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (170, 697, 1608)
EXPECTED_F1 = 12.85
WINNER_NAME = "dual6_quarter_40_64"
NINTH_PRUNE = next(r for r in prune1285.RULES if r["name"] == "drop_all_ninth_profiled_zero_precision_pitch_strength")
...
def main() -> None:
    ...
    ninth_kept = prune1285.prune_residual(eighth_kept, winner_scores, alt_scores, NINTH_PRUNE)
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 12.85 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {actual}/{champion_score['pitchF1']}")
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 12.85 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-12.85-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "recommendedNextAction": "design-next-zero-or-near-zero-precision-prune-from-validated-12.85-residual-recall-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 12.85 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
