...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1229-champion-fifth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1229-champion-fifth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (166, 701, 1669)
EXPECTED_F1 = 12.29
...
RULES = [
    {"name": "drop_fifth_zero_low_strength", "drop": sorted([list(x) for x in FIFTH_ZERO_BUCKETS if x[1] in {"6_8", "8_10"}])},
    {"name": "drop_fifth_zero_mid_strength", "drop": sorted([list(x) for x in FIFTH_ZERO_BUCKETS if x[1] in {"10_13", "13_16"}])},
    {"name": "drop_all_fifth_profiled_zero_precision_pitch_strength", "drop": sorted([list(x) for x in FIFTH_ZERO_BUCKETS])},
]
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 12.29 pruned recall champion...", flush=True)
    ...
    champion = recall.precond.merge_with_cap(core, fourth_kept)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 12.29 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    results = {}
    for rule in RULES:
        fifth_kept = prune_residual(fourth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, fifth_kept)
        audit = prune1163.evaluate(candidate, champion, reference, champion_score)
        results[rule["name"]] = {"rule": rule, "keptProposedAdditions": int(sum(fifth_kept.values())), **audit}
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_12_29_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 12.29 fifth pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_12_29_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-12.29-recall-champion-fifth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-fifth-pruned-champion" if validated else "retain-12.29-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 12.29 CHAMPION FIFTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
