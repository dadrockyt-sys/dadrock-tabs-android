...
OUTPUT_PATH = PUBLIC / "gomyway-zero-precision-pruned-929-champion-residual-extras-profile-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-zero-precision-pruned-929-champion-residual-extras-profile-v1-manifest.json"
...
def main() -> None:
    ...
    print("Rebuilding validated 9.29 champion and profiling residual extras...", flush=True)
    ...
    additions_929 = gate929.residual_prune(additions_927, winner_scores, alt_scores, adaptive_base)

    prediction = precond.merge_with_cap(base_champion, additions_929)
    ...
    specialist_extras = Counter({t: c for t, c in (prediction - reference).items() if t in additions_929})
    specialist_matches = Counter({t: c for t, c in (prediction & reference).items() if t in additions_929})

    names = ["stepPitch", "stepAgreement", "scoreAgreement", "stepScore", "stepPitchScore", "stepPitchAgreement", "stepScoreAgreement", "stepPitchScoreAgreement", "stepPitchReason", "pitchScoreAgreement"]
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 9.29 residual profile.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.29-combined-pruned-champion-residual-profile",
        "championScore": full,
        ...
        "recommendedNextAction": "design-next-precision-prune-from-validated-9.29-residual-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY ZERO-PRECISION PRUNED 9.29 CHAMPION RESIDUAL EXTRAS PROFILE V1 COMPLETE")
    print("Passed: True")
