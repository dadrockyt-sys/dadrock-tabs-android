...
survivor = envelope.survivor

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
PROFILE_PATH = PUBLIC / "gomyway-2731-dual-stem-spectral-envelope-shape-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-2731-spectral-envelope-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2731-spectral-envelope-precision-prune-cv-v1-manifest.json"
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing spectral-envelope signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("27.31 spectral-envelope profile has no rows")
    ...
    if len(zero_rows) != EXPECTED_ZERO_SIGNATURES:
        raise RuntimeError(
            f"Expected exactly {EXPECTED_ZERO_SIGNATURES} spectral-envelope zero-precision signatures, got {len(zero_rows)}"
        )
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision spectral-envelope row: {row}")
    ...
    champion, _ = survivor.reconstruct_2731(
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 27.31 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"spectralenvelope2731_exact_{idx:02d}", make_exact_predicate(signature)))
    variants.append(("spectralenvelope2731_union_top2", union_pred(exact_signatures[:2])))
    variants.append(("spectralenvelope2731_union_top3", union_pred(exact_signatures[:3])))
    variants.append(("spectralenvelope2731_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2731": accepted_over_2731,
        }
        results[name] = result
        print(
            ...
            f"accepted={accepted_over_2731}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2731:
    ...
    if accepted:
        ...
        winner_name = "retain_27_31_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "truePruned": 0,
            ...
            "acceptedOver2731": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 27.31 spectral-envelope precision-prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-27.31-spectral-envelope-precision-prune-with-heldout-cv",
        "baseline2731Score": baseline,
    ...
    manifest = {
        ...
        "baseline2731PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 27.31 SPECTRAL ENVELOPE PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
