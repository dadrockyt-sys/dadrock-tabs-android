...
def capture_fresh_section3_reference_free(source_audio: bytes, suffix: str = ".m4a") -> dict[str, Any]:
    """Capture raw attacks + whole-onset spectra for measures 49-64.
    ...
    with tempfile.TemporaryDirectory(prefix="v143-fresh-section3-") as temp_dir:
        root = Path(temp_dir)
        source = root / f"uploaded{safe_suffix}"
        normalized = root / "normalized.wav"
        source.write_bytes(source_audio)
        if source.stat().st_size <= 0:
            raise RuntimeError("Fresh Section 3 source audio is empty")
        ...
        if not onset_groups:
            raise RuntimeError("No fresh Section 3 physical onset groups produced")
        ...
        for index, group in enumerate(onset_groups, start=1):
            ...
            if index % 250 == 0:
                print(f"fresh Section 3 onset spectra {index}/{len(onset_groups)}")
        ...
        return {
            "cacheVersion": 1,
            "scope": "fresh-section3-measures-49-64-reference-free",
            "section": {"name": "Fresh Section 3", "startMeasure": FIRST_MEASURE, "endMeasure": LAST_MEASURE},
            "timing": {
...
@app.local_entrypoint(name="capture_fresh_section3")
def capture_fresh_section3(audio_path: str = str(DEFAULT_AUDIO_PATH)) -> None:
    ...
    print("=== V143 FRESH VERSE 1 REFERENCE-FREE CAPTURE ===")
    print("Measures: 49-64")
    ...
    result = capture_fresh_section3_reference_free.remote(payload, source.suffix)

    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_PATH.write_text(json.dumps(result, indent=2) + "\n")

    print()
    print("=== V143 FRESH VERSE 1 CACHE CAPTURED ===")
    print("measureRange: 49..64")
