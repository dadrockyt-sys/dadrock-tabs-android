...
PUBLIC = ROOT / "public"
PROFILE_PATH = PUBLIC / "gomyway-2568-dual-stem-spectral-tonal-noise-contrast-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-2568-spectral-tonal-noise-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2568-spectral-tonal-noise-precision-prune-cv-v1-manifest.json"
EXPECTED = (183, 684, 375)
EXPECTED_F1 = 25.68
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing spectral tonal-noise signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if profile_payload.get("professionalReferenceUsedDuringDetection") is not False:
        raise RuntimeError("25.68 spectral tonal-noise profile is not reference-free during detection")
    rows = list(profile_payload.get("rows", []))
    if not rows:
        raise RuntimeError("25.68 spectral tonal-noise profile has no rows")
    ...
    if len(zero_rows) != EXPECTED_ZERO_SIGNATURES:
        raise RuntimeError(
            f"Expected exactly {EXPECTED_ZERO_SIGNATURES} spectral tonal-noise zero-precision signatures, got {len(zero_rows)}"
        )
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision spectral tonal-noise row: {row}")
    ...
    champion, period_pruned = spectral.reconstruct_2568(
        grid,
        winner_audio,
        winner_sr,
        alt_audio,
    ...
    if int(sum(period_pruned.values())) != spectral.EXPECTED_PERIOD_PRUNE_COUNT:
        raise RuntimeError(
            f"Expected frozen periodicity prune count {spectral.EXPECTED_PERIOD_PRUNE_COUNT}, got {sum(period_pruned.values())}"
        )
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 25.68 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"spectral2568_exact_{idx:02d}", make_exact_predicate(signature)))

    def union_pred(selected: list[str]) -> Callable[[dict[str, Any]], bool]:
        chosen = set(selected)
        return lambda row: bool(row_signatures(row) & chosen)

    variants.append(("spectral2568_union_top2", union_pred(exact_signatures[:2])))
    variants.append(("spectral2568_union_top3", union_pred(exact_signatures[:3])))
    variants.append(("spectral2568_union_top5", union_pred(exact_signatures[:5])))
    variants.append(("spectral2568_union_top8", union_pred(exact_signatures[:8])))
    variants.append(("spectral2568_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2568": accepted_over_2568,
        }
        results[name] = result
        print(
            ...
            f"accepted={accepted_over_2568}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2568:
    ...
    if accepted:
        ...
        winner_name = "retain_25_68_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "truePruned": 0,
            ...
            "acceptedOver2568": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 25.68 spectral tonal-noise precision prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-25.68-spectral-tonal-noise-precision-prune-with-heldout-cv",
        "baseline2568Score": baseline,
    ...
    manifest = {
        ...
        "baseline2568PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 25.68 SPECTRAL TONAL-NOISE PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
