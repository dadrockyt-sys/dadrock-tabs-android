...
OUTPUT_PATH = PUBLIC / "gomyway-3161-microtiming-contextual-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-3161-microtiming-contextual-cv-v1-manifest.json"
...
def structural_context(token: tuple[int, int, int]) -> set[str]:
    measure, step, pitch = token
    section = measure // 16
    measure_phase = measure % 4
    step_parity = step % 2
    step_quarter = step % 4
    register_band = "low" if pitch < 48 else ("mid" if pitch < 60 else "high")
    pitch_class = pitch % 12
    return {
        f"ctxSection::s{section}",
        f"ctxMeasurePhase::m{measure_phase}",
        f"ctxStepParity::p{step_parity}",
        f"ctxStepQuarter::q{step_quarter}",
        f"ctxRegister::{register_band}",
        f"ctxPitchClass::pc{pitch_class}",
        f"ctxMeasureStep::m{measure_phase}|q{step_quarter}",
        f"ctxSectionStep::s{section}|q{step_quarter}",
        f"ctxRegisterStep::{register_band}|q{step_quarter}",
    }
...
def contextual_signatures(
    token: tuple[int, int, int],
    micro_sigs: set[str],
    register_sigs: set[str],
) -> set[str]:
    contexts = structural_context(token)
    out: set[str] = set()

    # Microtiming conditioned on broad, reusable structural context.
    for ms in micro_sigs:
        for ctx in contexts:
            out.add(f"microCtx::{ms}||{ctx}")

    # Microtiming conditioned on existing local register-distribution signatures.
    # These are detection-side features already used in the validated 31.61 chain.
    for ms in micro_sigs:
        for rs in register_sigs:
            out.add(f"microRegister::{ms}||{rs}")
...
def main() -> None:
    ...
    for token, count in champion.items():
        ...
        ms = micro_signatures(token, grid, winner_audio, winner_sr, alt_audio, alt_sr)
        rs = set(register.signatures_for(register.local_features(token, maps)))
        cs = contextual_signatures(token, ms, rs)
        pocket.append({
            "token": token,
            "count": int(count),
            "truth": truth,
            "microSignatures": sorted(ms),
    ...
    # Full-data candidates need at least 3 false examples and zero true examples.
    selected = select_zero(pocket, min_false=3)
    ...
    for fold in range(FOLDS):
        ...
        # With only seven examples, require two supporting false examples in training.
        learned = select_zero(train, min_false=2)
        fold_true = 0
        fold_false = 0
        for row in test:
    ...
    section_stability_passed = true_pruned == 0 and false_pruned >= 3
    ...
    for shift_ms in SHIFT_MS:
        ...
        for row in pocket:
            ...
            ms = micro_signatures(token, grid, winner_audio, winner_sr, alt_audio, alt_sr, shift_ms=shift_ms)
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during contextual microtiming CV")

    output = {
        "schemaVersion": 1,
        "passed": accepted,
        "profileType": "31.61-contextual-microtiming-cv",
        "baselineScore": baseline,
        "candidateScore": candidate_score,
        "targetCrossFamilySignature": target_signature,
        "selectedContextualZeroPrecisionSignatures": sorted(selected),
    ...
    print("GOMYWAY 31.61 CONTEXTUAL MICROTIMING CV V1 COMPLETE")
    ...
    print("Contextual zero-precision signature count:", len(selected))
    ...
    print("Matched-note loss:", matched_note_loss)
