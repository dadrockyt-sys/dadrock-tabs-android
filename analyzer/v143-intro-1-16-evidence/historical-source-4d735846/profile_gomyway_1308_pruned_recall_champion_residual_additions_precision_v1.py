...
import benchmark_gomyway_1305_champion_eleventh_zero_precision_pitch_strength_prune_v1 as prune1308
...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1308-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1308-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1591)
EXPECTED_F1 = 13.08
WINNER_NAME = "dual6_quarter_40_64"
ELEVENTH_PRUNE = next(r for r in prune1308.RULES if r["name"] == "drop_eleventh_zero_midi55_mid_strengths")
...
def main() -> None:
    ...
    eleventh_kept = prune1308.prune_residual(tenth_kept, winner_scores, alt_scores, ELEVENTH_PRUNE)
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 13.08 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {actual}/{champion_score['pitchF1']}")
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.08 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.08-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "recommendedNextAction": "design-next-zero-or-near-zero-precision-prune-from-validated-13.08-residual-recall-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.08 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
