...
ANCHOR36_STATE_PATH = PUBLIC / "gomyway-rhythm-whole-song-with-anchor-36-v1.json"
REGISTRATION51_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-51-read-only-registration-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-rhythm-whole-song-with-anchors-36-51-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-rhythm-whole-song-with-anchors-36-51-v1-manifest.json"
...
def compare_rhythm(a: dict[str, Any], b: dict[str, Any]) -> dict[str, float]:
    ...
    if shared:
        multiplicity_scores: list[float] = []
        for step in shared:
            ma = int(a["multiplicityByStep"].get(step, 0))
            mb = int(b["multiplicityByStep"].get(step, 0))
            multiplicity_scores.append(
                1.0 - min(1.0, abs(ma - mb) / max(1, ma, mb))
            )
...
def main() -> None:
    ...
    anchor36_state = load(ANCHOR36_STATE_PATH)
    registration51 = load(REGISTRATION51_PATH)
    ...
    if anchor36_state.get("passed") is not True:
        raise RuntimeError("Anchor 36 whole-song state V1 is not green.")
    if anchor36_state.get("anchor36WinningMeasures") != EXPECTED_ANCHOR36_GAINS:
        raise RuntimeError("Anchor 36 coverage gains changed unexpectedly.")
    if anchor36_state.get("protectedSourceHashUnchanged") is not True:
        raise RuntimeError("Anchor 36 whole-song state did not preserve source hash.")

    if registration51.get("passed") is not True:
        raise RuntimeError("Anchor 51 registration V1 is not green.")
    if registration51.get("anchor51RegisteredReadOnly") is not True:
        raise RuntimeError("Anchor 51 is not registered read-only.")
    if registration51.get("registeredRhythmAnchorMeasures") != EXPECTED_REGISTERED_ANCHORS:
        raise RuntimeError("Registered rhythm-anchor set changed unexpectedly.")
    if registration51.get("anchor36CoverageGainsPreserved") != EXPECTED_ANCHOR36_GAINS:
        raise RuntimeError("Anchor 36 gains were not preserved by anchor 51 registration.")

    registration = registration51.get("anchorRegistration")
    if not isinstance(registration, dict):
        raise RuntimeError("Anchor 51 nested registration is missing.")
    if registration.get("measureNumber") != NEW_ANCHOR:
        raise RuntimeError("Anchor 51 registration target changed unexpectedly.")
    if registration.get("registrationScope") != "rhythm-structure-only":
        raise RuntimeError("Anchor 51 registration scope changed unexpectedly.")
    if registration.get("occupiedSteps") != EXPECTED_OCCUPIED_STEPS:
        raise RuntimeError("Anchor 51 occupied-step pattern changed unexpectedly.")
    if registration.get("eligibleAsSimilarityAnchor") is not True:
        raise RuntimeError("Anchor 51 is not eligible as a similarity anchor.")
    learned_domains = registration.get("learnedDomains")
    if not isinstance(learned_domains, dict):
        raise RuntimeError("Anchor 51 learned-domain registration is missing.")
    if learned_domains.get("rhythmStructure") is not True:
        raise RuntimeError("Anchor 51 rhythm-structure registration is missing.")
    if any(learned_domains.get(key) is not False for key in ("chordShape", "timing", "technique")):
        raise RuntimeError("Anchor 51 unexpectedly contains non-rhythm knowledge.")
    if registration51.get("automaticApplyAllowed") is not False:
        raise RuntimeError("Anchor 51 registration unexpectedly allows automatic application.")
    if registration51.get("protectedSourceHashUnchanged") is not True:
        raise RuntimeError("Anchor 51 registration did not preserve source hash.")

    profiles = build_profiles(events)
    if sorted(profiles[NEW_ANCHOR]["occupiedSteps"]) != EXPECTED_OCCUPIED_STEPS:
        raise RuntimeError("Protected source profile for measure 51 no longer matches registration.")

    prior_rows = anchor36_state.get("rows")
    if not isinstance(prior_rows, list):
        raise RuntimeError("Anchor 36 whole-song rows are missing.")
    prior_by_measure = {
    ...
    for measure in range(TARGET_START, LAST_MEASURE + 1):
        prior = prior_by_measure.get(measure)
        if prior is None:
            raise RuntimeError(f"Missing anchor-36 state row for measure {measure}.")

        old_anchor = int(prior["bestAnchorAfterAnchor36"])
        old_score = float(prior["bestStructuralSimilarityAfterAnchor36"])
        anchor51 = compare_rhythm(profiles[measure], profiles[NEW_ANCHOR])
        anchor51_score = float(anchor51["rhythmStructuralSimilarityScore"])

        wins = anchor51_score > old_score
        best_anchor = NEW_ANCHOR if wins else old_anchor
        best_score = anchor51_score if wins else old_score
        delta = round(best_score - old_score, 6)

        if wins:
            won_by_anchor51.append(measure)
        if delta > 0:
        ...
        rows.append({
            ...
            "priorBestAnchorMeasure": old_anchor,
            "priorBestStructuralSimilarityScore": old_score,
            "anchor51RhythmComparison": anchor51,
            "anchor51Wins": wins,
            "bestAnchorAfterAnchors3651": best_anchor,
            "bestStructuralSimilarityAfterAnchors3651": round(best_score, 6),
            "incrementalStructuralImprovementFrom51": delta,
            "classificationClaimed": False,
            "automaticApplyAllowed": False,
        })

    new_scores = [float(row["bestStructuralSimilarityAfterAnchors3651"]) for row in rows]
    counts = {
    ...
    recommended = (
        "review-gomyway-rhythm-anchor-51-whole-song-impact-v1"
        if passed
        else "diagnose-gomyway-rhythm-whole-song-with-anchors-36-51-v1"
    )

    output = {
        "schemaVersion": 1,
        "diagnosticType": "read-only-incremental-whole-song-rhythm-similarity-with-anchors-36-51",
        "passed": passed,
        ...
        "newAnchorMeasure": NEW_ANCHOR,
        "newAnchorScope": "rhythm-structure-only",
        "anchor36CoverageGainsPreserved": EXPECTED_ANCHOR36_GAINS,
        "anchor51WinCount": len(won_by_anchor51),
        "anchor51WinningMeasures": won_by_anchor51,
        "incrementallyImprovedMeasureCount": len(improved_measures),
        "incrementallyImprovedMeasures": improved_measures,
        "bestStructuralScoreAtLeast090Count": counts["atLeast090"],
        "bestStructuralScoreAtLeast080Count": counts["atLeast080"],
        "bestStructuralScoreAtLeast070Count": counts["atLeast070"],
        "bestStructuralScoreAtLeast060Count": counts["atLeast060"],
        ...
        "readyForAnchor51ImpactReview": passed,
    ...
    print("GOMYWAY RHYTHM WHOLE SONG WITH ANCHORS 36+51 V1 COMPLETE")
    print("Passed:", passed)
    ...
    print("Anchor 51 wins:", len(won_by_anchor51), won_by_anchor51)
    print("Incrementally improved measures:", len(improved_measures), improved_measures)
    ...
    for row in rows:
        if row["anchor51Wins"]:
            comparison = row["anchor51RhythmComparison"]
            print(
                f"measure={row['measureNumber']} oldAnchor={row['priorBestAnchorMeasure']} "
                f"old={row['priorBestStructuralSimilarityScore']} anchor51={comparison['rhythmStructuralSimilarityScore']} "
                f"occupied={comparison['occupiedStepJaccard']} multiplicity={comparison['sharedStepMultiplicitySimilarity']} "
                f"density={comparison['rowDensitySimilarity']} delta={row['incrementalStructuralImprovementFrom51']}"
            )
    ...
    print("Ready for anchor 51 impact review:", passed)
