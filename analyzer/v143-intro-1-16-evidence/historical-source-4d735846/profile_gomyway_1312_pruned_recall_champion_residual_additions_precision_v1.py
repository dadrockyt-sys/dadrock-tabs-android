...
import profile_gomyway_1308_pruned_recall_champion_residual_additions_precision_v1 as p1308
import benchmark_gomyway_1308_champion_twelfth_zero_precision_pitch_strength_prune_v1 as prune1312

v2 = p1308.v2
v3 = p1308.v3
recall = p1308.recall
profile = p1308.profile

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1312-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1312-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1582)
EXPECTED_F1 = 13.12
WINNER_NAME = "dual6_quarter_40_64"
TWELFTH_PRUNE = next(
    r for r in prune1312.RULES
    if r["name"] == "drop_all_twelfth_profiled_zero_precision_pitch_strength"
)
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.12 pruned recall champion...", flush=True)
    ...
    twelfth_kept = prune1312.prune_residual(
        eleventh_kept, winner_scores, alt_scores, TWELFTH_PRUNE
    )

    champion = recall.precond.merge_with_cap(core, twelfth_kept)
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected 13.12 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, "
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.12 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.12-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "recommendedNextAction": "design-next-zero-or-near-zero-precision-prune-from-validated-13.12-residual-recall-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.12 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
