...
OUTPUT_PATH = PUBLIC / "gomyway-3676-onset-slot-micro-temporal-nested-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-onset-slot-micro-temporal-nested-cv-v1-manifest.json"
...
def choose_feature_and_tail(train: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for feature in FEATURES:
        for q in TAIL_QUANTILES:
            ...
            if done == 1 or done % 17 == 0 or done == total:
                print(f"    heartbeat micro-temporal search {done}/{total}", flush=True)
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("Micro-temporal candidateSlots missing; run the micro-temporal stability profiler first")
    if tuple(payload.get("frozenChampionMatchedMissingExtra") or []) != EXPECTED:
        raise RuntimeError("Micro-temporal profile not anchored to frozen 36.76 champion")

    measures = [int(r["measure"]) for r in rows]
    lo, hi = min(measures), max(measures)

    print("Starting strict nested micro-temporal onset-shape CV V1", flush=True)
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during micro-temporal nested CV")

    output = {
        "schemaVersion": 1,
        "profileType": "36.76-onset-slot-micro-temporal-nested-cv-exploratory",
        ...
        "microTemporalOnsetShapeArchitectureGeneralizes": generalizes,
    ...
    print("GOMYWAY 36.76 ONSET SLOT MICRO TEMPORAL NESTED CV V1 COMPLETE")
    ...
    print("Micro-temporal onset-shape architecture generalizes:", generalizes)
