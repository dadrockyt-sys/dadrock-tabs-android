...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1253-champion-seventh-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1253-champion-seventh-zero-precision-pitch-strength-prune-v1-manifest.json"
...
RULES = [
    {
        "name": "drop_seventh_zero_low_strength",
        "drop": sorted([list(x) for x in SEVENTH_ZERO_BUCKETS if x[1] in {"6_8", "8_10"}]),
    },
    {
        "name": "drop_seventh_zero_mid_high_strength",
        "drop": sorted([list(x) for x in SEVENTH_ZERO_BUCKETS if x[1] in {"10_13", "20_plus"}]),
    },
    {
        "name": "drop_all_seventh_profiled_zero_precision_pitch_strength",
        "drop": sorted([list(x) for x in SEVENTH_ZERO_BUCKETS]),
    },
]
...
def main() -> None:
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 12.53 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    results = {}
    for rule in RULES:
        seventh_kept = prune_residual(sixth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, seventh_kept)
        audit = prune1163.evaluate(candidate, champion, reference, champion_score)
        results[rule["name"]] = {
            "rule": rule,
            "keptProposedAdditions": int(sum(seventh_kept.values())),
            **audit,
        }
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_12_53_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 12.53 seventh pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_12_53_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-12.53-recall-champion-seventh-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-seventh-pruned-champion" if validated else "retain-12.53-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 12.53 CHAMPION SEVENTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
