...
def capture_fresh_section2_reference_free(source_audio: bytes, suffix: str = ".m4a") -> dict[str, Any]:
    """Capture raw attacks + whole-onset spectra for measures 33-48.
    ...
    with tempfile.TemporaryDirectory(prefix="v143-fresh-section2-") as temp_dir:
        root = Path(temp_dir)
        source = root / f"uploaded{safe_suffix}"
        normalized = root / "normalized.wav"
        source.write_bytes(source_audio)
        if source.stat().st_size <= 0:
            raise RuntimeError("Fresh Section 2 source audio is empty")
        ...
        if not onset_groups:
            raise RuntimeError("No fresh Section 2 physical onset groups produced")
        ...
        for index, group in enumerate(onset_groups, start=1):
            ...
            if index % 250 == 0:
                print(f"fresh Section 2 onset spectra {index}/{len(onset_groups)}")
        ...
        return {
            "cacheVersion": 1,
            "scope": "fresh-section2-measures-33-48-reference-free",
            "section": {"name": "Fresh Section 2", "startMeasure": FIRST_MEASURE, "endMeasure": LAST_MEASURE},
            "timing": {
...
@app.local_entrypoint(name="capture_fresh_section2")
def capture_fresh_section2(audio_path: str = str(DEFAULT_AUDIO_PATH)) -> None:
    ...
    result = capture_fresh_section2_reference_free.remote(payload, source.suffix)
