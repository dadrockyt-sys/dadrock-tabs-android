...
import profile_gomyway_1315_pruned_recall_champion_residual_additions_precision_v1 as p1315

v2 = p1315.v2
v3 = p1315.v3
recall = p1315.recall
profile = p1315.profile

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1315-champion-fourteenth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1315-champion-fourteenth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1576)
EXPECTED_F1 = 13.15
WINNER_NAME = "dual6_quarter_40_64"

FOURTEENTH_ZERO_BUCKETS = {(40, "10_13")}
RULES = [
    {"name": "drop_fourteenth_zero_midi40_10_13", "drop": [[40, "10_13"]]},
    {
        "name": "drop_all_fourteenth_profiled_zero_precision_pitch_strength",
        "drop": sorted([list(x) for x in FOURTEENTH_ZERO_BUCKETS]),
    },
]
...
def main() -> None:
    ...
    core_score = p1315.grade(core, reference)
    ...
    thirteenth_kept = p1315.prune1315.prune_residual(
        twelfth_kept, winner_scores, alt_scores, p1315.THIRTEENTH_PRUNE
    )

    champion = recall.precond.merge_with_cap(core, thirteenth_kept)
    champion_score = p1315.grade(champion, reference)
    champion_actual = (
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected 13.15 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got "
    ...
    results = {}
    for rule in RULES:
        fourteenth_kept = prune_residual(thirteenth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, fourteenth_kept)
        audit = evaluator(candidate, champion, reference, champion_score)
        results[rule["name"]] = {
            "rule": rule,
            "keptProposedAdditions": int(sum(fourteenth_kept.values())),
            **audit,
        }
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_13_15_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.15 fourteenth pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_13_15_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.15-recall-champion-fourteenth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": (
            "profile-residual-recall-additions-from-validated-fourteenth-pruned-champion"
            if validated
            else "retain-13.15-and-profile-finer-cross-signatures"
        ),
    }
    manifest = {
        "schemaVersion": 1,
    ...
    print("GOMYWAY 13.15 CHAMPION FOURTEENTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
