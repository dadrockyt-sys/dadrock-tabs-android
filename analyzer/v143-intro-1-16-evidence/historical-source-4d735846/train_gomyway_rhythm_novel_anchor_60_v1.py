...
IMPACT_REVIEW_PATH = PUBLIC / "gomyway-rhythm-anchor-51-whole-song-impact-review-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-60-training-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-60-training-v1-manifest.json"

EXPECTED_SOURCE_EVENT_COUNT = 949
TARGET_MEASURE = 60
EXPECTED_REGISTERED_RHYTHM_ANCHORS = [36, 51]
EXPECTED_ANCHOR36_GAINS = [66, 99]
EXPECTED_ANCHOR51_GAINS = [52, 113]
QUEUED_AFTER = [73, 82, 91, 99, 101, 113]

TECHNIQUE_KEYS = (
    "technique", "techniques", "articulation", "articulations", "bend",
...
def compact_event(event: dict[str, Any], source_index: int) -> dict[str, Any]:
    ...
    for key in (
        "measureNumber", "measure", "quantizedStep", "step", "stringIndex",
        "string", "fret", "midiPitch", "pitch", "startTime", "start_time",
        "start", "duration", "durationSeconds",
    ):
        if key in event:
...
def main() -> None:
    ...
    impact_review = load(IMPACT_REVIEW_PATH)
    ...
    if impact_review.get("passed") is not True:
        raise RuntimeError("Anchor 51 whole-song impact review V1 is not green.")
    if impact_review.get("readyForNovelAnchor60Training") is not True:
        raise RuntimeError("Impact review is not ready for novel anchor 60 training.")
    if impact_review.get("nextChronologicalTrainingAnchor") != TARGET_MEASURE:
        raise RuntimeError("Novel training target changed unexpectedly.")
    if impact_review.get("anchor51ImpactAcceptedReadOnly") is not True:
        raise RuntimeError("Anchor 51 impact is no longer accepted read-only.")
    if impact_review.get("registeredRhythmAnchorMeasures") != EXPECTED_REGISTERED_RHYTHM_ANCHORS:
        raise RuntimeError("Registered rhythm-anchor set changed unexpectedly.")
    if impact_review.get("anchor36CoverageGainsPreserved") != EXPECTED_ANCHOR36_GAINS:
        raise RuntimeError("Anchor 36 coverage gains changed unexpectedly.")
    if impact_review.get("anchor51WinningMeasures") != EXPECTED_ANCHOR51_GAINS:
        raise RuntimeError("Anchor 51 coverage gains changed unexpectedly.")
    if impact_review.get("broadOvergeneralizationDetected") is not False:
        raise RuntimeError("Anchor 51 review now reports broad overgeneralization.")
    if impact_review.get("thresholdRelaxationAllowed") is not False:
        raise RuntimeError("Impact review unexpectedly allows threshold relaxation.")
    if impact_review.get("automaticApplyAllowed") is not False:
        raise RuntimeError("Impact review unexpectedly allows automatic application.")
    if impact_review.get("protectedSourceHashUnchanged") is not True:
        raise RuntimeError("Impact review did not preserve protected source hash.")
    ...
    timing_strict_unique = len(set(timing_values)) == len(timing_values) if timing_complete else False
    ...
    for index, event in measure_events:
        ...
        if found:
            explicit_technique_events.append({"sourceEventIndex": index, "evidence": found})
    ...
    recommended = (
        "review-gomyway-rhythm-novel-anchor-60-training-v1"
        if passed
        else "diagnose-gomyway-rhythm-novel-anchor-60-training-v1"
    )
    ...
    output = {
        ...
        "compactSourceEvents": [compact_event(event, index) for index, event in measure_events],
        "rhythmEvidence": {
        ...
        "registeredRhythmAnchorMeasuresPreserved": EXPECTED_REGISTERED_RHYTHM_ANCHORS,
        "anchor36CoverageGainsPreserved": EXPECTED_ANCHOR36_GAINS,
        "anchor51CoverageGainsPreserved": EXPECTED_ANCHOR51_GAINS,
        ...
        "readyForNovelAnchor60TrainingReview": passed,
    ...
    print("GOMYWAY RHYTHM NOVEL ANCHOR 60 TRAINING V1 COMPLETE")
    print("Passed:", passed)
    ...
    print("Registered rhythm anchors preserved:", EXPECTED_REGISTERED_RHYTHM_ANCHORS)
    print("Anchor 36 coverage gains preserved:", EXPECTED_ANCHOR36_GAINS)
    print("Anchor 51 coverage gains preserved:", EXPECTED_ANCHOR51_GAINS)
    ...
    print("Ready for novel anchor 60 training review:", passed)
