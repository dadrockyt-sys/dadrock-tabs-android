...
OUTPUT_PATH = PUBLIC / "gomyway-3676-cross-experiment-hard-slot-anatomy-v2.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-cross-experiment-hard-slot-anatomy-v2-manifest.json"
...
def standardized_difference(a: list[dict[str, Any]], b: list[dict[str, Any]], name: str) -> float:
    ...
    if not math.isfinite(sd) or sd < 1e-9:
        return 0.0
    return (
        float(np.mean([feature_value(r, name) for r in a]))
        - float(np.mean([feature_value(r, name) for r in b]))
    ) / sd
...
def quantile_from_fold(fold_result: dict[str, Any]) -> float | None:
    chosen = fold_result.get("chosen") or {}
    for value in (
        chosen.get("quantile"),
        fold_result.get("chosenQuantile"),
        fold_result.get("quantile"),
    ):
...
def main() -> None:
    ...
    reconstruction_stats: Counter[str] = Counter()

    print(f"Cross-experiment anatomy V2: {len(experiments)} completed experiments", flush=True)
    for exp_name, payload in experiments:
        ...
        for scheme, fold_rows in scheme_rows(payload):
            for fold_result in fold_rows:
                ...
                raw_threshold = fold_result.get("threshold")
                if raw_threshold is not None and math.isfinite(float(raw_threshold)):
                    threshold = float(raw_threshold)
                    reconstruction_stats["savedThreshold"] += 1
                else:
                    q = quantile_from_fold(fold_result)
                    if q is None or not model.get("features"):
                        threshold = float("inf")
                        reconstruction_stats["unreconstructableThreshold"] += 1
                    else:
                        threshold = threshold_for(train, model, q)
                        reconstruction_stats["reconstructedThreshold"] += 1
    ...
    print("Threshold reconstruction:", dict(reconstruction_stats), flush=True)
    print(
        f"Groups: hardTrue={len(hard_true)} recoverableTrue={len(recoverable_true)} "
        f"persistentFalse={len(persistent_false)}",
        flush=True,
    )
    ...
    rhythm = {
        ...
        "recoverableTrueStepMod8": dict(Counter(int(r.get("step", r.get("gridStep", 0))) % 8 for r in recoverable_true)),
        "persistentFalseStepMod4": dict(Counter(int(r.get("step", r.get("gridStep", 0))) % 4 for r in persistent_false)),
        "persistentFalseStepMod8": dict(Counter(int(r.get("step", r.get("gridStep", 0))) % 8 for r in persistent_false)),
    }
    ...
    output = {
        "schemaVersion": 2,
        "profileType": "36.76-cross-experiment-hard-slot-anatomy-diagnostic-fixed-threshold-reconstruction",
        "baselinePitchF1": EXPECTED_F1,
        "baselineMatchedMissingExtra": list(EXPECTED),
        "experimentsAnalyzed": [name for name, _ in experiments],
        "thresholdReconstruction": dict(reconstruction_stats),
    ...
    print("GOMYWAY 36.76 CROSS-EXPERIMENT HARD-SLOT ANATOMY V2 COMPLETE")
