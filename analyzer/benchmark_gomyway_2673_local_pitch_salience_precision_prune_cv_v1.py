...
OUTPUT_PATH = PUBLIC / "gomyway-2673-local-pitch-salience-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2673-local-pitch-salience-precision-prune-cv-v1-manifest.json"
EXPECTED = (183, 684, 319)
EXPECTED_F1 = 26.73
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing local pitch salience signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if len(zero_rows) != EXPECTED_ZERO_SIGNATURES:
        raise RuntimeError(
            f"Expected exactly {EXPECTED_ZERO_SIGNATURES} local pitch salience zero-precision signatures, got {len(zero_rows)}"
        )
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision local pitch salience row: {row}")
    ...
    champion, _ = salience.transient.reconstruct_2673(
        grid,
        winner_audio,
        winner_sr,
        alt_audio,
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 26.73 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"localsalience2673_exact_{idx:02d}", make_exact_predicate(signature)))

    def union_pred(selected: list[str]) -> Callable[[dict[str, Any]], bool]:
        chosen = set(selected)
        return lambda row: bool(row_signatures(row) & chosen)

    variants.append(("localsalience2673_union_top2", union_pred(exact_signatures[:2])))
    variants.append(("localsalience2673_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2673": accepted_over_2673,
        }
        results[name] = result
        print(
            ...
            f"accepted={accepted_over_2673}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2673:
    ...
    if accepted:
        ...
        winner_name = "retain_26_73_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "truePruned": 0,
            ...
            "acceptedOver2673": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 26.73 local pitch salience precision prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-26.73-local-pitch-salience-precision-prune-with-heldout-cv",
        "baseline2673Score": baseline,
    ...
    manifest = {
        ...
        "baseline2673PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 26.73 LOCAL PITCH SALIENCE PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
