...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1163-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1163-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (161, 706, 1740)
EXPECTED_F1 = 11.63
WINNER_NAME = "dual6_quarter_40_64"
WINNER_PRUNE = next(r for r in prune1163.RULES if r["name"] == "drop_all_profiled_zero_precision_pitch_strength")
...
def main() -> None:
    ...
    if (grade(core, reference)["matched"], grade(core, reference)["missing"], grade(core, reference)["extra"]) != EXPECTED_CORE:
        raise RuntimeError("Frozen 9.29 core mismatch")
    ...
    kept = prune1163.prune_proposed(proposed, winner_scores, alt_scores, WINNER_PRUNE)
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 11.63 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {actual}/{champion_score['pitchF1']}")
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 11.63 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-11.63-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "professionalReferenceRole": "downstream-grading-training-label-and-residual-addition-profiling-only",
        ...
        "recommendedNextAction": "design-next-zero-or-near-zero-precision-prune-from-validated-11.63-residual-recall-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 11.63 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
