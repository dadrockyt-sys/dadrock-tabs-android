...
import benchmark_gomyway_1858_harmonic_shape_precision_prune_cv_v1 as b1858
...
OUTPUT_PATH = PUBLIC / "gomyway-2065-harmonic-shape-survivors-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2065-harmonic-shape-survivors-precision-v1-manifest.json"
...
def main() -> None:
    ...
    if not zero_rows:
        raise RuntimeError("18.58 harmonic-shape profile has no zero-precision signatures")
    ...
    for tok, count in champion_1858.items():
        ...
        if row is None:
            continue
        if b1858.row_signatures(row) & zero_signatures:
            pruned[tok] = count
        else:
            survivor_rows.append(row)

    champion = champion_1858 - pruned
    ...
    for row in survivor_rows:
        ...
        for signature in b1858.row_signatures(row):
            groups[signature][truth] += count
        details.append({**row, "survives2065Prune": True, "trueSurvivor": truth == "true"})
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 20.65 harmonic-shape survivor profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-20.65-harmonic-shape-survivors-precision",
        "champion2065Score": score,
        "validatedHarmonicShapePruneRule": "shape1858_union_all_zero_precision",
        "validatedHarmonicShapeSignatureCount": len(zero_signatures),
        "validatedHarmonicShapePruneCount": int(sum(pruned.values())),
        ...
        "recommendedNextAction": "benchmark-only-repeatable-zero-precision-20.65-harmonic-shape-survivor-signatures-with-prune-specific-heldout-cv-or-freeze-family-if-exhausted",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 20.65 HARMONIC SHAPE SURVIVORS PRECISION V1 COMPLETE")
    print("Passed: True")
    print("Champion pitch F1:", score["pitchF1"])
    print("Champion matched/missing/extra:", score["matched"], "/", score["missing"], "/", score["extra"])
    print("Validated harmonic-shape signature count:", len(zero_signatures))
    print("Validated harmonic-shape prune count:", sum(pruned.values()))
    print("Harmonic-shape survivor rows:", len(survivor_rows))
    print("Generalizable zero-precision harmonic-shape survivor signatures (5+ false, 0 true):")
    for row in zero_precision[:50]:
        print(f"  {row['signature']}: true=0 false={row['false']} precision=0.0%")
    print("Top supported true harmonic-shape survivor signatures (5+ true):")
