...
OUTPUT_PATH = PUBLIC / "gomyway-spectral-top1-adaptive-local-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-spectral-top1-adaptive-local-gate-v1-manifest.json"
...
# Detector-side adaptive gates only. These thresholds are fixed before scoring.
RULES = [
    {"name": "local3_med12", "radius": 1, "median": 12.0},
    {"name": "local3_med13", "radius": 1, "median": 13.0},
    {"name": "local5_med12", "radius": 2, "median": 12.0},
    {"name": "local5_med13", "radius": 2, "median": 13.0},
    {"name": "local5_med14", "radius": 2, "median": 14.0},
]
...
def adaptive_additions(
    base_additions: Counter[tuple[int, int, int]],
    ...
    token_score: dict[tuple[int, int, int], float] = {}
    for token in base_additions:
        score = max(float(winner_scores.get(token, 0.0)), float(alt_scores.get(token, 0.0)))
        token_score[token] = score
        by_measure[token[0]].append(score)

    local_median: dict[int, float] = {}
    for measure in range(17, 114):
        scores: list[float] = []
        for neighbor_measure in range(measure - radius, measure + radius + 1):
            scores.extend(by_measure.get(neighbor_measure, []))
        local_median[measure] = statistics.median(scores) if scores else 0.0

    return Counter({
        token: 1
        for token in base_additions
        if local_median.get(token[0], 0.0) >= median_threshold
    })
...
def main() -> None:
    ...
    for rule in RULES:
        additions = adaptive_additions(top1, winner_scores, alt_scores, int(rule["radius"]), float(rule["median"]))
        ...
        median_fold = sorted(fold_deltas)[FOLDS // 2]
        ...
        mean_block = round(sum(block_deltas) / len(BLOCKS), 2)
        median_block = statistics.median(block_deltas)
        section_passed = pb >= 5 and pmb >= 5 and cat == 0 and mean_block > 0 and median_block > 0
        ...
        results[str(rule["name"])] = {
            ...
            "blocks": block_rows,
            "positiveF1Blocks": pb,
            "positiveMatchedBlocks": pmb,
            "catastrophicRegressionBlocks": cat,
            "meanBlockDeltaPoints": mean_block,
            "medianBlockDeltaPoints": round(float(median_block), 2),
        ...
        print(
            ...
            f"cv={cv_passed} folds={pf}/5 blocks={pb}/6 matchedBlocks={pmb}/6 cat={cat} section={section_passed} accepted={accepted}",
            flush=True,
        )
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during adaptive local gate benchmark.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "spectral-top1-adaptive-local-detector-side-gate",
        ...
        "recommendedNextAction": "lock-adaptive-spectral-champion" if winner["acceptedOverChampion"] else "retain-6.99-and-stop-global-spectral-admission",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY SPECTRAL TOP1 ADAPTIVE LOCAL GATE V1 COMPLETE")
    print("Passed: True")
