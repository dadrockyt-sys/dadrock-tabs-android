...
prune1308 = p1308.prune1308

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1308-champion-twelfth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1308-champion-twelfth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1591)
EXPECTED_F1 = 13.08
WINNER_NAME = "dual6_quarter_40_64"

TWELFTH_ZERO_BUCKETS = {
    (57, "13_16"),
    (56, "6_8"),
    (56, "8_10"),
    (62, "13_16"),
...
RULES = [
    {"name": "drop_twelfth_zero_midi57_13_16", "drop": [[57, "13_16"]]},
    {"name": "drop_twelfth_zero_midi56_strengths", "drop": [[56, "6_8"], [56, "8_10"]]},
    {"name": "drop_twelfth_zero_midi62_13_16", "drop": [[62, "13_16"]]},
    {"name": "drop_all_twelfth_profiled_zero_precision_pitch_strength", "drop": sorted([list(x) for x in TWELFTH_ZERO_BUCKETS])},
]
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.08 pruned recall champion...", flush=True)
    ...
    core_score = p1308.grade(core, reference)
    ...
    eleventh_kept = prune1308.prune_residual(tenth_kept, winner_scores, alt_scores, p1308.ELEVENTH_PRUNE)

    champion = recall.precond.merge_with_cap(core, eleventh_kept)
    champion_score = p1308.grade(champion, reference)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 13.08 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    evaluator = p1272.prune1163.evaluate
    results = {}
    for rule in RULES:
        twelfth_kept = prune_residual(eleventh_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, twelfth_kept)
        audit = evaluator(candidate, champion, reference, champion_score)
        results[rule["name"]] = {"rule": rule, "keptProposedAdditions": int(sum(twelfth_kept.values())), **audit}
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_13_08_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.08 twelfth pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_13_08_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.08-recall-champion-twelfth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-twelfth-pruned-champion" if validated else "retain-13.08-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.08 CHAMPION TWELFTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
