...
const OVERLAY_PATH = path.join(ROOT, 'public', 'gomyway-v8-supervised-intro-overlay-v2.json');
const PDF_PATH = path.join(ROOT, 'public', 'gomyway-v8-supervised-intro-proof-v2.pdf');
const MANIFEST_PATH = path.join(ROOT, 'public', 'gomyway-v8-supervised-intro-proof-v2-manifest.json');
...
const EXTRA_DOUBLE_STOP_MEASURES = [2, 4];
const EXTRA_DOUBLE_STOP_STEP = 15;
...
function buildSupervisedEvents(pack) {
  ...
  for (const measureNumber of EXTRA_DOUBLE_STOP_MEASURES) {
    const ending = events.find((event) => (
      event.measureNumber === measureNumber
      && event.quantizedStep === 14
      && event.notes.length > 1
    ));
    if (!ending) {
      throw new Error(`Missing ending double-stop source at measure ${measureNumber}, step 14.`);
    }
    events.push({
      ...JSON.parse(JSON.stringify(ending)),
      quantizedStep: EXTRA_DOUBLE_STOP_STEP,
      source: 'professional-label-supervised-extra-ending-double-stop',
      supervisedTraining: {
        ...ending.supervisedTraining,
        schemaVersion: 2,
        addedFromVisualBenchmark: true,
        repeatedEndingDoubleStop: true,
        productionEligible: false,
      },
    });
  }
...
async function main() {
  ...
  for (const measure of MEASURES) {
    ...
    for (const step of EXPECTED_STEPS) {
      if (!steps.has(step)) missingSlots.push([measure, step]);
    }
  }
  for (const measure of EXTRA_DOUBLE_STOP_MEASURES) {
    const extra = (map.get(measure) || []).find((event) => (
      event.quantizedStep === EXTRA_DOUBLE_STOP_STEP && event.notes.length > 1
    ));
    if (!extra) missingSlots.push([measure, EXTRA_DOUBLE_STOP_STEP]);
  }
  if (missingSlots.length > 0) throw new Error(`Missing supervised slots: ${JSON.stringify(missingSlots)}`);

  const overlay = {
    schemaVersion: 2,
    overlayType: 'supervised-intro-training-target-with-repeated-ending-double-stops',
    measureRange: [1, 6],
    sourceEventPath: path.relative(ROOT, SOURCE_PATH),
    trainingPackPath: path.relative(ROOT, PACK_PATH),
    events,
    eventCount: events.length,
    addedEndingDoubleStops: EXTRA_DOUBLE_STOP_MEASURES.map((measureNumber) => ({
      measureNumber,
      quantizedStep: EXTRA_DOUBLE_STOP_STEP,
    })),
    trainingOnly: true,
  ...
  page.drawText('Jimmy PAIge V8 — Supervised Intro Target Proof V2', { x: 237, y: 553, size: 11, font: fonts.bold, color: rgb(0.1, 0.1, 0.1) });
  page.drawText('TRAINING ONLY • bends, exact string/fret targets, repeated ending double-stops', { x: 226, y: 536, size: 7.5, font: fonts.body, color: rgb(0.32, 0.32, 0.32) });
  ...
  const repeatedEndingDoubleStops = events.filter((event) => (
    EXTRA_DOUBLE_STOP_MEASURES.includes(event.measureNumber)
    && event.quantizedStep === EXTRA_DOUBLE_STOP_STEP
    && event.notes.length > 1
  )).length;
  const manifest = {
    schemaVersion: 2,
    passed: events.length === 38
      && missingSlots.length === 0
      && bends === 6
      && doubleStops === 5
      && repeatedEndingDoubleStops === 2,
    eventCount: events.length,
    bendsRendered: bends,
    endingDoubleStopsRendered: doubleStops,
    repeatedEndingDoubleStopsRendered: repeatedEndingDoubleStops,
    repeatedEndingDoubleStopMeasures: EXTRA_DOUBLE_STOP_MEASURES,
    missingSlots,
    pages: 1,
  ...
  console.log('JIMMY PAIGE V8 SUPERVISED INTRO OVERLAY + PROOF V2');
  ...
  console.log('Repeated ending double-stops rendered:', repeatedEndingDoubleStops);
  console.log('Repeated ending double-stop measures:', EXTRA_DOUBLE_STOP_MEASURES);
