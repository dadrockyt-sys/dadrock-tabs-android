...
IMPACT_REVIEW_PATH = PUBLIC / "gomyway-rhythm-anchor-36-whole-song-impact-review-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-51-training-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-rhythm-novel-anchor-51-training-v1-manifest.json"
...
def main() -> None:
    ...
    if impact_review.get("passed") is not True:
        raise RuntimeError("Anchor 36 whole-song impact review V1 is not green.")
    if impact_review.get("readyForNovelAnchor51Training") is not True:
        raise RuntimeError("Impact review is not ready for novel anchor 51 training.")
    ...
    if impact_review.get("anchor36ImpactAcceptedReadOnly") is not True:
        raise RuntimeError("Anchor 36 impact is no longer accepted read-only.")
    if impact_review.get("broadOvergeneralizationDetected") is not False:
        raise RuntimeError("Anchor 36 impact review now reports broad overgeneralization.")
    ...
    recommended = (
        "review-gomyway-rhythm-novel-anchor-51-training-v1"
        if passed
        else "diagnose-gomyway-rhythm-novel-anchor-51-training-v1"
    )
    ...
    output = {
        ...
        "anchor36CoverageGainsPreserved": [66, 99],
        ...
        "readyForNovelAnchor51TrainingReview": passed,
    ...
    print("GOMYWAY RHYTHM NOVEL ANCHOR 51 TRAINING V1 COMPLETE")
    print("Passed:", passed)
    ...
    print("Anchor 36 coverage gains preserved: [66, 99]")
    ...
    print("Ready for novel anchor 51 training review:", passed)
