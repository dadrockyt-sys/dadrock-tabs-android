...
POLICY_NAME = "local-or-two-view-harmonic-support-primary-v2"


@dataclass(frozen=True)
class ContextualHarmonicPrimaryV2Diagnostics:
...
def apply_contextual_harmonic_primary_shadow_v2(
    carrier_rows: Sequence[Mapping[str, Any]],
    grid: Mapping[EventKey, float],
    precision: PrecisionShadowResult,
) -> tuple[PrecisionShadowResult, ContextualHarmonicPrimaryV2Diagnostics]:
    """Repair strongest-raw upper-harmonic primaries from source-only context.

    V2 preserves V1's same-measure +/-2-step lower-primary confirmation. When
    ...
    each source view separately and the lower candidate has strictly more positive
    upper-harmonic-family members than the current primary in both views.
    ...
    for key in sorted(precision.retained_events):
        ...
        if row is None:
            raise RuntimeError(f"Contextual harmonic v2 has no row for {key}")
        ...
        after.add(lower)
        if not after:
            raise RuntimeError(f"Contextual harmonic v2 emptied pitch set at {key}")
    ...
    if shadow.retained_events != precision.retained_events:
        raise RuntimeError("Contextual harmonic v2 changed attack identity")
    for key in shadow.retained_events:
        ...
        if not selected.issubset(observed):
            raise RuntimeError(f"Contextual harmonic v2 invented pitch at {key}")
        if int(shadow.primary_midis[key]) not in selected:
            raise RuntimeError(f"Contextual harmonic v2 lost primary at {key}")

    diagnostics = ContextualHarmonicPrimaryV2Diagnostics(
...
__all__ = [
    "POLICY_NAME",
    "ContextualHarmonicPrimaryV2Diagnostics",
    "apply_contextual_harmonic_primary_shadow_v2",
]
