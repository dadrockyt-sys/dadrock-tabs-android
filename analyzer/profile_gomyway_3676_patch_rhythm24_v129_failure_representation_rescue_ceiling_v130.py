...
V129_PATH = PUBLIC / "gomyway-3676-patch-rhythm24-v128-failure-anatomy-v129.json"
OUTPUT_PATH = PUBLIC / "gomyway-3676-patch-rhythm24-v129-failure-representation-rescue-ceiling-v130.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-patch-rhythm24-v129-failure-representation-rescue-ceiling-v130-manifest.json"
...
def main() -> None:
    ...
    v129 = json.loads(V129_PATH.read_text(encoding="utf-8"))
    ...
    if int(v129.get("schemaVersion", -1)) != 129:
        raise RuntimeError("V129 failure anatomy required")

    failures = list(v129.get("failureRows") or [])
    if len(failures) != 11:
        raise RuntimeError(f"Expected 11 exposed V128 failures, got {len(failures)}")
    ...
    print("GOMYWAY V130 EXPOSED V128 FAILURE REPRESENTATION RESCUE CEILING", flush=True)
    print("Evaluating only the 11 already-exposed V128 failures; no untouched reserve used", flush=True)
    ...
    for n, fr in enumerate(failures, 1):
        ...
        src = v128_rows.get((round(phase, 12), fold))
        if src is None:
            raise RuntimeError(f"Could not reconstruct V128 row phase={phase} fold={fold}")
        ...
        old_q = float(src.get("originalTrainingOnlyQ"))
        selector = src.get("selector") or {}
        candidate_q, decision, _ = v80.selected_q({"outerQ": old_q, "selector": selector})
        bucket = str(fr.get("originalQBucket"))
        q_to_use = candidate_q if bucket == "tight" else ANCHOR_Q
        if bucket == "broad" and str(fr.get("v96Decision")) == "keep-broad-low-dispersion":
        ...
        print(f"{n:02d}/11 phase={phase:.7f} fold={fold} rescuedBy={rescued_by}", flush=True)

    oracle_rescued = sum(bool(r["rescuedBy"]) for r in detail)
    remaining = len(detail) - oracle_rescued
    oracle_score = 309 + oracle_rescued
    ...
    structure_rows.sort(key=lambda r: -max((r["rescuesByRepresentation"] or {"": 0}).values()))

    after = sha256(candidate_path)
    if before != after:
        raise RuntimeError("Protected candidate changed during V130")
    ...
    out = {
        "schemaVersion": 130,
        "profileType": "exposed-v128-failure-only-representation-rescue-ceiling-diagnostic",
        "summary": summary,
        "structuralRescueCounts": structure_rows,
        "failureDetail": detail,
        "derivedInteractionFeatures": interaction_names,
        "evaluatesOnlyAlreadyExposedV128Failures": True,
        "doesNotMeasureRepresentationLossesOnV128Passes": True,
    ...
    MANIFEST_PATH.write_text(json.dumps({k: v for k, v in out.items() if k != "failureDetail"}, indent=2) + "\n", encoding="utf-8")

    print("\nGOMYWAY V130 FAILURE REPRESENTATION RESCUE CEILING COMPLETE")
    print("Frozen V128: 309/320 = 96.5625%")
    for rep in ALT_REPS:
        print(f"{rep}: rescues={rescue_counts[rep]}/11")
    ...
    for row in structure_rows:
        print(row)
    print("\nImportant: V130 tests exposed failures only and does NOT measure losses on the 309 V128 passes")
