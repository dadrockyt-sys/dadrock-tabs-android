...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1258-champion-eighth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1258-champion-eighth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (168, 699, 1635)
EXPECTED_F1 = 12.58
...
RULES = [
    {
        "name": "drop_eighth_zero_low_mid_strength",
        "drop": sorted([list(x) for x in EIGHTH_ZERO_BUCKETS if x[1] == "8_10"]),
    },
    {
        "name": "drop_eighth_zero_high_strength",
        "drop": sorted([list(x) for x in EIGHTH_ZERO_BUCKETS if x[1] in {"13_16", "20_plus"}]),
    },
    {
        "name": "drop_all_eighth_profiled_zero_precision_pitch_strength",
        "drop": sorted([list(x) for x in EIGHTH_ZERO_BUCKETS]),
    },
]
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 12.58 pruned recall champion...", flush=True)
    ...
    champion = recall.precond.merge_with_cap(core, seventh_kept)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 12.58 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    results = {}
    for rule in RULES:
        eighth_kept = prune_residual(seventh_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, eighth_kept)
        audit = prune1163.evaluate(candidate, champion, reference, champion_score)
        results[rule["name"]] = {
            "rule": rule,
            "keptProposedAdditions": int(sum(eighth_kept.values())),
            **audit,
        }
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_12_58_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 12.58 eighth pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_12_58_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-12.58-recall-champion-eighth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-eighth-pruned-champion" if validated else "retain-12.58-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 12.58 CHAMPION EIGHTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
