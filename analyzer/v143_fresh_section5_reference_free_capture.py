...
def capture_fresh_section5_reference_free(source_audio: bytes, suffix: str = ".m4a") -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="v143-fresh-section5-") as temp_dir:
        root = Path(temp_dir)
        source = root / f"uploaded{safe_suffix}"
        normalized = root / "normalized.wav"
        source.write_bytes(source_audio)
        if source.stat().st_size <= 0:
            raise RuntimeError("Fresh Section 5 source audio is empty")
        ...
        if not onset_groups:
            raise RuntimeError("No fresh Section 5 physical onset groups produced")
        ...
        for index, group in enumerate(onset_groups, start=1):
            ...
            if index % 250 == 0:
                print(f"fresh Section 5 onset spectra {index}/{len(onset_groups)}")
        ...
        return {
            "cacheVersion": 1,
            "scope": "fresh-section5-measures-65-80-reference-free",
            "section": {"name": "Fresh Section 5", "startMeasure": FIRST_MEASURE, "endMeasure": LAST_MEASURE},
            "timing": {
...
@app.local_entrypoint(name="capture_fresh_section5")
def capture_fresh_section5(audio_path: str = str(DEFAULT_AUDIO_PATH)) -> None:
    ...
    print("=== V143 FRESH SECTION 5 REFERENCE-FREE CAPTURE ===")
    ...
    result = capture_fresh_section5_reference_free.remote(payload, source.suffix)

    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_PATH.write_text(json.dumps(result, indent=2) + "\n")

    print()
    print("=== V143 FRESH SECTION 5 CACHE CAPTURED ===")
