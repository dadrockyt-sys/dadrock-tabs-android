...
OUTPUT_PATH = PUBLIC / "gomyway-3676-patch-rhythm24-v104-joint-model-q-rescue-ceiling-v105.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-patch-rhythm24-v104-joint-model-q-rescue-ceiling-v105-manifest.json"
...
def main() -> None:
            ...
            for fold in range(OUTER_FOLDS):
                ...
                if use_v96:
                    ...
                    for r, l in MODEL_GRID:
                        ...
                        for q in Q_GRID:
                            key = f"r{r}_lambda{l:g}_q{q:.3f}"
                            passed, _ = v88.pass_at_q(scores, y[test], q)
                            combo_passes[key] = bool(passed)
                else:
                    v96_pass = v28_pass
                    combo_passes = {k: bool(v28_pass) for k in combo_stats}

                total += 1
                v28_passes += int(v28_pass)
                v96_passes += int(v96_pass)
                oracle_pass = any(combo_passes.values())
                ...
                for key, passed in combo_passes.items():
                    s = combo_stats[key]
                    s["passes"] += int(passed)
                ...
                rows_out.append({
                    ...
                    "v96Lambda": lam,
                    "excluded": excluded,
                    "jointOraclePassed": bool(oracle_pass),
                })
    ...
    ranked = sorted(
        combo_stats.items(),
        key=lambda kv: (-kv[1]["passes"], kv[1]["regressionsVsV28"], -kv[1]["rescuesVsV28"], kv[0]),
    )
    top_fixed = dict(ranked[:15])

    after = sha256(candidate_path)
    if before != after:
        raise RuntimeError("Protected candidate changed during V105")
    ...
    summary = {
        ...
        "modelGrid": [{"pairRadius": r, "lambda": lam} for r, lam in MODEL_GRID],
        "qGrid": Q_GRID,
        "topFixedModelQCounterfactuals": top_fixed,
        "perfectPerFoldJointModelQOraclePasses": oracle_passes,
        "perfectPerFoldJointModelQOracleScorePercent": round(100.0 * oracle_passes / total, 4),
        "oracleRescuesOfV96Failures": oracle_rescues,
        "v96FailuresNotRescuedByAnyModelQCombination": unrescued,
    }

    out = {
        "schemaVersion": 105,
        "profileType": "fixed-cosine-joint-model-q-rescue-ceiling-on-old-exposed-v56-v57",
        "summary": summary,
        "rowsDetail": rows_out,
        ...
        "diagnosticJointModelQGridEnumerated": True,
    ...
    print("GOMYWAY V105 JOINT MODEL+Q RESCUE CEILING DIAGNOSTIC COMPLETE")
    print(f"Jimmy exposed V96 scoreboard: {v96_passes}/{total} = {100.0*v96_passes/total:.4f}%")
    print("\n=== TOP FIXED MODEL+Q COUNTERFACTUALS ===")
    for key, s in ranked[:15]:
        print(key, s)
    print(f"\nPerfect per-fold JOINT MODEL+Q oracle: {oracle_passes}/{total} = {100.0*oracle_passes/total:.4f}%")
    print(f"Oracle rescues of V96 failures: {oracle_rescues}")
    print(f"V96 failures not rescued by ANY model+q combination: {unrescued}")
    ...
    print("Diagnostic joint model+q outcomes tainted for future selection: True")
