...
app = modal.App("dadrock-v143-contextual-prune-candidate-product")
...
def _research_normalize_audio(source: Path, destination: Path) -> Path:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Reference-free candidate normalization failed:\n"
            + (result.stderr or result.stdout or "unknown ffmpeg error")[-4000:]
        )
    if not destination.exists() or destination.stat().st_size <= 0:
        raise RuntimeError("Reference-free candidate normalization produced no audio")
    return destination
...
def _build_stems(normalized: Path, output_dir: Path) -> tuple[Path, Path]:
    ...
    for label, path in (("direct", direct), ("cascade", cascade)):
        if not path.exists() or path.stat().st_size <= 0:
            raise RuntimeError(f"Candidate {label} guitar view is missing: {path}")
    if stems.get("deterministic") is not True or stems.get("referenceFree") is not True:
        raise RuntimeError("Candidate separator lost deterministic reference-free invariants")
...
def _promote_candidate_sustain(events: list[dict[str, Any]], tempo_bpm: float) -> list[dict[str, Any]]:
    ...
    for index, raw in enumerate(events):
        ...
        event["rhythmSustain"] = {
            ...
            "source": "reference-free-two-view-harmonic-persistence-candidate",
...
def analyze_corrected_candidate(
    source_audio: bytes,
    suffix: str = ".audio",
    measure_start: int = 1,
    measure_end: int = 113,
    ...
    if len(source_audio) > 50 * 1024 * 1024:
        raise ValueError("Corrected candidate audio cannot exceed 50 MB")
    ...
    with tempfile.TemporaryDirectory(prefix="v143-corrected-candidate-") as temp_dir:
        root = Path(temp_dir)
        ...
        targets = set(range(int(measure_start), int(measure_end) + 1))
        ...
        candidate = build_corrected_candidate_assembly(
            carrier.rows,
            carrier.grid,
            correction,
            carrier.timing,
        ...
        if attack_locations != set(correction.corrected_events):
            raise RuntimeError("Candidate semantics/sustain changed corrected attack identity")
        if len(events) < len(attack_locations):
            raise RuntimeError("Candidate event assembly lost corrected attacks")
        ...
        return {
            ...
            "selectedCount": int(len(correction.corrected_events)),
            "assembly": {
                "version": 3,
                "mode": "v143-contextual-prune-corrected-candidate",
                "polyphonicExpansion": len(events) > len(correction.corrected_events),
                "selectedAttackCount": len(correction.corrected_events),
            ...
            "liveV143": {
                ...
                "runtimeLabelsRequired": False,
                "candidateMode": "isolated-contextual-prune-corrected",
            },
            "candidate": {
                "schemaVersion": 1,
                "mode": "v143-contextual-prune-corrected-approved-audio",
...
def approved_audio(
    ...
    output_path: str = "debug/v143-contextual-prune/corrected-candidate-product.json",
) -> None:
    ...
    result = analyze_corrected_candidate.remote(source_bytes, source.suffix)
    candidate = result.get("candidate") or {}
    if candidate.get("approvedFixture") is not True:
        raise RuntimeError("Corrected candidate did not receive the approved fixture")
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(result, indent=2, allow_nan=False) + "\n", encoding="utf-8")
    print(json.dumps(result["candidateDiagnostics"], sort_keys=True))
    print(json.dumps(result["correctionDiagnostics"], sort_keys=True))
