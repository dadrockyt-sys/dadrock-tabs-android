...
OUTPUT_PATH = PUBLIC / "gomyway-2731-pitch-trajectory-survivors-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2731-pitch-trajectory-survivors-precision-v1-manifest.json"
EXPECTED = (183, 684, 290)
EXPECTED_F1 = 27.31
EXPECTED_ZERO_SIGNATURES = 6
EXPECTED_PRUNE_COUNT = 13
...
def reconstruct_2731(grid, winner_audio, winner_sr, alt_audio, alt_sr, reference):
    champion2705 = trajectory.reconstruct_2705(
    ...
    if len(zero_rows) != EXPECTED_ZERO_SIGNATURES:
        raise RuntimeError(
            f"Expected {EXPECTED_ZERO_SIGNATURES} validated pitch-trajectory signatures, got {len(zero_rows)}"
        )
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid validated zero-precision pitch-trajectory row: {row}")

    exact = {str(row["signature"]) for row in zero_rows}
    pruned: Counter[tuple[int, int, int]] = Counter()
    for tok, count in champion2705.items():
    ...
    if prune_count != EXPECTED_PRUNE_COUNT or true_pruned != 0 or false_pruned != EXPECTED_PRUNE_COUNT:
        raise RuntimeError(
            f"Failed to reconstruct validated pitch-trajectory prune: "
            f"pruned={prune_count} true={true_pruned} false={false_pruned}"
        )

    champion = champion2705 - pruned
    ...
    if actual != EXPECTED or abs(float(score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected frozen 27.31 champion {EXPECTED}/{EXPECTED_F1}, "
...
def main() -> None:
    ...
    champion, applied_prune = reconstruct_2731(
        grid, winner_audio, winner_sr, alt_audio, alt_sr, reference
    )
    score = recur.grade(champion, reference)
    ...
    def record(tok: tuple[int, int, int], count: int, truth: str) -> None:
        ...
        wf = trajectory.trajectory_features(winner_audio, winner_sr, center, pitch)
        af = trajectory.trajectory_features(alt_audio, alt_sr, center, pitch)
        signatures = sorted(trajectory.signatures_for(wf, af))
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 27.31 pitch-trajectory survivor profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-27.31-pitch-trajectory-survivors-precision",
        "champion2731Score": score,
        "featureFamily": "dual-stem-pitch-trajectory-stability-survivors",
        "appliedPitchTrajectorySignatureCount": EXPECTED_ZERO_SIGNATURES,
        "appliedPitchTrajectoryPruneCount": int(sum(applied_prune.values())),
        "appliedPitchTrajectoryTruePruned": int(sum((applied_prune & reference).values())),
    ...
    print("GOMYWAY 27.31 PITCH TRAJECTORY SURVIVORS PRECISION V1 COMPLETE")
    print("Passed: True")
    print("Champion pitch F1:", score["pitchF1"])
    print("Champion matched/missing/extra:", score["matched"], "/", score["missing"], "/", score["extra"])
    print("Applied pitch-trajectory signature count:", EXPECTED_ZERO_SIGNATURES)
    print("Applied pitch-trajectory prune count:", int(sum(applied_prune.values())))
    print("Applied pitch-trajectory true pruned:", int(sum((applied_prune & reference).values())))
    print("Generalizable zero-precision pitch-trajectory survivor signatures (5+ false, 0 true):", len(zero))
    for row in zero[:40]:
        print(f"{row['signature']}: true={row['true']} false={row['false']} precision={row['precision']}")
    print("Top supported-true pitch-trajectory survivor signatures:")
