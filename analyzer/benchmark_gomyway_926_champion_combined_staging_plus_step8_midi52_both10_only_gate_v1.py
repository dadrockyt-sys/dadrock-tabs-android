...
OUTPUT_PATH = PUBLIC / "gomyway-926-champion-combined-staging-plus-step8-midi52-both10-only-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-926-champion-combined-staging-plus-step8-midi52-both10-only-gate-v1-manifest.json"
...
def combined_prune(additions, winner_scores, alt_scores, adaptive_base):
    staged = gate926.deep_prune(additions, winner_scores, alt_scores, STAGING_RULE)
    filtered = staged.copy()
    for token, count in list(staged.items()):
        _measure, step, pitch = token
        reason = pruning.reason_bucket(token, adaptive_base, winner_scores, alt_scores)
        if step == 8 and pitch == 52 and reason == "both10_only":
...
def main() -> None:
    ...
    print("Rebuilding official validated 9.26 champion and combined staging/reason candidate...", flush=True)
    ...
    filtered = combined_prune(additions_926, winner_scores, alt_scores, adaptive_base)
    ...
    dropped = sum(additions_926.values()) - sum(filtered.values())
    validated_new_champion = bool(result["acceptedOverChampion"])

    after = sha256(CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during combined 9.26 benchmark.")

    rule = {
        "name": "drop_step0_midi57_score8_10_single_weak_plus_step8_midi52_both10_only",
        "stagingRule": STAGING_RULE,
        "complementarySelector": {"step": 8, "pitch": 52, "reason": "both10_only"},
    }
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-9.26-champion-combined-safe-staging-plus-reason-gate",
        "baselineScore": baseline_score,
        "rule": rule,
        "result": result,
        "additionsDroppedFrom926": dropped,
    ...
    print("GOMYWAY 9.26 CHAMPION COMBINED STAGING PLUS STEP8 MIDI52 BOTH10_ONLY GATE V1 COMPLETE")
    print("Passed: True")
