...
prune1312 = p1312.prune1312

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1312-champion-thirteenth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1312-champion-thirteenth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1582)
EXPECTED_F1 = 13.12
WINNER_NAME = "dual6_quarter_40_64"

THIRTEENTH_ZERO_BUCKETS = {
    (51, "13_16"),
    (62, "20_plus"),
}

RULES = [
    {"name": "drop_thirteenth_zero_midi51_13_16", "drop": [[51, "13_16"]]},
    {"name": "drop_thirteenth_zero_midi62_20_plus", "drop": [[62, "20_plus"]]},
    {"name": "drop_all_thirteenth_profiled_zero_precision_pitch_strength", "drop": sorted([list(x) for x in THIRTEENTH_ZERO_BUCKETS])},
]
...
def main() -> None:
    ...
    core_score = p1312.grade(core, reference)
    ...
    twelfth_kept = prune1312.prune_residual(eleventh_kept, winner_scores, alt_scores, p1312.TWELFTH_PRUNE)

    champion = recall.precond.merge_with_cap(core, twelfth_kept)
    champion_score = p1312.grade(champion, reference)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 13.12 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    evaluator = p1272.prune1163.evaluate
    results = {}
    for rule in RULES:
        thirteenth_kept = prune_residual(twelfth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, thirteenth_kept)
        audit = evaluator(candidate, champion, reference, champion_score)
        results[rule["name"]] = {"rule": rule, "keptProposedAdditions": int(sum(thirteenth_kept.values())), **audit}
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_13_12_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.12 thirteenth pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_13_12_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.12-recall-champion-thirteenth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-thirteenth-pruned-champion" if validated else "retain-13.12-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.12 CHAMPION THIRTEENTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
