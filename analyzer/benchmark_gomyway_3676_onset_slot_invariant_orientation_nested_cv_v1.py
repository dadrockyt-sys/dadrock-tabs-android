...
OUTPUT_PATH = PUBLIC / "gomyway-3676-onset-slot-invariant-orientation-nested-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-onset-slot-invariant-orientation-nested-cv-v1-manifest.json"
EXPECTED = (272, 595, 341)
EXPECTED_F1 = 36.76
OUTER_FOLDS = 5
INNER_FOLDS = 3
FEATURES = ["attackMean", "fluxMean", "sustainMean", "midShareMean", "highShareMean", "onsetAgreement"]
TOP_K_CHOICES = [1, 2, 3, 5]
...
def learn_model(rows: list[dict[str, Any]], top_k: int, min_consistency: float) -> dict[str, Any]:
    ...
    eligible = [
        ...
        if int(info["support"]) >= 5 and float(info["consistency"]) >= min_consistency
    ]
    eligible.sort(key=lambda x: (-float(x[1]["consistency"]), -float(x[1]["meanAbsEffect"]), x[0]))
            ...
            for q in QUANTILES:
                ...
                for scheme_name, fold_fn in schemes:
                    for fold in range(INNER_FOLDS):
                        ...
                        fold_rows.append({"scheme": scheme_name, "fold": fold, "true": held["true"], "false": held["false"], "precision": held["precision"], "basePrecision": base["precision"], "lift": round(lift, 2), "passed": passed})
                ...
                candidates.append({"minConsistency": consistency, "topK": top_k, "quantile": q, "innerPassCount": pass_count, "innerFoldCount": len(fold_rows), "innerTrue": true_total, "innerFalse": false_total, "meanLift": round(mean_lift, 3), "folds": fold_rows})
    chosen = max(candidates, key=lambda r: (int(r["innerPassCount"]), float(r["meanLift"]), int(r["innerTrue"]) - int(r["innerFalse"]), int(r["innerTrue"]), float(r["minConsistency"]), -int(r["topK"]), -float(r["quantile"])))
    return int(chosen["topK"]), float(chosen["minConsistency"]), float(chosen["quantile"]), chosen
...
def evaluate_scheme(rows: list[dict[str, Any]], name: str, fold_fn: Callable[[int], int]) -> tuple[bool, list[dict[str, Any]]]:
    ...
    for fold in range(OUTER_FOLDS):
        ...
        row = {"scheme": name, "fold": fold, "trainRows": len(train), "testRows": len(test), "chosenTopK": top_k, "chosenMinConsistency": consistency, "chosenQuantile": q, "model": model, "threshold": threshold, "innerSelection": inner, "heldoutBase": base, "heldoutCandidate": held, "heldoutPrecisionLift": round(lift, 2), "passed": passed}
        out.append(row)
        print(f"  features={model['features']} consistency={consistency} q={q} held={held['true']}/{held['false']} precision={held['precision']} base={base['precision']} lift={round(lift, 2)} pass={passed}", flush=True)
...
def main() -> None:
    ...
    print("Starting strict nested invariant-orientation onset-slot CV V1", flush=True)
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during invariant-orientation onset-slot nested CV")

    output = {"schemaVersion": 1, "passed": True, "profileType": "36.76-onset-slot-invariant-orientation-nested-cv-exploratory", "baselinePitchF1": EXPECTED_F1, "baselineMatchedMissingExtra": list(EXPECTED), "normalCvPassed": normal_pass, "normalCv": normal, "sectionStabilityPassed": section_pass, "sectionCv": section, "shiftedWindowStabilityPassed": shifted_pass, "shiftedWindowCv": shifted, "invariantOrientationOnsetSlotArchitectureGeneralizes": generalizes, "validatedNewChampion": False, "validationNote": "Exploratory event-slot validation only. Feature orientation stability, feature count, consistency threshold, score weights, and decision quantile are learned from training data only through nested validation. No pitch recovery or champion promotion is allowed.", "professionalReferenceUsedDuringDetection": False, "professionalReferenceRole": "downstream-grading-training-label-validation-only", "protected949CandidateHashUnchanged": True, "candidateEventsModified": False, "v7EventsModified": False, "rendererModified": False, "protectedBaselinesChanged": False, "productionSeparatorChanged": False, "productionPromotionAllowed": False}
    manifest = {"schemaVersion": 1, "passed": True, "output": str(OUTPUT_PATH.relative_to(ROOT)), "candidateSha256": after, "invariantOrientationOnsetSlotArchitectureGeneralizes": generalizes, "validatedNewChampion": False, "productionPromotionAllowed": False}
    OUTPUT_PATH.write_text(json.dumps(output, indent=2) + "\n", encoding="utf-8")
    MANIFEST_PATH.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")

    print("GOMYWAY 36.76 ONSET SLOT INVARIANT ORIENTATION NESTED CV V1 COMPLETE")
    ...
    print("Invariant-orientation onset-slot architecture generalizes:", generalizes)
