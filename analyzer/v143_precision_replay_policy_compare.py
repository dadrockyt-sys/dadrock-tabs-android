...
def _positive(candidates: Mapping[int, Mapping[str, Any]]) -> dict[int, Mapping[str, Any]]:
    return {
        ...
        if _finite(item["attack"]) > POSITIVE_ATTACK_FLOOR and _finite(item["body"]) > POSITIVE_BODY_FLOOR
    }
...
def _strongest_raw_midi(candidates: Mapping[int, Mapping[str, Any]]) -> int:
    ...
    return int(max(source, key=lambda midi: (_finite(source[midi]["score"]), _finite(source[midi]["attack"]), -int(midi))))
...
def _harmonic_family_score(midi: int, positive: Mapping[int, Mapping[str, Any]]) -> float:
    ...
    for interval, weight in HARMONIC_INTERVAL_WEIGHTS.items():
        ...
        if upper is None or _finite(upper["attack"]) <= POSITIVE_ATTACK_FLOOR or _finite(upper["body"]) <= POSITIVE_BODY_FLOOR:
            continue
        score += float(weight) * max(0.0, min(_finite(base["score"]), _finite(upper["score"])))
    return float(score)
...
def _recomputed_primary_midi(candidates: Mapping[int, Mapping[str, Any]]) -> int:
    ...
    primary = max(family_scores, key=lambda midi: (family_scores[midi], _finite(positive[midi]["attack"]), -int(midi)))
...
def _verified_primary_midi(candidates: Mapping[int, Mapping[str, Any]]) -> int:
    ...
    if stored != recomputed:
        raise ValueError(f"stored replay primary {stored} does not match independently recomputed primary {recomputed}")
...
def _select(attack: Mapping[str, Any], *, policy: str) -> tuple[set[int], list[dict[str, Any]]]:
    ...
    for midi, item in positive.items():
        ...
        accepted = all(passes.values()) if policy == "legacy" or harmonic else sum(passes.values()) >= 2
        if policy not in {"legacy", "v2"}:
            raise ValueError(f"unknown policy: {policy}")
        if accepted:
            kept.add(int(midi))
        decisions.append({"midi": int(midi), "harmonicAbovePrimary": bool(harmonic), "floor": float(floor), "ratios": ratios, "passes": passes, "accepted": bool(accepted)})
...
def build_report(replay: Mapping[str, Any]) -> dict[str, Any]:
    if replay.get("schemaVersion") != 2 or replay.get("policy") != EXPECTED_POLICY:
        raise ValueError("replay schema/policy mismatch")
    for field in ("fixedRetainedAttackPitchReplayReady", "attackPolicyReplayReady", "sourceViewEvidenceReady", "precisionStrengthRecomputeReady", "zeroValuePreservationReady"):
        if replay.get(field) is not True:
            raise ValueError(f"replay evidence is not ready: {field}")
    if replay.get("referenceFree") is not True or replay.get("professionalReferenceUsed") is not False or replay.get("runtimeLabelsRequired") is not False or replay.get("productionModified") is not False:
        raise ValueError("replay provenance/safety flags are invalid")
    ...
    candidate_total = stored_selected_total = legacy_total = v2_total = added_total = removed_total = changed_attacks = 0
    v2_replay_mismatch_count = 0
    failed_dimension_counts: Counter[str] = Counter()
    primary64_attack_count = primary64_changed_count = non64_attack_count = non64_changed_count = 0
    ...
    for attack in attacks:
        ...
        for midi in additions:
            failed = sorted(name for name, passed in decision_by_midi[midi]["passes"].items() if not passed)
        ...
        if additions or removals or not stored_matches_v2:
            per_attack.append({
                "measure": key[0], "step": key[1], "primaryMidi": int(primary), "primaryRecomputed": True,
                "legacyPitchSet": sorted(legacy), "v2PitchSet": sorted(v2), "storedSelectedPitchSet": sorted(stored),
                "v2AddedMidis": additions, "v2RemovedMidis": removals, "storedMatchesV2": stored_matches_v2,
            })

    if candidate_total != int(replay.get("originalPitchHypothesisCount") or -1) or candidate_total != int(replay.get("retainedOriginalPitchHypothesisCount") or -1):
    ...
    if v2_replay_mismatch_count != 0:
        raise RuntimeError(f"stored v2 selection disagrees with independent CPU replay at {v2_replay_mismatch_count} attacks")

    return {
        "schemaVersion": 2,
        "classification": "reference-free-replay-policy-compare",
        "source": {
            "replayPolicy": str(replay.get("policy") or ""), "attackUniverse": "fixed-retained-attacks",
            "retainedAttackCount": len(attacks), "eligibleAttackCount": eligible_attack_count,
            "originalPitchHypothesisCount": candidate_total, "eligiblePitchHypothesisCount": eligible_pitch_count,
            "fixedRetainedAttackPitchReplayReady": True, "attackPolicyReplayReady": True,
            "sourceViewEvidenceReady": True, "precisionStrengthRecomputeReady": True, "zeroValuePreservationReady": True,
            "primaryRecomputeMatches": True, "storedV2ReplayMatches": True,
            "professionalReferenceUsed": False, "newInferenceUsed": False,
        },
        "comparison": {
            "storedSelectedPitchCount": stored_selected_total, "legacyRecomputedPitchCount": legacy_total,
            "v2RecomputedPitchCount": v2_total, "v2AddedPitchCount": added_total, "v2RemovedPitchCount": removed_total,
            "changedAttackCount": changed_attacks, "v2ReplayMismatchAttackCount": 0, "primaryRecomputeMismatchAttackCount": 0,
            "addedPitchFailedDimensionCounts": dict(sorted(failed_dimension_counts.items())),
            "primaryMidi64AttackCount": primary64_attack_count, "primaryMidi64ChangedAttackCount": primary64_changed_count,
            "non64AttackCount": non64_attack_count, "non64ChangedAttackCount": non64_changed_count,
        },
        "changedAttacks": per_attack,
        "referenceFree": True, "runtimeLabelsRequired": False, "productionModified": False,
    }


def _item(midi: int, score: float, attack: float, body: float, *, primary: bool = False, selected: bool = False) -> dict[str, Any]:
    return {"midi": midi, "score": score, "attack": attack, "body": body, "primary": primary, "selected": selected}


def _attack(*items: dict[str, Any], measure: int = 1, step: int = 0) -> dict[str, Any]:
    return {"measure": measure, "step": step, "retained": True, "candidateMidis": [int(item["midi"]) for item in items], "candidates": list(items)}


def _replay(attack: Mapping[str, Any]) -> dict[str, Any]:
    pitch_count = len(attack.get("candidates") or [])
    return {
        "schemaVersion": 2, "policy": EXPECTED_POLICY, "retainedAttackCount": 1, "eligibleAttackCount": 2,
        "originalPitchHypothesisCount": pitch_count, "retainedOriginalPitchHypothesisCount": pitch_count,
        "eligiblePitchHypothesisCount": pitch_count + 3, "fixedRetainedAttackPitchReplayReady": True,
        "attackPolicyReplayReady": True, "sourceViewEvidenceReady": True, "precisionStrengthRecomputeReady": True,
        "zeroValuePreservationReady": True, "referenceFree": True, "professionalReferenceUsed": False,
        "runtimeLabelsRequired": False, "productionModified": False, "attacks": [attack],
    }
...
def _self_test() -> None:
    ...
    from v143_precision_replay_voicing_validator import _self_test as _voicing_self_test
    ...
    cases = [
        ("score+attack", _attack(_item(60,1,1,1,primary=True,selected=True), _item(64,.86,.83,.79,selected=True)), {60}, {60,64}),
        ("score+body", _attack(_item(60,1,1,1,primary=True,selected=True), _item(64,.86,.79,.83,selected=True)), {60}, {60,64}),
        ("attack+body", _attack(_item(60,1,1,1,primary=True,selected=True), _item(64,.79,.86,.86,selected=True)), {60}, {60,64}),
        ("one-only", _attack(_item(60,1,1,1,primary=True,selected=True), _item(64,.86,.79,.79,selected=False)), {60}, {60}),
        ("harmonic-two-of-three", _attack(_item(60,1,1,1,primary=True,selected=True), _item(72,.95,.95,.91,selected=False)), {60}, {60}),
        ("harmonic-three-of-three", _attack(_item(60,1,1,1,primary=True,selected=True), _item(72,.95,.95,.95,selected=True)), {60,72}, {60,72}),
        ("promoted-harmonic-guard", _attack(_item(60,.8,.8,.8,primary=True,selected=True), _item(72,1,1,1,selected=False)), {60}, {60}),
        ("no-positive", _attack(_item(60,-.4,-.1,-.3,primary=True,selected=True), _item(64,-.5,-.2,-.4,selected=True)), {60,64}, {60,64}),
    ]
    for name, attack, expected_legacy, expected_v2 in cases:
        assert _verified_primary_midi(_candidate_map(attack)) == _stored_primary_midi(_candidate_map(attack)), name
        assert _select(attack, policy="legacy")[0] == expected_legacy, name
        assert _select(attack, policy="v2")[0] == expected_v2, name

    assert secondary_gate_decision(score_ratio=.86, attack_ratio=.83, body_ratio=.79, harmonic_above_primary=False)
    assert not secondary_gate_decision(score_ratio=.86, attack_ratio=.79, body_ratio=.79, harmonic_above_primary=False)
    assert not secondary_gate_decision(score_ratio=.95, attack_ratio=.95, body_ratio=.91, harmonic_above_primary=True)
    assert secondary_gate_decision(score_ratio=.95, attack_ratio=.95, body_ratio=.95, harmonic_above_primary=True)

    corrupted_primary = _attack(_item(60,.8,.8,.8,primary=False,selected=True), _item(72,1,1,1,primary=True,selected=True))
    try:
        _verified_primary_midi(_candidate_map(corrupted_primary))
    except ValueError:
        pass
    else:
        raise AssertionError("primary recomputation accepted corrupted stored primary")

    good_replay = _replay(cases[0][1])
    report = build_report(good_replay)
    assert report["comparison"]["legacyRecomputedPitchCount"] == 1
    assert report["comparison"]["v2RecomputedPitchCount"] == 2
    ...
    try:
        ...
        raise AssertionError("strict replay accepted stored-v2 selection mismatch")

    _voicing_self_test()
    print("PASS precision replay policy compare strict schema2 + voicing self-test")
...
def main() -> None:
    ...
    from v143_precision_replay_voicing_validator import validate_product_voicing
    report["voicingValidation"] = validate_product_voicing(product)
    Path(args.output).write_text(json.dumps(report, indent=2, sort_keys=True, allow_nan=False) + "\n")
    print(json.dumps({"comparison": report["comparison"], "voicingValidation": report["voicingValidation"]}, sort_keys=True))
