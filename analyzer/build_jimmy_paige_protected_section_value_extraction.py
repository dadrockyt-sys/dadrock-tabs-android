...
def as_measure(value: Any) -> int | None:
    if isinstance(value, int) and 1 <= value <= 113:
        return value
...
def collect_measure_values(value: Any) -> dict[int, dict[str, list[dict[str, Any]]]]:
    ...
    def walk(item: Any, inherited_measure: int | None = None, path: str = "root") -> None:
        ...
        if isinstance(item, dict):
                ...
                for key, child in item.items():
                    ...
                    for signal, tokens in SIGNAL_KEYS.items():
                        if any(token in lowered for token in tokens):
                            if scalar(child):
                                collected[current_measure][signal].append(
            ...
            for key, child in item.items():
                walk(child, current_measure, f"{path}.{key}")

        elif isinstance(item, list):
            for index, child in enumerate(item):
                walk(child, current_measure, f"{path}[{index}]")
...
def merge_measure_values(
    ...
    for measure, signal_map in source.items():
        for signal, rows in signal_map.items():
            for row in rows:
                destination[measure][signal].append(
...
def section_summary(
    ...
    signals_present = {
        signal: count > 0 for signal, count in signal_counts.items()
    }
...
def main() -> None:
    ...
    for path in candidate_paths:
        if not path.exists() or path in {EVIDENCE_PATH, PREFLIGHT_PATH, OUTPUT_PATH, reference_path}:
            continue
        try:
