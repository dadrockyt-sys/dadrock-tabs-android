...
def _present_near(
    ...
    for delta in range(-tolerance, tolerance + 1):
        ...
        if 0 <= source_step < STEPS_PER_MEASURE and any(
            int(atom["midi"]) == midi
            for atom in indexed.get(measure, {}).get(source_step, [])
        ):
            return True
    return False
...
def _build_feature_cache(
    ...
    *,
    tolerance: int,
    phase_modulus: int,
) -> dict[tuple[int, int], list[tuple[int, float, int, float, float]]]:
    """Precompute all expensive local/recurrence evidence once.

    Each tuple is (midi, best_quality, distance, recurrence, phase_support).
    Weight sweeps then become only cheap arithmetic over these cached tuples.
    """
    cache: dict[tuple[int, int], list[tuple[int, float, int, float, float]]] = {}
        ...
        for target_step in range(STEPS_PER_MEASURE):
            ...
            for delta in range(-tolerance, tolerance + 1):
                ...
                for atom in indexed.get(measure, {}).get(source_step, []):
                    ...
                    candidate = (quality, abs(delta))
                    previous = local.get(midi)
                    if previous is None or candidate[0] > previous[0] or (
                        candidate[0] == previous[0] and candidate[1] < previous[1]
                    ):
                        local[midi] = candidate

            features: list[tuple[int, float, int, float, float]] = []
            for midi, (best_quality, distance) in local.items():
                dev_pool = [m for m in DEVELOPMENT_MEASURES if m != measure]
                recurrence = (
                    sum(
                        1
                        for other in dev_pool
                        if _present_near(indexed, other, target_step, midi, tolerance)
                    )
                    / max(len(dev_pool), 1)
                )
                ...
                phase_pool = [
                    other
                    for other in dev_pool
                    if (other - FIRST_MEASURE) % phase_modulus
                    == (measure - FIRST_MEASURE) % phase_modulus
                ]
                if phase_pool:
                    phase = sum(
                        1
                        for other in phase_pool
                        if _present_near(indexed, other, target_step, midi, tolerance)
                    ) / len(phase_pool)
                else:
                    phase = recurrence

                features.append((midi, best_quality, distance, recurrence, phase))

            cache[(measure, target_step)] = features

    return cache


def _decode_cached(
    feature_cache: dict[tuple[int, int], list[tuple[int, float, int, float, float]]],
    measures: tuple[int, ...],
    config: dict[str, Any],
) -> dict[tuple[int, int], set[int]]:
    ...
    for measure in measures:
        for step in range(STEPS_PER_MEASURE):
            scored = [
                (
                    local_weight * max(quality - distance_penalty * distance, 0.0)
                    + recurrence_weight * recurrence
                    + phase_weight * phase,
                    midi,
                )
                for midi, quality, distance, recurrence, phase in feature_cache.get((measure, step), [])
            ]
...
def main() -> None:
    ...
    print("=== V143 FAST REPETITION-CONSENSUS DECODER ===", flush=True)
    print("candidatePitchAtomCount:", len(atoms), flush=True)
    print("Precomputing temporal/repetition evidence...", flush=True)

    feature_caches: dict[tuple[int, int], dict[tuple[int, int], list[tuple[int, float, int, float, float]]]] = {}
    for tolerance in (1, 2):
        for phase_modulus in (2, 4):
            feature_caches[(tolerance, phase_modulus)] = _build_feature_cache(
                indexed, tolerance=tolerance, phase_modulus=phase_modulus
            )
    print(f"Feature cache ready in {time.perf_counter() - started:.2f}s", flush=True)

    configs: list[dict[str, Any]] = []
    total = 2 * 3 * 3 * 3 * 3 * 6 * 3 * 2 * 2
    done = 0
    best_objective = -1.0
                            ...
                            for second_ratio in (0.55, 0.70, 0.85):
                                for max_polyphony in (1, 2):
                                    for phase_modulus in (2, 4):
                                        ...
                                        predicted = _decode_cached(
                                            feature_caches[(tolerance_steps, phase_modulus)],
                                        ...
                                        if done % 1000 == 0 or done == total:
                                            print(
                                                ...
                                                f"best={best_objective:.3f}% elapsed={time.perf_counter() - started:.1f}s",
                                                flush=True,
                                            )
    ...
    holdout_prediction = _decode_cached(
        feature_caches[(int(best_config["toleranceSteps"]), int(best_config["phaseModulus"]))],
        HOLDOUT_MEASURES,
        best_config,
    )
    ...
    report = {
        "decoderFamily": "reference-free-fast-repetition-consensus-temporal-decoder",
