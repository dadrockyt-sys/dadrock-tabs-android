...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-3161-spectral-tonal-noise-survivors-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-3161-spectral-tonal-noise-survivors-v1-manifest.json"
...
def main() -> None:
    ...
    def record(tok: tuple[int, int, int], count: int, truth: str) -> None:
        ...
        wf = spectral.spectral_features(winner_audio, winner_sr, center, pitch)
        af = spectral.spectral_features(alt_audio, alt_sr, center, pitch)
        signatures = sorted(spectral.signatures_for(wf, af))
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 31.61 spectral tonal-noise survivor profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-31.61-spectral-tonal-noise-survivors",
        "champion3161Score": score,
        "reconstruction": reconstruction,
        "featureFamily": "dual-stem-spectral-flatness-crest-harmonic-noise-residual-survivors",
    ...
    print("GOMYWAY 31.61 SPECTRAL TONAL-NOISE SURVIVORS V1 COMPLETE")
    print("Passed: True")
    print("Champion pitch F1:", score["pitchF1"])
    print("Champion matched/missing/extra:", score["matched"], "/", score["missing"], "/", score["extra"])
    print("Generalizable zero-precision spectral tonal-noise signatures (5+ false, 0 true):", len(zero))
    for row in zero[:50]:
        print(f"{row['signature']}: true={row['true']} false={row['false']} precision={row['precision']}")
    print("Top supported-true spectral tonal-noise signatures:")
