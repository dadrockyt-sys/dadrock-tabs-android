...
import benchmark_gomyway_1316_champion_finer_score_agreement_cross_signature_gate_v1 as gate1328

finer = gate1328.finer
p1316 = gate1328.p1316
v2 = gate1328.v2
v3 = gate1328.v3
recall = gate1328.recall
profile = gate1328.profile

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1328-champion-finer-cross-signatures-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1328-champion-finer-cross-signatures-v1-manifest.json"
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.28 champion for finer residual profiling...", flush=True)
    ...
    fourteenth_kept = p1316.prune1316.prune_residual(thirteenth_kept, winner_scores, alt_scores, p1316.FOURTEENTH_PRUNE)
    finer_kept = gate1328.prune_residual(
        fourteenth_kept,
        winner_scores,
        alt_scores,
        {"drop": gate1328.TARGET_SIGNATURES},
    )

    champion = recall.precond.merge_with_cap(core, finer_kept)
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 13.28 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {actual}/{champion_score['pitchF1']}")
    ...
    for token, count in effective.items():
        ...
        bump(
            tables["sectionStepPitchScoreAgreement"],
            f"{section}_step{step}_midi{pitch}_{score_band}_{agreement}",
            t,
            f,
        )
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.28 finer cross-signature profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.28-champion-finer-residual-cross-signature-profile",
        ...
        "recommendedNextAction": "design-next-finer-cross-signature-prune-from-validated-13.28-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.28 CHAMPION FINER CROSS-SIGNATURE PROFILE V1 COMPLETE")
    print("Passed: True")
