...
def capture_fresh_verse1_reference_free(source_audio: bytes, suffix: str = ".m4a") -> dict[str, Any]:
    """Capture raw attacks + whole-onset spectra for measures 17-32.
    ...
    with tempfile.TemporaryDirectory(prefix="v143-fresh-verse1-") as temp_dir:
        root = Path(temp_dir)
        source = root / f"uploaded{safe_suffix}"
        normalized = root / "normalized.wav"
        source.write_bytes(source_audio)
        if source.stat().st_size <= 0:
            raise RuntimeError("Fresh Verse 1 source audio is empty")
        ...
        if not onset_groups:
            raise RuntimeError("No fresh Verse 1 physical onset groups produced")
        ...
        for index, group in enumerate(onset_groups, start=1):
            ...
            if index % 250 == 0:
                print(f"fresh Verse 1 onset spectra {index}/{len(onset_groups)}")
        ...
        return {
            "cacheVersion": 1,
            "scope": "fresh-verse1-measures-17-32-reference-free",
...
@app.local_entrypoint(name="capture_fresh_verse1")
def capture_fresh_verse1(audio_path: str = str(DEFAULT_AUDIO_PATH)) -> None:
    ...
    result = capture_fresh_verse1_reference_free.remote(payload, source.suffix)
