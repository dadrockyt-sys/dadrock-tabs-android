...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-3161-harmonic-residual-cancellation-survivors-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-3161-harmonic-residual-cancellation-survivors-v1-manifest.json"
...
def main() -> None:
    ...
    def record(tok: tuple[int, int, int], count: int, truth: str) -> None:
        ...
        features = residual.residual_features(
            winner_audio, winner_sr, alt_audio, alt_sr, center, pitch
        )
        signatures = sorted(residual.signatures_for(features))
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 31.61 harmonic residual/cancellation survivor profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-31.61-harmonic-residual-cancellation-survivors",
        "champion3161Score": score,
        "reconstruction": reconstruction,
        "featureFamily": "cross-stem-harmonic-residual-cancellation-magnitude-agreement-and-dominance-survivors",
    ...
    print("GOMYWAY 31.61 HARMONIC RESIDUAL CANCELLATION SURVIVORS V1 COMPLETE")
    print("Passed: True")
    print("Champion pitch F1:", score["pitchF1"])
    print("Champion matched/missing/extra:", score["matched"], "/", score["missing"], "/", score["extra"])
    print("Generalizable zero-precision harmonic-residual survivor signatures (5+ false, 0 true):", len(zero))
    for row in zero[:50]:
        print(f"{row['signature']}: true={row['true']} false={row['false']} precision={row['precision']}")
    print("Top supported-true harmonic-residual survivor signatures:")
