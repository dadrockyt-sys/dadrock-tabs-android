...
PUBLIC = ROOT / "public"
PROFILE_PATH = PUBLIC / "gomyway-2328-dual-stem-transient-onset-morphology-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-2328-transient-onset-morphology-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2328-transient-onset-morphology-precision-prune-cv-v1-manifest.json"
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    a = transient.ratio_bucket(float(row["minAttackPreRatio"]), "attack")
    ax = transient.ratio_bucket(float(row["maxAttackPreRatio"]), "maxattack")
    c = transient.ratio_bucket(float(row["minFirst20Concentration"]), "conc")
    e = transient.ratio_bucket(float(row["maxEarlyOnsetRatio"]), "early")
    f = transient.flux_bucket(float(row["minSpectralFlux"]))
    l = transient.latency_bucket(float(row["maxPeakLatencyMs"]))
    cr = transient.crest_bucket(float(row["minOnsetCrest"]))
    d = transient.diff_bucket(float(row["stemAttackDifferenceOctaves"]))
    return {
        f"minAttack::{a}",
        f"maxAttack::{ax}",
...
def main() -> None:
    ...
    if profile_payload.get("professionalReferenceUsedDuringDetection") is not False:
        raise RuntimeError("23.28 transient-onset profile is not reference-free during detection")
    rows = list(profile_payload.get("rows", []))
    if not rows:
        raise RuntimeError("23.28 transient-onset profile has no rows")
    ...
    if len(zero_rows) != 5:
        raise RuntimeError(f"Expected exactly 5 transient-onset zero-precision signatures, got {len(zero_rows)}")
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 23.28 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"transient2328_exact_{idx:02d}", make_exact_predicate(signature)))

    def union_pred(selected: list[str]) -> Callable[[dict[str, Any]], bool]:
        chosen = set(selected)
        return lambda row: bool(row_signatures(row) & chosen)

    variants.append(("transient2328_union_top2", union_pred(exact_signatures[:2])))
    variants.append(("transient2328_union_top3", union_pred(exact_signatures[:3])))
    variants.append(("transient2328_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2328": accepted_over_2328,
        }
        results[name] = result
        print(
            ...
            f"sections={result['sectionStabilityPassed']} shifted={result['shiftedWindowStabilityPassed']} accepted={accepted_over_2328}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2328:
    ...
    if accepted:
        ...
        winner_name = "retain_23_28_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "extraReduction": 0,
            ...
            "acceptedOver2328": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 23.28 transient-onset precision prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-23.28-transient-onset-morphology-precision-prune-with-heldout-cv",
        "baseline2328Score": baseline,
    ...
    manifest = {
        ...
        "baseline2328PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 23.28 TRANSIENT-ONSET MORPHOLOGY PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
