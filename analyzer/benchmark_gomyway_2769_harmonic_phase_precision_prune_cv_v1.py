...
OUTPUT_PATH = PUBLIC / "gomyway-2769-harmonic-phase-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2769-harmonic-phase-precision-prune-cv-v1-manifest.json"
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing harmonic-phase signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("27.69 harmonic-phase profile has no rows")
    ...
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision harmonic-phase row: {row}")
    ...
    champion, _ = survivor.reconstruct_2769(
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 27.69 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"harmonicphase2769_exact_{idx:02d}", make_exact_predicate(signature)))
    variants.append(("harmonicphase2769_union_top2", union_pred(exact_signatures[:2])))
    variants.append(("harmonicphase2769_union_top3", union_pred(exact_signatures[:3])))
    variants.append(("harmonicphase2769_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2769": accepted_over_2769,
        }
        results[name] = result
        print(
            ...
            f"accepted={accepted_over_2769}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2769:
    ...
    if accepted:
        ...
        winner_name = "retain_27_69_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "truePruned": 0,
            ...
            "acceptedOver2769": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 27.69 harmonic-phase precision-prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-27.69-harmonic-phase-precision-prune-with-heldout-cv",
        "baseline2769Score": baseline,
    ...
    manifest = {
        ...
        "baseline2769PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 27.69 HARMONIC PHASE PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
