...
v124 = v134.v124
DEV_DIR = PUBLIC / "training" / "v141-envelope-curvature-development"
SPEC_PATH = DEV_DIR / "v141-envelope-curvature-spec.json"
OUT_PATH = DEV_DIR / "v141-training-only-selector.json"
...
if not SPEC_PATH.is_file():
    raise RuntimeError(f"Frozen V141 envelope-curvature spec missing: {SPEC_PATH}")

spec = json.loads(SPEC_PATH.read_text(encoding="utf-8"))
if int(spec.get("schemaVersion", -1)) != 141:
    raise RuntimeError("V141 envelope-curvature spec schema mismatch")
if spec.get("frozenBeforeDevelopmentRun") is not True:
    raise RuntimeError("V141 feature recipe was not frozen before development")
if int(spec.get("featureCount", -1)) != 15:
    raise RuntimeError("Expected exactly 15 frozen V141 curvature features")
if spec.get("heldoutV134ScoringAllowedDuringDevelopment") is not False:
    raise RuntimeError("V141 spec allows held-out V134 scoring during development")
for key in (
    ...
    if spec.get(key) is not False:
        raise RuntimeError(f"V141 frozen safety spec violation: {key}={spec.get(key)}")
if spec.get("qTuningAllowed") is not False:
    raise RuntimeError("V141 spec allows Q tuning")
if spec.get("lambdaTuningAllowed") is not False:
    raise RuntimeError("V141 spec allows lambda tuning")
if spec.get("pairRadiusTuningAllowed") is not False:
    raise RuntimeError("V141 spec allows pair-radius tuning")
...
if PHASE_OFFSETS != [0.0, 0.125]:
    raise RuntimeError(f"Unexpected frozen V141 phase offsets: {PHASE_OFFSETS}")
...
if bands != ["low", "lowMid", "mid", "highMid", "high"]:
    raise RuntimeError(f"Unexpected frozen V141 bands: {bands}")

curvature_cols: list[np.ndarray] = []
curvature_names: list[str] = []
for band in bands:
    burst = f"mean::{band}Burst"
    rise = f"mean::{band}Rise"
    decay30 = f"mean::{band}Decay30"
    decay60 = f"mean::{band}Decay60"
    post = f"mean::{band}PostSlope"
    missing = [n for n in (burst, rise, decay30, decay60, post) if n not in name_to_idx]
    if missing:
        raise RuntimeError(f"Missing V141 source columns for {band}: {missing}")

    b = xb[:, name_to_idx[burst]]
    r = xb[:, name_to_idx[rise]]
    d30 = xb[:, name_to_idx[decay30]]
    d60 = xb[:, name_to_idx[decay60]]
    p = xb[:, name_to_idx[post]]

    curvature_cols.append(b - 2.0 * r + d30)
    curvature_names.append(f"v141::{band}::early_curvature")
    curvature_cols.append(r - 2.0 * d30 + d60)
    curvature_names.append(f"v141::{band}::mid_curvature")
    curvature_cols.append(d30 - 2.0 * d60 + p)
    curvature_names.append(f"v141::{band}::late_curvature")

curvature_x = np.column_stack(curvature_cols)
if curvature_x.shape[1] != 15:
    raise RuntimeError(f"Expected 15 V141 curvature columns, got {curvature_x.shape[1]}")
if not np.isfinite(curvature_x).all():
    raise RuntimeError("Non-finite V141 curvature feature values")
...
def evaluate_view(row: dict, base_matrix: np.ndarray, challenger_matrix: np.ndarray, outer_train: np.ndarray, phase_offset: float) -> dict:
        ...
        inner_rows.append(
            {
                ...
                "v134Pass": base["pass"],
                "v141Pass": challenger["pass"],
                "v134Lift": base["lift"],
                "v141Lift": challenger["lift"],
                "liftDelta": round(challenger["lift"] - base["lift"], 6),
            }
        )

    gains = sum(int(item["v141Pass"] and not item["v134Pass"]) for item in inner_rows)
    losses = sum(int(item["v134Pass"] and not item["v141Pass"]) for item in inner_rows)
...
def training_only_decision(row: dict) -> dict:
    ...
    challenger_matrix = np.concatenate([base_matrix, curvature_x], axis=1)
    ...
    views = [evaluate_view(row, base_matrix, challenger_matrix, outer_train, offset) for offset in PHASE_OFFSETS]
    total_losses = sum(int(view["innerLosses"]) for view in views)
    selected = bool(all(bool(view["viewRulePassed"]) for view in views) and total_losses <= MAX_TOTAL_LOSSES)
    return {
        "phase": phase,
        "fold": outer_fold,
        ...
        "reason": "replicated frozen V141 training-only dominance" if selected else "replicated V141 dominance rule not met",
    }


print("=== V141 ENVELOPE CURVATURE TRAINING-ONLY DEVELOPMENT ===")
print("Frozen feature recipe loaded before scoring: True")
print("Envelope curvature features:", curvature_x.shape[1])
...
if candidate_before != candidate_after:
    raise RuntimeError("Protected 949-event candidate changed during V141 development")

out = {
    "experiment": "V141-envelope-curvature-training-only-development",
    "baselineChampion": {"name": "V134", "passes": 311, "total": 320, "scorePercent": 97.1875},
    "frozenEnvelopeCurvatureSpec": {
        "path": str(SPEC_PATH.relative_to(ROOT)),
        "sha256": sha256(SPEC_PATH),
        "featureCount": int(curvature_x.shape[1]),
        "featureNames": curvature_names,
    },
    "selector": {"rule": rule, "selectedCarriers": len(selected), "selectedByRepresentation": by_rep},
    "decisions": decisions,
...
print()
print("=== V141 DEVELOPMENT COMPLETE ===")
print(f"Training-only envelope-curvature selections: {len(selected)}/320")
