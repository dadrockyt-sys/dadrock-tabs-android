...
OUTPUT_PATH = PUBLIC / "gomyway-precision-pruned-champion-metrical-gate-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-precision-pruned-champion-metrical-gate-v1-manifest.json"

FOLDS = 5
BLOCKS = temporal.BLOCKS
SHIFTED_WINDOWS = temporal.SHIFTED_WINDOWS
CURRENT_CHAMPION_F1 = 8.22
TEMPORAL_RULE = {
...
# Predeclared detector-side metrical rules. These use only rhythm-grid position and
# detector confidence; they do not hard-code measures, pitches, or reference labels.
RULES = [
    {"name": "even_steps_only", "mode": "even"},
    {"name": "quarter_grid_only", "mode": "quarter"},
    {"name": "quarter_grid_or_both10", "mode": "quarter_or_both", "bothFloor": 10.0},
    {"name": "quarter_grid_or_both13", "mode": "quarter_or_both", "bothFloor": 13.0},
]
...
def metrical_prune(additions: Counter, winner_scores: dict, alt_scores: dict, rule: dict[str, Any]) -> Counter:
    out: Counter = Counter()
    mode = str(rule["mode"])
    both_floor = float(rule.get("bothFloor", 0.0))
    for token, count in additions.items():
        ...
        if mode == "even":
            ...
            keep = (step % 4 == 0)
        elif mode == "quarter_or_both":
            w = float(winner_scores.get(token, 0.0))
            a = float(alt_scores.get(token, 0.0))
            keep = (step % 4 == 0) or (w >= both_floor and a >= both_floor)
        else:
            raise ValueError(f"Unknown metrical mode: {mode}")
        if keep:
            out[token] = count
    return out
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 8.22 champion and metrical-gate candidates...", flush=True)
    ...
    champion_additions = pruning.prune(temporal_additions, winner_scores, alt_scores, adaptive_base, PRUNING_RULE)
    ...
    for rule in RULES:
        filtered_additions = metrical_prune(champion_additions, winner_scores, alt_scores, rule)
        ...
        section_passed = (
            blocks["positiveF1"] >= 4
            and blocks["extraReduced"] == len(BLOCKS)
        ...
        shifted_passed = (
            ...
            and windows["extraReduced"] == len(SHIFTED_WINDOWS)
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during metrical precision benchmark.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-8.22-champion-metrical-precision-gate",
        ...
        "recommendedNextAction": "lock-metrically-pruned-champion" if winner["acceptedOverChampion"] else "retain-8.22-and-profile-zero-precision-step-structure",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY PRECISION-PRUNED CHAMPION METRICAL GATE V1 COMPLETE")
    print("Passed: True")
    ...
    print("Winner accepted over 8.22 champion:", winner["acceptedOverChampion"])
