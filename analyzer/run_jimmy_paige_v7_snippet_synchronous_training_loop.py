...
OUTPUT_PATH = REPO_ROOT / "public" / "gomyway-jimmy-paige-v7-snippet-synchronous-training.json"
CHECKPOINT_PATH = REPO_ROOT / "public" / "gomyway-jimmy-paige-v7-snippet-synchronous-training-checkpoint.json"
LOG_PATH = REPO_ROOT / "jimmy-paige-v7-snippet-synchronous-training.log"

app = modal.App("dadrock-jimmy-paige-v7-snippet-synchronous-training")
image = (
...
def extract_attempt(audio_bytes: bytes, parameters: dict[str, Any]) -> bytes:
    ...
    started = time.time()
    print(f"[remote] synchronous attempt started: {parameters.get('name')}", flush=True)
    ...
    try:
        ...
        print(
            f"[remote] synchronous attempt complete: {parameters.get('name')} "
...
def _heartbeat(stop_event: threading.Event, attempt_number: int, total: int, name: str, started: float, interval: int) -> None:
    while not stop_event.wait(interval):
        _log(
            f"[synchronous heartbeat] attempt={attempt_number}/{total} | "
            f"name={name} | elapsed={time.time() - started:.1f}s | remote call active"
        )


def main() -> None:
    heartbeat_seconds = max(10, int(os.getenv("JIMMY_HEARTBEAT_SECONDS", "15")))
    max_attempts = max(
        1,
    ...
    report: dict[str, Any] = {
        "benchmarkVersion": 8,
        "benchmarkType": "jimmy-paige-v7-snippet-synchronous-training-loop",
    ...
    _log(
        "Starting synchronous Jimmy PAIge training | "
    ...
    with app.run():
        for index, raw_parameters in enumerate(ATTEMPTS[:max_attempts], start=1):
            ...
            name = str(parameters.get("name", f"attempt-{index}"))
            attempt_started = time.time()
            stop_event = threading.Event()
            thread = threading.Thread(
                target=_heartbeat,
                args=(stop_event, index, max_attempts, name, attempt_started, heartbeat_seconds),
                daemon=True,
            )

            _log(f"Attempt {index}/{max_attempts} starting synchronously | name={name}")
            thread.start()
            try:
                result_bytes = extract_attempt.remote(audio_bytes, parameters)
            finally:
                stop_event.set()
                thread.join(timeout=heartbeat_seconds + 2)
            ...
            _write_json(CHECKPOINT_PATH, report)
            _log(
                f"Attempt {index}/{max_attempts} complete | events={len(events)} | "
                f"correct={attempt_report['correctCandidateSlots']}/9 | "
                f"presence={attempt_report['candidatePresencePercentage']}% | "
                f"elapsed={attempt_report['totalElapsedSeconds']}s"
            )
