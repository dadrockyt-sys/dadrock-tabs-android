...
CHORD_TEMPLATES: list[tuple[str, set[int], tuple[float, float, bool]]] = [
    ("Am", {9, 0, 4}, (5.0, 7.0, False)),
    ("C/G", {0, 4, 7}, (5.0, 8.0, False)),
    ("D/F#", {2, 6, 9}, (2.0, 4.0, False)),
    ("Fmaj7", {5, 9, 0, 4}, (0.0, 3.0, True)),
    ("G/B", {7, 11, 2}, (0.0, 3.0, True)),
]
...
def phrase_notes(groups: list[list[dict[str, Any]]]) -> list[dict[str, Any]]:
    return [note for group in groups for note in group]


def infer_chord(groups: list[list[dict[str, Any]]]) -> dict[str, Any] | None:
    notes = phrase_notes(groups)
    if not notes:
        return None

    weighted = Counter()
    bass_note: dict[str, Any] | None = None
    for note in notes:
        ...
        if bass_note is None or midi < int(bass_note.get("midi") or 999):
            bass_note = note

    present = set(weighted)
    bass_pc = int(bass_note.get("midi") or 0) % 12 if bass_note else None
    total_weight = sum(weighted.values()) or 1.0
    best: dict[str, Any] | None = None

    for name, tones, zone in CHORD_TEMPLATES:
        ...
        if expected_bass is not None:
            score += 0.16 if bass_pc == expected_bass else -0.04
    ...
    if best is None or float(best["confidence"]) < 0.42:
        return None
    return best
...
def chord_path_adjustment(
    ...
    if lower <= center <= upper:
        reward = 18.0 * confidence
        adjustment -= reward
        reasons.append(f"inside-detected-chord-zone-{reward:.2f}")
    elif center < lower:
        penalty = ((lower - center) * 11.0 + 8.0) * confidence
        adjustment += penalty
        reasons.append(f"below-detected-chord-zone+{penalty:.2f}")
    else:
        penalty = ((center - upper) * 11.0 + 8.0) * confidence
        adjustment += penalty
        reasons.append(f"above-detected-chord-zone+{penalty:.2f}")

    opens = int(profile["openUpperCount"])
    if allow_open:
        if opens:
            reward = min(14.0, 4.0 + opens * 3.0) * confidence
            adjustment -= reward
            reasons.append(f"open-voicing-character-{reward:.2f}")
        elif center > 1.5:
            penalty = 10.0 * confidence
            adjustment += penalty
            reasons.append(f"missing-open-voicing+{penalty:.2f}")
    elif opens:
        penalty = min(20.0, opens * 7.0) * confidence
        adjustment += penalty
        reasons.append(f"unwanted-open-upper+{penalty:.2f}")

    if int(profile["span"]) > 4:
        penalty = (int(profile["span"]) - 4) * 2.5
        adjustment += penalty
        reasons.append(f"wide-hand-shape+{penalty:.2f}")
...
def chord_aware_build_phrase_paths(
    ...
    chord = infer_chord(groups)
    rescored: list[tuple[float, Any, float, dict[str, Any]]] = []

    for raw_score, path in candidates:
        adjustment, diagnostic = chord_path_adjustment(path, chord)
    ...
    _VOICING_DECISIONS.append(
        {
            "phraseIndex": len(_VOICING_DECISIONS),
            "phraseStart": phrase_start,
            "anchor": int(anchor),
            "detectedChord": chord,
            "candidateCount": len(rescored),
            "winner": rescored[0][3] if rescored else None,
            "topCandidates": [
                {
                    "rank": rank,
                    "score": round(score, 3),
...
v25.build_phrase_paths = chord_aware_build_phrase_paths
...
def analyze_audio_file(audio_path: str, transcription_type: str) -> dict[str, Any]:
    ...
    understanding["phrasePathChordVoicingZones"] = {
        "honestFixtureBaseline": 19.06,
        "decisionCount": len(_VOICING_DECISIONS),
        "decisions": list(_VOICING_DECISIONS),
        "policy": (
            "infer-the-harmonic-family-from-each-phrase-pitch-set-and-bass-note-"
            "then-rerank-the-complete-surviving-fingering-paths-at-the-layer-that-"
            "actually-selects-the-rendered-path"
        ),
    }
    result["musicalUnderstanding"] = understanding
    result["engineVersion"] = "5.7-phase-1-phrase-path-chord-voicing-zones"
    result["guitarBrainLesson"] = (
        "choose-the-chord-voicing-family-at-the-complete-phrase-path-layer-not-at-an-unused-style-hook"
    )
    return result
