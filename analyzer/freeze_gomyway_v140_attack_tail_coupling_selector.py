...
DEV_PATH = DEV_DIR / "v140-training-only-selector.json"
OUT_PATH = DEV_DIR / "v140-frozen-selector.json"
...
EXPECTED_KEYS = {
    (0.3056640625, 4, "cosine"),
    (0.6650390625, 2, "v112_interactions"),
    (0.7744140625, 2, "v112_interactions"),
}
...
if OUT_PATH.exists():
    raise RuntimeError(
        f"Frozen V140 selector already exists at {OUT_PATH}; refusing to overwrite it."
    )

if not SPEC_PATH.is_file():
    raise RuntimeError(f"Frozen V140 attack-tail spec missing: {SPEC_PATH}")
if not DEV_PATH.is_file():
    raise RuntimeError(f"V140 training-only development output missing: {DEV_PATH}")
...
if spec.get("frozenBeforeDevelopmentRun") is not True:
    raise RuntimeError("V140 attack-tail spec was not frozen before development")
if int(spec.get("featureCount", -1)) != 15:
    raise RuntimeError("V140 frozen attack-tail feature count is not 15")

spec_rule = dict(spec.get("selectionRule") or {})
if [float(x) for x in spec_rule.get("validationPhaseOffsets", [])] != EXPECTED_OFFSETS:
    raise RuntimeError("V140 replicated validation phase offsets changed")
...
required_false = {
    ...
    "v139HeldoutEvaluationRowsRead": dev.get("v139HeldoutEvaluationRowsRead"),
...
for name, value in required_false.items():
    if value is not False:
        raise RuntimeError(
            f"V140 development contamination/safety guard failed: {name}={value}"
        )

if dev.get("protected949CandidateHashUnchanged") is not True:
    raise RuntimeError(
        "Protected 949-event candidate was not unchanged during V140 development"
    )

baseline = dict(dev.get("baselineChampion") or {})
if int(baseline.get("passes", -1)) != 311 or int(baseline.get("total", -1)) != 320:
    raise RuntimeError("V140 development is not anchored to V134 311/320")

frozen_spec = dict(dev.get("frozenAttackTailSpec") or {})
if str(frozen_spec.get("sha256") or "") != sha256(SPEC_PATH):
    raise RuntimeError(
        "V140 development output does not match the current frozen attack-tail spec"
    )
if int(frozen_spec.get("featureCount", -1)) != 15:
    raise RuntimeError("V140 development output does not contain 15 attack-tail features")

rule = dict((dev.get("selector") or {}).get("rule") or {})
if rule != spec_rule:
    raise RuntimeError("V140 development selector rule does not match frozen spec")
...
if len(selected) != EXPECTED_SELECTED:
    raise RuntimeError(
        f"Expected exactly {EXPECTED_SELECTED} V140 selections, got {len(selected)}"
    )
...
if keys != EXPECTED_KEYS:
    raise RuntimeError(f"V140 selected carrier keys changed: {keys} != {EXPECTED_KEYS}")

by_rep = Counter(str(item.get("v134Representation") or "") for item in selected)
if dict(by_rep) != EXPECTED_BY_REP:
    raise RuntimeError(
        f"V140 selected-by-representation changed: {dict(by_rep)} != {EXPECTED_BY_REP}"
    )

for item in selected:
    if int(item.get("totalInnerLossesAcrossViews", -1)) > max_total_losses:
        raise RuntimeError(f"Selected V140 row violates total-loss rule: {item}")

    views = list(item.get("replicatedViews") or [])
    if len(views) != len(EXPECTED_OFFSETS):
        raise RuntimeError(f"Selected V140 row missing replicated views: {item}")

    offsets = [float(view.get("phaseOffset", 999.0)) for view in views]
    if offsets != EXPECTED_OFFSETS:
        raise RuntimeError(f"Selected V140 row phase offsets changed: {offsets}")
    ...
    for view in views:
        ...
        if view.get("viewRulePassed") is not True:
            raise RuntimeError(f"Selected V140 row has failed replicated view: {view}")
        if valid < min_valid:
            raise RuntimeError(f"Selected V140 view violates min-valid rule: {view}")
        if gains < min_gains:
            raise RuntimeError(f"Selected V140 view violates min-gains rule: {view}")
        if losses != 0:
            raise RuntimeError(f"Selected V140 view contains a pass loss: {view}")
        if mean_delta <= 0.0:
            raise RuntimeError(f"Selected V140 view violates positive-mean rule: {view}")
        if median_delta < 0.0:
            raise RuntimeError(f"Selected V140 view violates nonnegative-median rule: {view}")
        if nonnegative < required_nonnegative:
            raise RuntimeError(
                f"Selected V140 view violates nonnegative-fold rule: {view}"
            )
...
out = {
    "schemaVersion": 140,
    "experiment": "V140-frozen-attack-tail-coupling-selector",
    "baselineChampion": {
        "name": "V134",
        "passes": 311,
        "total": 320,
    ...
    "v139HeldoutEvaluationRowsUsed": False,
...
print("V139 held-out rows used: False")
