...
OUTPUT_PATH = PUBLIC / "gomyway-2813-temporal-density-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2813-temporal-density-precision-prune-cv-v1-manifest.json"
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing temporal-density signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("28.13 temporal-density profile has no rows")
    ...
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision temporal-density row: {row}")
    ...
    champion, _ = survivor.reconstruct_2813(
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 28.13 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"temporaldensity2813_exact_{idx:02d}", make_exact_predicate(signature)))
    variants.append(("temporaldensity2813_union_top2", union_pred(exact_signatures[:2])))
    variants.append(("temporaldensity2813_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2813": accepted_over_2813,
        }
        results[name] = result
        print(
            ...
            f"accepted={accepted_over_2813}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2813:
    ...
    if accepted:
        ...
        winner_name = "retain_28_13_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "truePruned": 0,
            ...
            "acceptedOver2813": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 28.13 temporal-density precision-prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-28.13-temporal-density-precision-prune-with-heldout-cv",
        "baseline2813Score": baseline,
    ...
    manifest = {
        ...
        "baseline2813PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 28.13 TEMPORAL DENSITY PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
