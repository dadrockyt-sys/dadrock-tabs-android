...
import benchmark_gomyway_3676_onset_slot_richer_audio_feature_consensus_nested_cv_v1 as v1

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
PROFILE_PATH = PUBLIC / "gomyway-3676-onset-slot-richer-audio-stability-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-3676-onset-slot-richer-audio-feature-consensus-nested-cv-v2.json"
MANIFEST_PATH = PUBLIC / "gomyway-3676-onset-slot-richer-audio-feature-consensus-nested-cv-v2-manifest.json"
EXPECTED = (272, 595, 341)
EXPECTED_F1 = 36.76
OUTER_FOLDS = 5
INNER_FOLDS = 3
FEATURES = list(v1.FEATURES)
CONSENSUS_MIN_SUPPORT = list(v1.CONSENSUS_MIN_SUPPORT)
FAILURE_PENALTY = list(v1.FAILURE_PENALTY)
QUANTILES = list(v1.QUANTILES)
TOP_K_CHOICES = list(v1.TOP_K_CHOICES)
...
def build_consensus_model_from_inner(
...
def precompute_search_splits(train: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for scheme_name, fold_fn in schemes:
        for fold in range(INNER_FOLDS):
            ...
            print(f"    precompute consensus split {done}/{total} ({scheme_name} {fold + 1}/{INNER_FOLDS}) ...", flush=True)
            inner_models = v1.collect_inner_models(inner_train)
            out.append({
                "scheme": scheme_name,
                "fold": fold,
                "train": inner_train,
                "test": inner_test,
                "innerModels": inner_models,
...
def choose_consensus_hyperparams(train: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
    splits = precompute_search_splits(train)
    candidates: list[dict[str, Any]] = []
    for min_support in CONSENSUS_MIN_SUPPORT:
        for penalty in FAILURE_PENALTY:
            for top_k in TOP_K_CHOICES:
                # Model construction is independent of q, so build once per split/consensus setting.
                built: list[tuple[dict[str, Any], dict[str, Any], dict[str, Any]]] = []
                for split in splits:
                    model, _ = build_consensus_model_from_inner(
                        split["train"], split["innerModels"], min_support, penalty, top_k
                    )
                    built.append((split, model, split["base"]))
                for q in QUANTILES:
                    folds: list[dict[str, Any]] = []
                    for split, model, b in built:
                        if not model["features"]:
                            held = {"true": 0, "false": 0, "precision": 0.0}
                        else:
                            threshold = threshold_for(split["train"], model, q)
                            held = apply(split["test"], model, threshold)
...
def evaluate_scheme(rows: list[dict[str, Any]], name: str, fold_fn: Callable[[int], int]) -> tuple[bool, list[dict[str, Any]]]:
    ...
    for fold in range(OUTER_FOLDS):
        ...
        print("    precompute final outer-train consensus models ...", flush=True)
        final_inner_models = v1.collect_inner_models(train)
        model, consensus = build_consensus_model_from_inner(
            train,
            final_inner_models,
...
def main() -> None:
    before = sha256(v1.base.richer.onset.prof.recall.CANDIDATE_PATH)
    ...
    lo, hi = min(measures), max(measures)
    print("Starting optimized strict nested richer-audio feature-consensus onset-slot CV V2", flush=True)
    ...
    after = sha256(v1.base.richer.onset.prof.recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during optimized richer-audio feature-consensus nested CV")

    output = {
        "schemaVersion": 2,
        "passed": True,
        "profileType": "36.76-onset-slot-richer-audio-feature-consensus-nested-cv-exploratory-optimized",
        ...
        "validationNote": "Optimized equivalent of V1 consensus search. Inner model families are precomputed once per split and reused across consensus hyperparameters. All feature success/failure consensus, signs/weights, and score thresholds remain training-only. Held-out labels are grading only. No pitch recovery or champion promotion is allowed.",
    ...
    print("GOMYWAY 36.76 ONSET SLOT RICHER AUDIO FEATURE CONSENSUS NESTED CV V2 COMPLETE")
