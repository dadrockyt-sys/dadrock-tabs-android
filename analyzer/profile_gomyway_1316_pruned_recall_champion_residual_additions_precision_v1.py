...
import benchmark_gomyway_1315_champion_fourteenth_zero_precision_pitch_strength_prune_v1 as prune1316
...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1316-pruned-recall-champion-residual-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1316-pruned-recall-champion-residual-additions-precision-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (172, 695, 1574)
EXPECTED_F1 = 13.16
WINNER_NAME = "dual6_quarter_40_64"
FOURTEENTH_PRUNE = next(
    r for r in prune1316.RULES
    if r["name"] == "drop_all_fourteenth_profiled_zero_precision_pitch_strength"
)
...
def main() -> None:
    ...
    fourteenth_kept = prune1316.prune_residual(thirteenth_kept, winner_scores, alt_scores, FOURTEENTH_PRUNE)
    ...
    if actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(
            f"Expected 13.16 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {actual}/{champion_score['pitchF1']}"
        )
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.16 residual profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.16-pruned-recall-champion-residual-additions-precision-profile",
        ...
        "recommendedNextAction": "inspect-validated-13.16-residual-profile-for-finer-cross-signatures-or-stop-zero-precision-loop",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.16 PRUNED RECALL CHAMPION RESIDUAL ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
