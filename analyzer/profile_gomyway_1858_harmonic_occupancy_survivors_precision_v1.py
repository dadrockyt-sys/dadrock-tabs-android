...
import benchmark_gomyway_1813_harmonic_occupancy_precision_prune_cv_v1 as b1813
...
PUBLIC = ROOT / "public"
PROFILE_1813_PATH = PUBLIC / "gomyway-1813-dual-stem-harmonic-occupancy-inharmonic-residual-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-1858-harmonic-occupancy-survivors-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1858-harmonic-occupancy-survivors-precision-v1-manifest.json"
...
def main() -> None:
    ...
    profile_payload = v2.load_json(PROFILE_1813_PATH)
    ...
    for row in survivor_rows:
        ...
        b = b1813.buckets(row)
        signatures = {
            "minOccupancy": b["minOcc"],
            "maxOccupancy": b["maxOcc"],
            "harmonicToSide": b["ratio"],
            "inharmonicResidual": b["resid"],
            "occupancyAgreement": b["agree"],
            "occupancyRatio": f"{b['minOcc']}|{b['ratio']}",
            "occupancyResidual": f"{b['minOcc']}|{b['resid']}",
            "ratioResidual": f"{b['ratio']}|{b['resid']}",
            "occupancyCross": f"{b['minOcc']}|{b['maxOcc']}|{b['agree']}",
            "harmonicStructureCross": f"{b['minOcc']}|{b['ratio']}|{b['resid']}|{b['agree']}",
        }
        for name, signature in signatures.items():
            groups[f"{name}::{signature}"][truth] += count
        details.append({**row, "survives1858Prune": True, "trueSurvivor": truth == "true"})
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 18.58 harmonic-occupancy survivor profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-18.58-harmonic-occupancy-survivors-precision",
        "champion1858Score": score,
        "validatedHarmonicOccupancyPruneRule": "occ1813_prune_union_all",
        "validatedHarmonicOccupancyPruneCount": int(sum(pruned.values())),
        ...
        "recommendedNextAction": "benchmark-only-repeatable-zero-precision-18.58-harmonic-occupancy-survivor-signatures-with-prune-specific-heldout-cv-or-freeze-family-if-exhausted",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 18.58 HARMONIC OCCUPANCY SURVIVORS PRECISION V1 COMPLETE")
    print("Passed: True")
    print("Champion pitch F1:", score["pitchF1"])
    print("Champion matched/missing/extra:", score["matched"], "/", score["missing"], "/", score["extra"])
    print("Validated harmonic-occupancy prune count:", sum(pruned.values()))
    print("Harmonic-occupancy survivor rows:", len(survivor_rows))
    print("Generalizable zero-precision harmonic-occupancy survivor signatures (5+ false, 0 true):")
    for row in zero_precision[:50]:
        print(f"  {row['signature']}: true=0 false={row['false']} precision=0.0%")
    print("Top supported true harmonic-occupancy survivor signatures (5+ true):")
