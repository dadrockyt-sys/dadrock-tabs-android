...
OUTPUT_PATH = PUBLIC / "gomyway-1454-periodicity-survivor-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1454-periodicity-survivor-additions-precision-v1-manifest.json"
EXPECTED_1454 = (183, 684, 1467)
EXPECTED_1454_F1 = 14.54
...
def main() -> None:
    ...
    champion_1430 = champion_1419 + gate.rows_to_counter(winner_rows, lambda row: True)
    ...
    champion_1454 = champion_1451 - fourth_pruned
    score_1454 = recur.grade(champion_1454, reference)
    actual = (int(score_1454["matched"]), int(score_1454["missing"]), int(score_1454["extra"]))
    if actual != EXPECTED_1454 or abs(float(score_1454["pitchF1"]) - EXPECTED_1454_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 14.54 champion {EXPECTED_1454}/{EXPECTED_1454_F1}, got {actual}/{score_1454['pitchF1']}")
    ...
    for row in remaining_rows:
        ...
        signatures = (
            ...
            f"{recur_b}|{ratio_b}|{template_b}",
        )
        for signature in signatures:
            groups[signature][truth] += 1
        details.append({**row, "survives1454Prune": True, "trueSurvivorAddition": truth == "true"})
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 14.54 survivor profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "profileType": "validated-14.54-periodicity-survivor-additions-precision",
        "champion1454Score": score_1454,
        "validatedFourthPruneRule": "survivor_prune_union_a_b_c",
        ...
        "recommendedNextAction": "benchmark-only-repeatable-zero-precision-14.54-survivor-prunes-with-prune-specific-heldout-cv-or-freeze-14.54-if-exhausted",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
        ...
        "championPitchF1": score_1454["pitchF1"],
        "survivorAdditionCount": len(remaining_rows),
    ...
    print("GOMYWAY 14.54 PERIODICITY SURVIVOR ADDITIONS PRECISION V1 COMPLETE")
    print("Passed: True")
    print("Champion pitch F1:", score_1454["pitchF1"])
    print("Champion matched/missing/extra:", score_1454["matched"], "/", score_1454["missing"], "/", score_1454["extra"])
