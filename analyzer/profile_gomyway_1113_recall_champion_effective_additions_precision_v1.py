...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1113-recall-champion-effective-additions-precision-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1113-recall-champion-effective-additions-precision-v1-manifest.json"
EXPECTED_BASE = (102, 765, 1227)
...
def bump(table: dict[str, list[int]], key: str, true_units: int, false_units: int) -> None:
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 champion and 11.13 recall winner...", flush=True)
    ...
    champion = recall.precond.merge_with_cap(base_champion, a)
    base_score = grade(champion, reference)
    if (base_score["matched"], base_score["missing"], base_score["extra"]) != EXPECTED_BASE:
        raise RuntimeError(f"Expected 9.29 baseline {EXPECTED_BASE}, got {base_score}")
    ...
    proposed = recall.recall_additions(grid, winner_scores, alt_scores, champion, variant)
    candidate = recall.precond.merge_with_cap(champion, proposed)
    recall_score = grade(candidate, reference)
    actual = (recall_score["matched"], recall_score["missing"], recall_score["extra"])
    if actual != EXPECTED_RECALL or abs(float(recall_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 11.13 recall winner {EXPECTED_RECALL}/{EXPECTED_F1}, got {actual}/{recall_score['pitchF1']}")

    effective = candidate - champion
    effective_true = effective & (reference - champion)
    effective_false = effective - effective_true

    tables = {name: {} for name in ["step", "pitch", "strength", "stepPitch", "stepStrength", "pitchStrength"]}
    for token, count in effective.items():
        _measure, step, pitch = token
        true_units = min(count, effective_true.get(token, 0))
        false_units = count - true_units
        min_score = min(winner_scores.get(token, 0.0), alt_scores.get(token, 0.0))
        strength = bucket(min_score)
        bump(tables["step"], f"step{step}", true_units, false_units)
        bump(tables["pitch"], f"midi{pitch}", true_units, false_units)
        bump(tables["strength"], strength, true_units, false_units)
        bump(tables["stepPitch"], f"step{step}_midi{pitch}", true_units, false_units)
        bump(tables["stepStrength"], f"step{step}_{strength}", true_units, false_units)
        bump(tables["pitchStrength"], f"midi{pitch}_{strength}", true_units, false_units)

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 11.13 recall precision profile.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-11.13-recall-champion-effective-additions-precision-profile",
        "baseChampionScore": base_score,
        "recallChampionScore": recall_score,
        "winnerVariant": variant,
        "proposedRecallAdditions": int(sum(proposed.values())),
        "effectiveNewUnits": int(sum(effective.values())),
        "effectiveTrueUnits": int(sum(effective_true.values())),
        "effectiveFalseUnits": int(sum(effective_false.values())),
        "precisionTables": {name: rows(table) for name, table in tables.items()},
        "professionalReferenceUsedDuringDetection": False,
        "professionalReferenceRole": "downstream-grading-training-label-and-effective-addition-profiling-only",
        ...
        "recommendedNextAction": "precision-prune-only-the-new-11.13-recall-additions-while-preserving-recall-matches",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
        ...
        "recallChampionPitchF1": recall_score["pitchF1"],
        "effectiveTrueUnits": int(sum(effective_true.values())),
        "effectiveFalseUnits": int(sum(effective_false.values())),
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 11.13 RECALL CHAMPION EFFECTIVE ADDITIONS PRECISION PROFILE V1 COMPLETE")
    print("Passed: True")
    print("Recall champion pitch F1:", recall_score["pitchF1"])
    print("Recall champion matched/missing/extra:", recall_score["matched"], "/", recall_score["missing"], "/", recall_score["extra"])
    print("Proposed recall additions:", sum(proposed.values()))
    print("Effective new units:", sum(effective.values()))
    print("Effective true / false:", sum(effective_true.values()), "/", sum(effective_false.values()))
