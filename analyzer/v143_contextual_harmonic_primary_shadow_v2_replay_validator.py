...
def _positive(attack: dict[str, Any]) -> dict[int, dict[str, Any]]:
    return {
        midi: item for midi, item in _candidate_map(attack).items()
        if float(item["attack"]) > POSITIVE_ATTACK_FLOOR and float(item["body"]) > POSITIVE_BODY_FLOOR
    }
...
def _strongest_raw(attack: dict[str, Any]) -> int | None:
    ...
    return int(max(positive, key=lambda midi: (
        float(positive[midi]["score"]), float(positive[midi]["attack"]), -int(midi)
    )))
...
def _local_lower(key: tuple[int, int], current: int, primary: dict[tuple[int, int], int], positive: dict[int, dict[str, Any]]) -> tuple[int | None, int]:
    ...
    if not support:
        return None, 0
    lower = max(support, key=lambda midi: (
        int(support[midi]), _family_score(int(midi), positive), float(positive[int(midi)]["score"]),
        float(positive[int(midi)]["attack"]), -int(midi),
    ))
...
def _two_view_lower(attack: dict[str, Any], current: int, positive: dict[int, dict[str, Any]]) -> tuple[int | None, dict[str, Any] | None]:
    ...
    winner = max(candidates, key=lambda row: (
        min(row[1], row[2]), max(row[1], row[2]), _family_score(row[0], positive),
        float(positive[row[0]]["score"]), float(positive[row[0]]["attack"]), -row[0],
    ))
    return winner[0], {
...
def _render_subset(attack: dict[str, Any], selected: set[int], primary: int) -> list[int]:
    ...
    others = sorted((m for m in selected if m != primary), key=lambda midi: (
        -float(cmap[midi]["score"]), -float(cmap[midi]["attack"]), -float(cmap[midi]["body"]), int(midi)
    ))
...
def validate(product: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in sorted(by_key):
        ...
        if local is not None:
            lower = int(local)
            reason = "local+two-view-harmonic-support" if two_view == local else "same-measure-local-primary-support"
        elif two_view is not None:
            lower = int(two_view)
            reason = "two-view-harmonic-support-dominance"
        else:
            continue
    ...
    return {
        "schemaVersion": 1,
        "policy": "local-or-two-view-harmonic-support-primary-v2",
