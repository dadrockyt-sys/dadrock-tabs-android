...
PUBLIC = ROOT / "public"
PROFILE_PATH = PUBLIC / "gomyway-2409-dual-stem-harmonic-band-concentration-v1.json"
OUTPUT_PATH = PUBLIC / "gomyway-2409-harmonic-band-concentration-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2409-harmonic-band-concentration-precision-prune-cv-v1-manifest.json"
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing harmonic-band signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("24.09 harmonic-band profile has no rows")
    ...
    if len(zero_rows) != EXPECTED_ZERO_SIGNATURES:
        raise RuntimeError(
            f"Expected exactly {EXPECTED_ZERO_SIGNATURES} harmonic-band zero-precision signatures, got {len(zero_rows)}"
        )
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision harmonic-band row: {row}")
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 24.09 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"harmonicband2409_exact_{idx:02d}", make_exact_predicate(signature)))

    def union_pred(selected: list[str]) -> Callable[[dict[str, Any]], bool]:
        chosen = set(selected)
        return lambda row: bool(row_signatures(row) & chosen)

    variants.append(("harmonicband2409_union_top2", union_pred(exact_signatures[:2])))
    variants.append(("harmonicband2409_union_top3", union_pred(exact_signatures[:3])))
    variants.append(("harmonicband2409_union_top5", union_pred(exact_signatures[:5])))
    variants.append(("harmonicband2409_union_all_zero_precision", union_pred(exact_signatures)))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2409": accepted_over_2409,
        }
        results[name] = result
        print(
            ...
            f"sections={result['sectionStabilityPassed']} shifted={result['shiftedWindowStabilityPassed']} accepted={accepted_over_2409}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2409:
    ...
    if accepted:
        ...
        winner_name = "retain_24_09_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "extraReduction": 0,
            ...
            "acceptedOver2409": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 24.09 harmonic-band precision prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-24.09-harmonic-band-concentration-precision-prune-with-heldout-cv",
        "baseline2409Score": baseline,
    ...
    manifest = {
        ...
        "baseline2409PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 24.09 HARMONIC-BAND CONCENTRATION PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
