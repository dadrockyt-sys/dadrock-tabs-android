...
OUTPUT_PATH = PUBLIC / "gomyway-metrically-pruned-champion-extras-profile-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-metrically-pruned-champion-extras-profile-v1-manifest.json"
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 8.77 metrically-pruned champion...", flush=True)
    ...
    if candidate_hash_before != candidate_hash_after:
        raise RuntimeError("Protected candidate changed during 8.77 extras profile.")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-8.77-metrically-pruned-champion-extra-profile",
        "championScore": full,
        ...
        "recommendedNextAction": "design-third-stage-precision-pruning-from-validated-8.77-detector-side-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY METRICALLY-PRUNED CHAMPION EXTRAS PROFILE V1 COMPLETE")
    print("Passed: True")
    ...
    for label, key in [
        ("Score-bucket specialist precision", "scoreBucket"),
        ("Stem-agreement specialist precision", "stemAgreement"),
        ("Recurrence-reason specialist precision", "recurrenceReason"),
        ("Section specialist precision", "section"),
        ("Top pitch specialist buckets", "pitch"),
        ("Rhythm-step specialist precision", "step"),
        ("Step-modulo-4 specialist precision", "stepModulo4"),
    ]:
        print(label + ":")
        rows = profiles[key][:12] if key == "pitch" else profiles[key]
        for row in rows:
