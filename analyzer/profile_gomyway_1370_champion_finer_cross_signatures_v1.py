...
import benchmark_gomyway_1345_champion_narrow_score_agreement_gate_v1 as gate1370

p1345 = gate1370.p1345
p1328 = p1345.p1328
...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1370-champion-finer-cross-signatures-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1370-champion-finer-cross-signatures-v1-manifest.json"
...
def section_for(measure: int) -> str:
    return p1345.section_for(measure)
...
def narrow_score_band(value: float) -> str:
    return p1345.narrow_score_band(value)
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 13.70 champion for deeper residual profiling...", flush=True)
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 13.70 finer cross-signature profiler")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-13.70-champion-finer-residual-cross-signature-profile",
        ...
        "recommendedNextAction": "design-next-finer-cross-signature-prune-from-validated-13.70-profile",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 13.70 CHAMPION FINER CROSS-SIGNATURE PROFILE V1 COMPLETE")
    print("Passed: True")
