...
import profile_gomyway_2850_temporal_density_survivors_precision_v1 as survivor
...
OUTPUT_PATH = PUBLIC / "gomyway-2850-measure-position-precision-prune-cv-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-2850-measure-position-precision-prune-cv-v1-manifest.json"
...
def row_signatures(row: dict[str, Any]) -> set[str]:
    ...
    if not signatures:
        raise RuntimeError(f"Profile row missing measure-position signatures for token {row.get('token')}")
...
def main() -> None:
    ...
    if not rows:
        raise RuntimeError("28.50 measure-position profile has no rows")
    ...
    for row in zero_rows:
        if int(row.get("true", -1)) != 0 or int(row.get("false", 0)) < 5:
            raise RuntimeError(f"Invalid zero-precision measure-position row: {row}")
    ...
    champion, _ = survivor.reconstruct_2850(
    ...
    if actual != EXPECTED or abs(float(baseline["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected frozen 28.50 champion {EXPECTED}/{EXPECTED_F1}, got {actual}/{baseline['pitchF1']}")

    variants: list[tuple[str, Callable[[dict[str, Any]], bool]]] = []
    for idx, signature in enumerate(exact_signatures, start=1):
        variants.append((f"measureposition2850_exact_{idx:02d}", make_exact_predicate(signature)))

    for n in (2, 3, 5, 8, 12, 16, len(exact_signatures)):
        label = "all" if n == len(exact_signatures) else f"top{n}"
        variants.append((f"measureposition2850_union_{label}_zero_precision", union_pred(exact_signatures[:n])))
    ...
    for name, predicate in variants:
        ...
        result = {
            ...
            "acceptedOver2850": accepted_over_2850,
        }
        results[name] = result
        print(
            ...
            f"accepted={accepted_over_2850}",
            flush=True,
        )
        print("  folds:", folds, flush=True)
        if accepted_over_2850:
    ...
    if accepted:
        ...
        winner_name = "retain_28_50_champion"
        winner_eval = {
            "fullScore": baseline,
            "pruneCount": 0,
            "truePruned": 0,
            ...
            "acceptedOver2850": False,
        }
        validated_new_champion = False

    after = sha256(recall.CANDIDATE_PATH)
    if before != after:
        raise RuntimeError("Protected candidate changed during 28.50 measure-position precision-prune benchmark")

    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-28.50-measure-position-precision-prune-with-heldout-cv",
        "baseline2850Score": baseline,
    ...
    manifest = {
        ...
        "baseline2850PitchF1": baseline["pitchF1"],
        "winner": winner_name,
        "validatedNewChampion": validated_new_champion,
        "productionPromotionAllowed": False,
    }
    ...
    print("GOMYWAY 28.50 MEASURE POSITION PRECISION PRUNE CV V1 COMPLETE")
    print("Passed: True")
