...
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1244-champion-sixth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1244-champion-sixth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (167, 700, 1651)
EXPECTED_F1 = 12.44
...
RULES = [
    {
        "name": "drop_sixth_zero_low_mid_strength",
        "drop": sorted([list(x) for x in SIXTH_ZERO_BUCKETS if x[1] in {"8_10", "10_13"}]),
    },
    {
        "name": "drop_sixth_zero_high_strength",
        "drop": sorted([list(x) for x in SIXTH_ZERO_BUCKETS if x[1] in {"20_plus"}]),
    },
    {
        "name": "drop_all_sixth_profiled_zero_precision_pitch_strength",
        "drop": sorted([list(x) for x in SIXTH_ZERO_BUCKETS]),
    },
]
...
def main() -> None:
    ...
    print("Rebuilding frozen validated 9.29 core and 12.44 pruned recall champion...", flush=True)
    ...
    champion = recall.precond.merge_with_cap(core, fifth_kept)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 12.44 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    results = {}
    for rule in RULES:
        sixth_kept = prune_residual(fifth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, sixth_kept)
        audit = prune1163.evaluate(candidate, champion, reference, champion_score)
        results[rule["name"]] = {
            "rule": rule,
            "keptProposedAdditions": int(sum(sixth_kept.values())),
            **audit,
        }
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        ...
        winner_name, winner = "retain_12_44_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 12.44 sixth pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_12_44_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-12.44-recall-champion-sixth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-sixth-pruned-champion" if validated else "retain-12.44-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 12.44 CHAMPION SIXTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
