...
OUT_PATH = (
    ...
    / "training"
    / "v138-cross-band-sustain-development"
    / "v138-training-only-selector.json"
)
...
if int(spec.get("featureCount", -1)) != len(spec.get("features") or []):
    raise RuntimeError("V138 feature count does not match frozen feature list")
...
if spec.get("heldoutV134ScoringAllowedDuringDevelopment") is not False:
    raise RuntimeError("V138 spec allows held-out scoring during development")
if spec.get("v136dEvaluationRowsAllowed") is not False:
    raise RuntimeError("V138 spec allows V136D evaluation-row reuse")
if spec.get("v137HeldoutEvaluationRowsAllowed") is not False:
    raise RuntimeError("V138 spec allows V137 held-out evaluation-row reuse")
if spec.get("professionalMidtermAnswersAllowed") is not False:
    raise RuntimeError("V138 spec allows professional/midterm answers")
if spec.get("reserve11mod16InspectionAllowed") is not False:
    raise RuntimeError("V138 spec allows reserve inspection")
if spec.get("candidateModificationAllowed") is not False:
    raise RuntimeError("V138 spec allows candidate modification")
...
def training_only_decision(row: dict) -> dict:
    ...
    challenger_matrix = np.concatenate(
        [base_matrix, structure_x],
        axis=1,
    )
        ...
        inner_rows.append(
            {
                ...
                "v134Pass": base["pass"],
                "v138Pass": challenger["pass"],
                "v134Lift": base["lift"],
                "v138Lift": challenger["lift"],
                "liftDelta": round(
                    challenger["lift"] - base["lift"],
                    6,
                ),
            }
        )
    ...
    return {
        ...
        "nonnegativeLiftFolds": nonnegative_folds,
        "requiredNonnegativeLiftFolds": required_nonnegative,
        "reason": (
            "strict frozen V138 training-only dominance"
            if selected
            else "frozen V138 dominance rule not met"
        ),
        "innerResults": inner_rows,
    }


print("=== V138 CROSS-BAND SUSTAIN TRAINING-ONLY DEVELOPMENT ===")
print("Frozen feature recipe loaded before scoring: True")
print("Cross-band structure features:", structure_x.shape[1])
...
if candidate_before != candidate_after:
    raise RuntimeError(
        "Protected 949-event candidate changed during V138 development"
    )

out = {
    "experiment": "V138-cross-band-sustain-training-only-development",
    "baselineChampion": {
        "name": "V134",
        "passes": 311,
        "total": 320,
    ...
    "frozenStructureSpec": {
        ...
        "featureCount": int(structure_x.shape[1]),
        "featureNames": structure_names,
    },
    "selector": {
        "rule": rule,
...
print()
print("=== V138 DEVELOPMENT COMPLETE ===")
print(f"Training-only structure selections: {len(selected)}/320")
