...
import benchmark_gomyway_1253_champion_seventh_zero_precision_pitch_strength_prune_v1 as prune1258
import benchmark_gomyway_1258_champion_eighth_zero_precision_pitch_strength_prune_v1 as prune1272
import profile_gomyway_1113_recall_champion_effective_additions_precision_v1 as profile

ROOT = Path(__file__).resolve().parents[1]
PUBLIC = ROOT / "public"
OUTPUT_PATH = PUBLIC / "gomyway-1272-champion-ninth-zero-precision-pitch-strength-prune-v1.json"
MANIFEST_PATH = PUBLIC / "gomyway-1272-champion-ninth-zero-precision-pitch-strength-prune-v1-manifest.json"
EXPECTED_CORE = (102, 765, 1227)
EXPECTED_CHAMPION = (169, 698, 1622)
EXPECTED_F1 = 12.72
...
SEVENTH_PRUNE = next(r for r in prune1258.RULES if r["name"] == "drop_all_seventh_profiled_zero_precision_pitch_strength")
EIGHTH_PRUNE = next(r for r in prune1272.RULES if r["name"] == "drop_all_eighth_profiled_zero_precision_pitch_strength")
...
RULES = [
    {"name": "drop_ninth_zero_10_13_strength", "drop": [[56, "10_13"]]},
    {"name": "drop_ninth_zero_20_plus_strength", "drop": sorted([list(x) for x in NINTH_ZERO_BUCKETS if x[1] == "20_plus"])},
    {"name": "drop_all_ninth_profiled_zero_precision_pitch_strength", "drop": sorted([list(x) for x in NINTH_ZERO_BUCKETS])},
]
...
def main() -> None:
    ...
    reference = recall.v3.reference_tokens(reference_payload)

    print("Rebuilding frozen validated 9.29 core and 12.72 pruned recall champion...", flush=True)
    ...
    seventh_kept = prune1258.prune_residual(sixth_kept, winner_scores, alt_scores, SEVENTH_PRUNE)
    eighth_kept = prune1272.prune_residual(seventh_kept, winner_scores, alt_scores, EIGHTH_PRUNE)

    champion = recall.precond.merge_with_cap(core, eighth_kept)
    ...
    if champion_actual != EXPECTED_CHAMPION or abs(float(champion_score["pitchF1"]) - EXPECTED_F1) > 0.01:
        raise RuntimeError(f"Expected 12.72 champion {EXPECTED_CHAMPION}/{EXPECTED_F1}, got {champion_actual}/{champion_score['pitchF1']}")

    results = {}
    for rule in RULES:
        ninth_kept = prune_residual(eighth_kept, winner_scores, alt_scores, rule)
        candidate = recall.precond.merge_with_cap(core, ninth_kept)
        audit = prune1163.evaluate(candidate, champion, reference, champion_score)
        results[rule["name"]] = {"rule": rule, "keptProposedAdditions": int(sum(ninth_kept.values())), **audit}
        s = audit["fullScore"]
        print(
    ...
    if accepted:
        winner_name, winner = max(accepted, key=lambda item: (float(item[1]["fullScore"]["pitchF1"]), int(item[1]["fullScore"]["matched"]), -int(item[1]["fullScore"]["extra"])))
    else:
        winner_name, winner = "retain_12_72_champion", {
    ...
    if before != after:
        raise RuntimeError("Protected candidate changed during 12.72 ninth pitch-strength prune benchmark.")

    ws = winner["fullScore"]
    validated = winner_name != "retain_12_72_champion"
    output = {
        "schemaVersion": 1,
        "passed": True,
        "benchmarkType": "validated-12.72-recall-champion-ninth-zero-precision-pitch-strength-prune",
        ...
        "recommendedNextAction": "profile-residual-recall-additions-from-validated-ninth-pruned-champion" if validated else "retain-12.72-and-profile-finer-cross-signatures",
    }
    manifest = {
        "schemaVersion": 1,
        "passed": True,
    ...
    print("GOMYWAY 12.72 CHAMPION NINTH ZERO-PRECISION PITCH-STRENGTH PRUNE V1 COMPLETE")
    print("Passed: True")
