...
const CONTRACT = 'songsterr-fresh-qualified-isolated-polyphonic-note-evidence-v1';
...
const EXPECTED_QUALIFICATION_CONTRACT = 'songsterr-fresh-model-note-qualification-v1';
...
if (!contextArg || !modelNotesArg || !qualificationArg || !outputArg) {
  throw new Error(
    'Usage: build_qualified_isolated_polyphonic_note_evidence.mjs '
...
const onsets = normalizedNotes.map((note, index) => {
  ...
  return {
    ...
    // Basic Pitch note-off remains diagnostic only; duration has separate authority.
    sourceEnd: null,
...
const evidence = {
  ...
  provenance: {
    ...
    modelNoteEndsUsedAsDuration: false,
    note: (
      'Basic Pitch emissions are preserved as raw candidates only. Promotion requires a separate exact-identity, '
      + 'reference-blind qualification decision independent of Basic Pitch candidate confidence. Explicitly rejected '
      + 'proposals remain preserved in evidence but are not promoted.'
    ),
  },
};
