...
export function projectV143RenderEvents(rawEvents) {
  ...
  for (let eventIndex = 0; eventIndex < Math.min(rawEvents.length, MAX_RENDER_EVENTS); eventIndex += 1) {
    ...
    const output = {
      // Raw analyzer events do not carry eventIndex; their authenticated identity
      // is their source-array index. Already-projected render events do carry it,
      // so preserve that identity on subsequent validation/render passes.
      eventIndex:
...
// projectV143RenderEvents. Unlike a compacting second projection, this preserves
// the original eventIndex values used by legato/connector relationships.
