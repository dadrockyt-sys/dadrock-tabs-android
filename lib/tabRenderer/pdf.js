...
export async function createBlankTabPDF({
  ...
  // Jimmy PAIge V8 target: portrait pages with two wide measures per row.
  ...
  // The wider professional layout needs more vertical breathing room.
  const pageOneRows = 7;
  const continuationRows = 9;
  ...
  function drawOpeningTimeSignature(page, rowTopY) {
    const [numerator = '4', denominator = '4'] =
      String(timeSignature).split('/');

    const timeSignatureX = startX + 12;
    const timeSignatureSize = 18;

    page.drawText(numerator, {
      x: timeSignatureX,
      y: rowTopY - 16,
      size: timeSignatureSize,
      font: boldFont,
      color: rgb(0.08, 0.08, 0.08),
    });
  ...
  function drawTabRow(
    ...
    if (firstMeasureNumber === 1) {
      drawOpeningTimeSignature(page, rowTopY);
    }
  }
