import fs from 'node:fs/promises';
import path from 'node:path';
import { PDFDocument, StandardFonts, rgb } from 'pdf-lib';

const PAGE = { width: 612, height: 792, left: 50, right: 560, bottom: 42 };
const STRINGS = ['E', 'B', 'G', 'D', 'A', 'E'];
const MEASURES_PER_ROW = 2;

function clean(value, fallback) {
  const text = String(value || fallback).replace(/\s+/g, ' ').trim();
  return text || fallback;
}

function sectionStartMap(sections = []) {
  return new Map(
    sections
      .filter((item) => item && Number(item.startMeasure) > 0)
      .map((item) => [Number(item.startMeasure), clean(item.label, 'Section')])
  );
}

function eventsByMeasure(events = [], totalMeasures = 1) {
  const grouped = new Map();

  for (const event of events) {
    if (!event || typeof event !== 'object') continue;

    const measureNumber = Number(event.measureNumber);
    const stringIndex = Number(event.stringIndex);
    const fret = Number(event.fret);
    const positionInMeasure = Number(event.positionInMeasure);

    if (
      !Number.isInteger(measureNumber) ||
      measureNumber < 1 ||
      measureNumber > totalMeasures ||
      !Number.isInteger(stringIndex) ||
      stringIndex < 0 ||
      stringIndex >= STRINGS.length ||
      !Number.isFinite(fret) ||
      fret < 0 ||
      !Number.isFinite(positionInMeasure)
    ) {
      continue;
    }

    const projected = {
      ...event,
      measureNumber,
      stringIndex,
      fret: Math.round(fret),
      positionInMeasure: Math.max(0, Math.min(0.999, positionInMeasure)),
    };

    const measureEvents = grouped.get(measureNumber) || [];
    measureEvents.push(projected);
    grouped.set(measureNumber, measureEvents);
  }

  for (const measureEvents of grouped.values()) {
    measureEvents.sort(
      (left, right) =>
        left.positionInMeasure - right.positionInMeasure ||
        left.stringIndex - right.stringIndex ||
        Number(left.eventIndex || 0) - Number(right.eventIndex || 0)
    );
  }

  return grouped;
}

export async function createJimmyPaigeV8Pdf({
  songTitle = 'Song Title',
  artistName = 'Artist Name',
  transcriptionType = 'Rhythm Guitar',
  tuning = 'Standard Tuning',
  bpm = 120,
  timeSignature = '4/4',
  totalMeasures = 1,
  sections = [],
  rhythmEvents = [],
} = {}) {
  const pdfDoc = await PDFDocument.create();
  const bold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const regular = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const italic = await pdfDoc.embedFont(StandardFonts.TimesRomanItalic);
  const logoBytes = await fs.readFile(
    path.join(process.cwd(), 'public', 'DadRock-Tabs-Logo.png')
  );
  const logoImage = await pdfDoc.embedPng(logoBytes);
  const largeLogo = logoImage.scaleToFit(190, 82);
  const smallLogo = logoImage.scaleToFit(95, 42);

  const safeMeasures = Math.max(1, Number(totalMeasures) || 1);
  const measureWidth = (PAGE.right - PAGE.left) / MEASURES_PER_ROW;
  const stringSpacing = 8;
  const staffHeight = stringSpacing * (STRINGS.length - 1);
  const rowHeight = 70;
  const starts = sectionStartMap(sections);
  const measureEvents = eventsByMeasure(rhythmEvents, safeMeasures);

  function drawHeader(page, compact = false) {
    if (compact) {
      page.drawImage(logoImage, {
        x: PAGE.left,
        y: 731,
        width: smallLogo.width,
        height: smallLogo.height,
      });
      page.drawText(
        `${clean(songTitle, 'Song Title')} — ${clean(artistName, 'Artist Name')}`,
        {
          x: PAGE.left + smallLogo.width + 10,
          y: 746,
          size: 11,
          font: bold,
          color: rgb(0.08, 0.08, 0.08),
        }
      );
      page.drawLine({
        start: { x: PAGE.left, y: 716 },
        end: { x: PAGE.right, y: 716 },
        thickness: 0.6,
        color: rgb(0.72, 0.72, 0.72),
      });
      return 690;
    }

    page.drawImage(logoImage, {
      x: (PAGE.width - largeLogo.width) / 2,
      y: 704,
      width: largeLogo.width,
      height: largeLogo.height,
    });
    page.drawText(clean(songTitle, 'Song Title'), {
      x: PAGE.left,
      y: 670,
      size: 20,
      font: bold,
      color: rgb(0.03, 0.03, 0.03),
    });
    page.drawText(clean(artistName, 'Artist Name'), {
      x: PAGE.left,
      y: 649,
      size: 12,
      font: regular,
      color: rgb(0.36, 0.36, 0.36),
    });
    page.drawText(clean(transcriptionType, 'Rhythm Guitar').toUpperCase(), {
      x: PAGE.left,
      y: 625,
      size: 9.5,
      font: bold,
      color: rgb(0.12, 0.12, 0.12),
    });
    const metadata = `${clean(tuning, 'Standard Tuning')} • ${clean(timeSignature, '4/4')} • ${Number(bpm) || 120} BPM`;
    const metadataWidth = regular.widthOfTextAtSize(metadata, 8.5);
    page.drawText(metadata, {
      x: PAGE.right - metadataWidth,
      y: 625,
      size: 8.5,
      font: regular,
      color: rgb(0.2, 0.2, 0.2),
    });
    return 590;
  }

  function drawTimeSignature(page, rowTop) {
    const [top = '4', bottom = '4'] = String(timeSignature).split('/');
    page.drawText(top, {
      x: PAGE.left + 13,
      y: rowTop - 17,
      size: 18,
      font: bold,
      color: rgb(0.08, 0.08, 0.08),
    });
    page.drawText(bottom, {
      x: PAGE.left + 13,
      y: rowTop - 37,
      size: 18,
      font: bold,
      color: rgb(0.08, 0.08, 0.08),
    });
  }

  function drawMeasureNotes(page, rowTop, measureNumber, measureIndex) {
    const notes = measureEvents.get(measureNumber) || [];
    const measureX = PAGE.left + measureIndex * measureWidth;
    const leftPadding = measureNumber === 1 ? 48 : 15;
    const rightPadding = 12;
    const usableWidth = measureWidth - leftPadding - rightPadding;
    const noteSize = 8.2;

    for (const note of notes) {
      const text = String(note.fret);
      const textWidth = bold.widthOfTextAtSize(text, noteSize);
      const centerX =
        measureX + leftPadding + note.positionInMeasure * usableWidth;
      const x = Math.max(
        measureX + leftPadding - textWidth / 2,
        Math.min(
          measureX + measureWidth - rightPadding - textWidth,
          centerX - textWidth / 2
        )
      );
      const stringY = rowTop - note.stringIndex * stringSpacing;

      page.drawRectangle({
        x: x - 1.5,
        y: stringY - 4.2,
        width: textWidth + 3,
        height: 8.5,
        color: rgb(1, 1, 1),
      });
      page.drawText(text, {
        x,
        y: stringY - 3.1,
        size: noteSize,
        font: bold,
        color: rgb(0.05, 0.05, 0.05),
      });
    }
  }

  function drawRow(page, rowTop, firstMeasure) {
    for (let index = 0; index < MEASURES_PER_ROW; index += 1) {
      const number = firstMeasure + index;
      if (number > safeMeasures) continue;

      const measureX = PAGE.left + index * measureWidth;
      const numberText = String(number);
      const numberSize = 7.5;
      const numberX = measureX + 4;
      const markerY = rowTop + 8;

      page.drawText(numberText, {
        x: numberX,
        y: markerY,
        size: numberSize,
        font: regular,
        color: rgb(0.38, 0.38, 0.38),
      });

      const sectionLabel = starts.get(number);
      if (sectionLabel) {
        const numberWidth = regular.widthOfTextAtSize(numberText, numberSize);
        page.drawText(`(${sectionLabel})`, {
          x: numberX + numberWidth + 4,
          y: markerY - 0.2,
          size: 6.6,
          font: italic,
          color: rgb(0.28, 0.28, 0.28),
        });
      }
    }

    STRINGS.forEach((label, index) => {
      const y = rowTop - index * stringSpacing;
      page.drawText(label, {
        x: 35,
        y: y - 3,
        size: 7,
        font: bold,
        color: rgb(0.12, 0.12, 0.12),
      });
      page.drawLine({
        start: { x: PAGE.left, y },
        end: { x: PAGE.right, y },
        thickness: 0.7,
        color: rgb(0.12, 0.12, 0.12),
      });
    });

    for (let divider = 0; divider <= MEASURES_PER_ROW; divider += 1) {
      const x = PAGE.left + divider * measureWidth;
      page.drawLine({
        start: { x, y: rowTop },
        end: { x, y: rowTop - staffHeight },
        thickness:
          divider === 0 || divider === MEASURES_PER_ROW ? 1.1 : 0.75,
        color: rgb(0.12, 0.12, 0.12),
      });
    }

    for (let index = 0; index < MEASURES_PER_ROW; index += 1) {
      const measureNumber = firstMeasure + index;
      if (measureNumber <= safeMeasures) {
        drawMeasureNotes(page, rowTop, measureNumber, index);
      }
    }

    if (firstMeasure === 1) drawTimeSignature(page, rowTop);
  }

  let page = pdfDoc.addPage([PAGE.width, PAGE.height]);
  let rowTop = drawHeader(page, false);
  let measure = 1;

  while (measure <= safeMeasures) {
    if (rowTop - rowHeight < PAGE.bottom) {
      page = pdfDoc.addPage([PAGE.width, PAGE.height]);
      rowTop = drawHeader(page, true);
    }

    drawRow(page, rowTop, measure);
    rowTop -= rowHeight;
    measure += MEASURES_PER_ROW;
  }

  const pages = pdfDoc.getPages();
  pages.forEach((currentPage, index) => {
    const footer = `Page ${index + 1} of ${pages.length} • Generated by DadRock Tabs • dadrocktabs.com`;
    const size = 7.5;
    const width = regular.widthOfTextAtSize(footer, size);
    currentPage.drawText(footer, {
      x: (PAGE.width - width) / 2,
      y: 18,
      size,
      font: regular,
      color: rgb(0.4, 0.4, 0.4),
    });
  });

  return pdfDoc.save();
}
