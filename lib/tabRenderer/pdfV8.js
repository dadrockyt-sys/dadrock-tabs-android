...
export async function createJimmyPaigeV8Pdf({
  ...
  function rowSectionStarts(firstMeasure) {
    const found = [];
    for (let index = 0; index < MEASURES_PER_ROW; index += 1) {
      const measureNumber = firstMeasure + index;
      const label = starts.get(measureNumber);
      if (label && measureNumber <= safeMeasures) {
        found.push({ index, measureNumber, label });
      }
    }
    return found;
  }
  ...
  function drawRow(page, rowTop, firstMeasure) {
    const sectionStarts = rowSectionStarts(firstMeasure);

    for (const section of sectionStarts) {
      const sectionX = PAGE.left + section.index * measureWidth;

      // Keep section names in their own band above measure numbers,
      // bends, accents, picking directions, and other notation.
      page.drawText(section.label, {
        x: sectionX,
        y: rowTop + sectionLabelOffset,
        size: 11,
        font: italic,
  ...
  while (measure <= safeMeasures) {
    const hasSectionStart = rowSectionStarts(measure).length > 0;
    const required = rowHeight + (hasSectionStart ? sectionOffset : 0);
