...
function formatTempo(value) {
  ...
  return `${Math.round(Number.isFinite(numeric) && numeric > 0 ? numeric : 120)} BPM`;
}
...
function eventToken(event) {
  ...
  if (
    Number(event.bendSemitones) >= 0.35 ||
    techniques.has('bend') ||
    techniques.has('bend-release') ||
    techniques.has('pre-bend')
  ) {
    ...
    const target = Number.isFinite(Number(event.bendTargetFret))
      ...
      : fret + amount;
    const prefix = techniques.has('pre-bend') ? 'pb' : 'b';
    let token = `${fret}${prefix}${target}`;
  ...
  if (techniques.has('vibrato')) token += '~';
  if (techniques.has('trill')) token += 'tr';
  return token;
}
...
function buildSystems(events) {
  ...
  for (let systemIndex = 0; systemIndex < systemCount; systemIndex += 1) {
    ...
    const lastMeasure = Math.min(
      maximumMeasure,
      firstMeasure + MEASURES_PER_SYSTEM - 1
    );
    systems.push({
      systemIndex,
      firstMeasure,
...
function connectorSymbol(event) {
  const techniques = techniqueSet(event);
  if (techniques.has('hammer-on')) return 'h';
  if (techniques.has('pull-off')) return 'p';
  if (techniques.has('slide-up')) return '/';
  if (techniques.has('slide-down')) return '\\';
  return '';
}

function techniqueAnnotation(event) {
  const techniques = techniqueSet(event);
  if (techniques.has('palm-mute')) return 'P.M.';
  if (techniques.has('let-ring')) return 'let ring';
  if (techniques.has('tap')) return 'T';
  if (techniques.has('trill')) return 'tr';
  if (techniques.has('sustain-tie')) return 'tie';
  if (techniques.has('pre-bend')) return 'pre-bend';
  return '';
}

function drawTechniqueConnector({ page, font, source, target, staffTop }) {
  const symbol = connectorSymbol(source);
  if (!symbol || !source || !target) return false;
  if (Number(source.stringIndex) !== Number(target.stringIndex)) return false;
  if (systemForMeasure(source.measure) !== systemForMeasure(target.measure)) return false;

  const sourcePos = positionForEvent(source, staffTop);
  const targetPos = positionForEvent(target, staffTop);
  if (targetPos.x <= sourcePos.x) return false;
...
export async function createV143RhythmPdf({
  ...
  if (presentation.oneNotePerMeasureCollapseDetected) {
    throw new Error(
      'V143 Rhythm render refused a suspicious one-note-per-measure collapsed event stream.'
    );
  }

  const sections = buildReferenceFreeRhythmSections(events);
  const sectionByMeasure = new Map(
    sections.map((section) => [Number(section.startMeasure), section])
  );
  ...
  function drawHeader(page, compact = false) {
    ...
    page.drawText('Timing: 16th-note grid • b bend • r release • h hammer-on • p pull-off • / \\ slide • ~ vibrato', {
      x: PAGE.marginX,
      y: 575,
      size: 6.25,
      font: bodyFont,
  ...
  function drawTimingGrid(page, staffTop, measureWidth, measureCount) {
    const stepWidth = measureWidth / STEPS_PER_MEASURE;
    for (let measureOffset = 0; measureOffset < measureCount; measureOffset += 1) {
      const measureX = PAGE.marginX + measureOffset * measureWidth;
      for (let step = 1; step < STEPS_PER_MEASURE; step += 1) {
        const x = measureX + step * stepWidth;
        const isBeat = step % 4 === 0;
        page.drawLine({
          start: { x, y: staffTop + (isBeat ? 3.2 : 1.8) },
          end: { x, y: staffTop - (isBeat ? STAFF_HEIGHT : 2.2) },
          thickness: isBeat ? 0.24 : 0.18,
          color: isBeat ? rgb(0.74, 0.74, 0.74) : rgb(0.88, 0.88, 0.88),
        });
      }
      for (let beat = 0; beat < 4; beat += 1) {
        const x = measureX + (beat * 4 + 0.5) * stepWidth;
        page.drawText(String(beat + 1), {
          x: x - 1.25,
          y: staffTop + 2.5,
          size: 4.8,
          font: bodyFont,
  ...
  function drawSystem(page, system, topY, locked = false) {
    const staffTop = topY - 18;
    const measureWidth = (PAGE.contentRight - PAGE.marginX) / MEASURES_PER_SYSTEM;
    const measureCount = Math.max(1, Number(system.measureCount) || MEASURES_PER_SYSTEM);
    const systemRight = PAGE.marginX + measureCount * measureWidth;
    const section = sectionByMeasure.get(system.firstMeasure);
    ...
    if (section) {
      page.drawText(section.label, {
        ...
        size: 6.6,
        font: titleFont,
        color: rgb(0.9, 0.24, 0),
      });
      page.drawText(`m. ${section.startMeasure}-${section.endMeasure}`, {
        x: PAGE.marginX + titleFont.widthOfTextAtSize(section.label, 6.6) + 7,
        y: topY - 1.5,
        size: 5.5,
        font: bodyFont,
    ...
    for (let offset = 0; offset < measureCount; offset += 1) {
    ...
    drawTimingGrid(page, staffTop, measureWidth, measureCount);
    ...
    if (locked) {
      page.drawRectangle({
        ...
        width: Math.max(4, systemRight - PAGE.marginX - 2),
    ...
    for (const event of system.events) {
      ...
      const size = token.length > 7 ? 5.2 : token.length > 5 ? 5.8 : token.length > 3 ? 6.5 : 7.4;
      ...
      if (Number(event.durationSteps) > 1) {
        ...
        if (sustainEnd > x + width / 2 + 3) {
          page.drawLine({
            start: { x: x + width / 2 + 2, y: y + 0.5 },
      ...
      const annotation = techniqueAnnotation(event);
      if (annotation) {
        page.drawText(annotation, {
          x: Math.max(PAGE.marginX, x - 5),
          y: staffTop + 14.8,
          size: annotation.length > 4 ? 4.7 : 5.5,
          font: annotation === 'P.M.' || annotation === 'T' ? titleFont : bodyFont,
    ...
    for (const source of system.events) {
      ...
      if (!connected) {
        const symbol = connectorSymbol(source);
        if (symbol) {
          const { x, y } = positionForEvent(source, staffTop);
          page.drawText(symbol, {
            x: x + 2.8,
            y: y + 4.2,
            size: 6,
  ...
  pages.forEach((pdfPage, pageIndex) => {
    if (preview) {
      ...
      pdfPage.drawText('• Full note placement  • timing grid  • sections  • techniques  • sustain', {
        x: PAGE.marginX + 18,
        y: 84,
        size: 7.4,
        font: bodyFont,
