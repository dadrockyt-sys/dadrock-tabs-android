...
function drawRehearsalMarks(page, system, staffTop, sections, titleFont, bodyFont) {
  for (const section of sections) {
    ...
    const rangeText = `SECTION ${raw}  m. ${section.startMeasure}-${section.endMeasure}`;
...
export async function createV143RhythmPdf({
  ...
  function drawFirstPageHeader(page) {
    page.drawText('DIY Guitar & Bass TAB Generator', {
      x: PAGE.marginX,
      y: 763,
      size: 7.2,
      font: titleFont,
    ...
    page.drawText('Powered by DadRock AI • V143 Rhythm', {
      x: PAGE.marginX,
      y: 751,
      size: 6.5,
      font: bodyFont,
    ...
    page.drawText('Timing: 16th-note grid', {
      x: PAGE.marginX,
      y: 664,
      size: 6.2,
      font: bodyFont,
  ...
  pages.forEach((pdfPage, pageIndex) => {
    ...
    const footerCenter = 'Generated by DadRock Tabs Studio • dadrocktabs.com';
