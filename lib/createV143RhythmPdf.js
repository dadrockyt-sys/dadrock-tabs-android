...
export async function createV143RhythmPdf({
  ...
  // The final renderer receives an already-authenticated V143 render stream.
  // Validate every event without compacting/re-projecting so the PDF layer can
  // never silently discard malformed rows or change connector event identity.
