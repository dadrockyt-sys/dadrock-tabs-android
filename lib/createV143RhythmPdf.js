...
export async function createV143RhythmPdf({
  ...
  function drawTimingGrid(page, staffTop, measureWidth) {
    const stepWidth = measureWidth / STEPS_PER_MEASURE;
    for (let measureOffset = 0; measureOffset < MEASURES_PER_SYSTEM; measureOffset += 1) {
      ...
      for (let step = 1; step < STEPS_PER_MEASURE; step += 1) {
        ...
        page.drawLine({
          ...
          color: isBeat ? rgb(0.77, 0.77, 0.77) : rgb(0.88, 0.88, 0.88),
        });
      }
  ...
  function drawSystem(page, system, topY, locked = false) {
    ...
    drawTimingGrid(page, staffTop, measureWidth);
