...
def evaluate_one(row: dict, matrix: np.ndarray) -> dict:
    ...
    model = legacy.recur.ridge.fit_ridge(
        matrix[train],
        y[train],
        lam,
    )

    scores = legacy.recur.ridge.scores_for(
        matrix[test],
        model,
    )

    held = legacy.rank_apply(
        scores,
        y[test],
        q,
    )

    base = legacy.base_stats(y[test])

    lift = (
        float(held["precision"])
...
if pass_mismatches:
    ...
    for r in pass_mismatches[:12]:
        print(r)
print("Historical V134 311/320 retained as context only.")
print("It is NOT used as a reproducibility gate for V136D.")
print("V136D will be compared against the recomputed baseline above.")
print()
print()
