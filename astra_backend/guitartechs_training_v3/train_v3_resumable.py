...
from guitartechs_training_v3.objective_decoder import (
...
RESUME_SCHEMA = "astra-guitar-techs-v3-resume-state-v1"
CANDIDATE_ID = "astra_guitartechs_tabcnn_v3_objective_decoder"
TOTAL_EPOCHS = 1000
...
V3_OBJECTIVE_DECODER_GIT_BLOB = "87e127c87851ea9a43b9a9e6240f1a7c4624424b"
V3_DESIGN_GIT_BLOB = "5074c691c5ec97625849d364a764c73694a86ce5"
V3_SYNTHETIC_VERIFICATION_GIT_BLOB = "e3cc2a9b15c9ee9be779d5e45a547efcae543087"
V3_AUTHORIZATION_GIT_BLOB = "133da867fa98b432fa3f7f2248c52cec330181ef"
...
def _infer_v3(model, feat, batch=1024):
...
def _identity(args, group_count, validation_capture_count):
    return {
        ...
        "source": {
            ...
            "v3ObjectiveDecoderGitBlob": V3_OBJECTIVE_DECODER_GIT_BLOB,
            "v3DesignGitBlob": V3_DESIGN_GIT_BLOB,
            "v3SyntheticVerificationGitBlob": V3_SYNTHETIC_VERIFICATION_GIT_BLOB,
            "v3AuthorizationGitBlob": V3_AUTHORIZATION_GIT_BLOB,
...
def _write_resume_receipt(path, payload, state_sha256):
    receipt = {
        "schema": "astra-guitar-techs-v3-resume-receipt-v1",
...
def train_segment(args):
    ...
    for epoch in range(start_epoch, args.end_epoch):
        ...
        for batch in batches:
            ...
            for item in batch:
                ...
                loss, _loss_parts = v3_sequence_loss(
                    logits,
                    y,
                    content_weight=content_weight,
                )
                if not torch.isfinite(loss):
                    raise RuntimeError("nonfinite V3 sequence loss")
        ...
        if (epoch + 1) % CHECKPOINT_EVERY_EPOCHS == 0:
            metrics = _aggregate_metrics_v3(selection_rows, model, args.data_dir)
    ...
    if len(checkpoint_summaries) != base.CHECKPOINTS:
        raise RuntimeError("final V3 checkpoint count mismatch")
    ...
    full_metrics = _aggregate_metrics_v3(val, model, args.data_dir)
    ...
    receipt = {
        "schema": "astra-guitar-techs-real-training-fold-v3",
        "candidateId": CANDIDATE_ID,
        "fold": args.fold,
        "trainPerformer": args.train_performer,
        "validationPerformer": args.val_performer,
    ...
    print("V3_TRAINING_RESULT=" + json.dumps({
        "fold": args.fold,
...
def resume_self_test(args):
    ...
    def step(model, optimizer):
        ...
        loss, _ = v3_sequence_loss(logits, y, content_weight=1.0)
        loss.backward()
        optimizer.step()
