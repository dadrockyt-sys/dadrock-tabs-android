...
PATH = Path(__file__).resolve().parents[1] / "guitartechs_training_v3" / "objective_decoder.py"
...
class GuitarTechsV3ComponentTests(unittest.TestCase):
    ...
    def test_active_weighting_increases_penalty_for_missed_active_frame(self):
        ...
        loss1, _ = MOD.v3_sequence_loss(logits, labels, active_weight=1.0, continuity_lambda=0.0, identity_lambda=0.0)
        loss2, _ = MOD.v3_sequence_loss(logits, labels, active_weight=1.5, continuity_lambda=0.0, identity_lambda=0.0)
    ...
    def test_pitch_equivalent_wrong_string_margin_is_penalized(self):
        ...
        x[0, 0, 1, 0] = 4.0
        _, bad = MOD.v3_sequence_loss(logits=x.reshape(1, 2, 6 * MOD.NUM_CLASSES), labels=labels)
        x[0, 0, 0, 5] = 7.0
        _, good = MOD.v3_sequence_loss(logits=x.reshape(1, 2, 6 * MOD.NUM_CLASSES), labels=labels)
    ...
    def test_continuity_penalty_detects_fragmenting_probability_swings(self):
        ...
        _, a = MOD.v3_sequence_loss(stable, labels, identity_lambda=0.0)
        _, b = MOD.v3_sequence_loss(fragmented.reshape(1, 4, 6 * MOD.NUM_CLASSES), labels, identity_lambda=0.0)
    ...
    def test_v3_objective_state_resume_is_exact(self):
        ...
        def step(model, opt):
            ...
            logits = model(inputs)
            loss, _ = MOD.v3_sequence_loss(logits, labels, content_weight=1.0)
            loss.backward()
            opt.step()
