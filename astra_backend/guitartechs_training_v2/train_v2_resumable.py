...
RESUME_SCHEMA = "astra-guitar-techs-v2-resume-state-v1"
CANDIDATE_ID = "astra_guitartechs_tabcnn_v2"
TOTAL_EPOCHS = 1000
...
AUTHORIZATION_RECEIPT_SHA256 = "0edfb4656675e31fce0c8cbd1ba42f37619e877b43793d3386781572dea8cb69"
...
def _identity(args, group_count, validation_capture_count):
    return {
        ...
        "source": {
            "baseTrainerGitBlob": BASE_TRAINER_GIT_BLOB,
            "samplerGitBlob": SAMPLER_GIT_BLOB,
            "thresholdsReceiptSha256": THRESHOLDS_RECEIPT_SHA256,
            "authorizationReceiptSha256": AUTHORIZATION_RECEIPT_SHA256,
...
def _write_resume_receipt(path, payload, state_sha256):
    receipt = {
        "schema": "astra-guitar-techs-v2-resume-receipt-v1",
...
def train_segment(args):
    ...
    for epoch in range(start_epoch, args.end_epoch):
        ...
        for batch in batches:
            ...
            for item in batch:
                ...
                loss = base.sequence_loss(logits, y)
        ...
        if (epoch + 1) % CHECKPOINT_EVERY_EPOCHS == 0:
            metrics = base.aggregate_metrics(selection_rows, model, args.data_dir)
    ...
    if optimizer_steps != _expected_steps(group_count, args.end_epoch):
        raise RuntimeError("V2 optimizer-step exposure mismatch")
    ...
    if len(checkpoint_summaries) != args.end_epoch // CHECKPOINT_EVERY_EPOCHS:
        raise RuntimeError("V2 checkpoint count mismatch at segment boundary")
    ...
    if len(checkpoint_summaries) != base.CHECKPOINTS:
        raise RuntimeError("final V2 checkpoint count mismatch")
    ...
    full_metrics = base.aggregate_metrics(val, model, args.data_dir)
...
def resume_self_test(args):
    ...
    print("V2_RESUME_SELF_TEST_PASS")
