...
DECODER_CONFIRMED_START_CONFIDENCE = 0.42
DECODER_CONFIRM_CONFIDENCE = 0.35
DECODER_CONFIRMED_START_VS_SILENCE_RATIO = 0.90
DECODER_CONFIRM_VS_SILENCE_RATIO = 0.80
...
def decode_with_hysteresis(
    ...
    *,
    ...
    confirmed_start_confidence: float = DECODER_CONFIRMED_START_CONFIDENCE,
    confirm_confidence: float = DECODER_CONFIRM_CONFIDENCE,
    confirmed_start_vs_silence_ratio: float = DECODER_CONFIRMED_START_VS_SILENCE_RATIO,
    confirm_vs_silence_ratio: float = DECODER_CONFIRM_VS_SILENCE_RATIO,
    ...
    if not (
        0.0 <= continue_confidence <= start_confidence <= 1.0
        and 0.0 <= confirmed_start_confidence <= start_confidence
        and 0.0 <= confirm_confidence <= 1.0
        and confirmed_start_vs_silence_ratio > 0.0
        and confirm_vs_silence_ratio > 0.0
    ):
    ...
    for string in range(NUM_STRINGS):
        ...
        for frame in range(frames):
            ...
            strong_start = (
                best_active >= start_confidence
                and best_active > silence
            )
            confirmed_start = False
            if not strong_start and frame + 1 < frames:
                next_row = probs[frame + 1, string]
                next_silence = float(next_row[SILENCE_CLASS])
                next_same = float(next_row[best_fret])
                confirmed_start = (
                    best_active >= confirmed_start_confidence
                    and best_active >= silence * confirmed_start_vs_silence_ratio
                    and next_same >= confirm_confidence
                    and next_same >= next_silence * confirm_vs_silence_ratio
                )

            if strong_start or confirmed_start:
                current = best_fret
                decoded[frame, string] = current
            else:
                current = -1
