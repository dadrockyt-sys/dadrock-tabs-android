...
RESUME_SCHEMA = "astra-guitar-techs-v4-resume-state-v1"
...
V4_OBJECTIVE_DECODER_GIT_BLOB = "b9a96f6b256de52bfbb5076482a03263415213cb"
V4_DESIGN_GIT_BLOB = "376ece1578d9adfb998600e85cfbb6b39523951f"
V4_SYNTHETIC_VERIFICATION_GIT_BLOB = "892ce3c88d68accef4c13aea6220c6ffd1cd22a5"
V4_AUTHORIZATION_GIT_BLOB = "427ca9fb3ccd7a406b2935ae75216d7591816efe"
V3_ERROR_DECOMPOSITION_RESULT_GIT_BLOB = "87ec9fb53fd025bd2d5c2b00a15b9bcd035b4433"
...
def _infer_v4(model, feat, batch=1024):
    ...
    with torch.no_grad():
        for lo in range(0, len(windows), batch):
            ...
            probs = torch.softmax(
                logits.reshape(-1, 1, base.NUM_STRINGS, base.NUM_CLASSES)[:, 0],
                dim=-1,
            ).cpu().numpy().astype(np.float32)
            probabilities[lo:lo + len(probs)] = probs
    return decode_with_hysteresis(probabilities)


def _aggregate_metrics_v4(rows, model, data_dir):
    captures = []
    for row in rows:
...
def _identity(args, group_count, validation_capture_count):
    return {
        ...
        "source": {
            ...
            "v4ObjectiveDecoderGitBlob": V4_OBJECTIVE_DECODER_GIT_BLOB,
            "v4DesignGitBlob": V4_DESIGN_GIT_BLOB,
            "v4SyntheticVerificationGitBlob": V4_SYNTHETIC_VERIFICATION_GIT_BLOB,
            "v4AuthorizationGitBlob": V4_AUTHORIZATION_GIT_BLOB,
            "v3ErrorDecompositionResultGitBlob": V3_ERROR_DECOMPOSITION_RESULT_GIT_BLOB,
...
def _write_resume_receipt(path, payload, state_sha256):
    receipt = {
        "schema": "astra-guitar-techs-v4-resume-receipt-v1",
...
def train_segment(args):
    ...
    for epoch in range(start_epoch, args.end_epoch):
        ...
        for batch in batches:
            ...
            for item in batch:
                ...
                loss, _loss_parts = v4_sequence_loss(
                    logits,
                    y,
                    content_weight=content_weight,
                )
                if not torch.isfinite(loss):
                    raise RuntimeError("nonfinite V4 sequence loss")
        ...
        if (epoch + 1) % CHECKPOINT_EVERY_EPOCHS == 0:
            metrics = _aggregate_metrics_v4(selection_rows, model, args.data_dir)
    ...
    if args.end_epoch < TOTAL_EPOCHS:
        ...
        print("V4_RESUME_STATE=" + json.dumps(receipt, sort_keys=True))
        return

    if len(checkpoint_summaries) != base.CHECKPOINTS:
        raise RuntimeError("final V4 checkpoint count mismatch")
    ...
    if selected is None:
        raise RuntimeError("no V4 checkpoint evaluated")
    _, selected_record, selected_state = selected
    model.load_state_dict(selected_state)

    full_metrics = _aggregate_metrics_v4(val, model, args.data_dir)
    ...
    receipt = {
        "schema": "astra-guitar-techs-real-training-fold-v4",
        "candidateId": CANDIDATE_ID,
        "fold": args.fold,
        "trainPerformer": args.train_performer,
        "validationPerformer": args.val_performer,
    ...
    print("V4_TRAINING_RESULT=" + json.dumps({
        "fold": args.fold,
...
def resume_self_test(args):
    ...
    def step(model, optimizer):
        ...
        loss, _ = v4_sequence_loss(logits, y, content_weight=1.0)
        loss.backward()
        optimizer.step()
