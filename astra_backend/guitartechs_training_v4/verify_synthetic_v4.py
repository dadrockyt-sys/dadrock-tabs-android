...
def objective_self_test(source_root):
    ...
    loss, parts = v4.v4_sequence_loss(logits, y, content_weight=1.0)
    if not torch.isfinite(loss):
        raise RuntimeError("V4 synthetic objective produced nonfinite loss")
    if float(parts["identityMargin"]) < 0 or float(parts["continuity"]) < 0:
        raise RuntimeError("V4 synthetic auxiliary loss is negative")
    loss.backward()
    # Reference-aware identity routing must not penalize a legitimate same-pitch
    # note that is explicitly present on another string in the reference.
    labels = torch.full((1, 6, 1), -1, dtype=torch.long)
    labels[0, 0, 0] = 5
    labels[0, 1, 0] = 0
    logits = torch.zeros((1, 1, 6 * v4.NUM_CLASSES), dtype=torch.float32)
    x = logits.reshape(1, 1, 6, v4.NUM_CLASSES)
    x[0, 0, 0, 5] = 7.0
    x[0, 0, 1, 0] = 7.0
    _, parts = v4.v4_sequence_loss(logits, labels, continuity_lambda=0.0)
    if float(parts["identityMargin"]) > 1e-6:
        raise RuntimeError("V4 reference-aware identity routing penalized a legitimate same-pitch unison")

    print("V4_OBJECTIVE_SELF_TEST_PASS")
...
def resume_self_test(source_root):
    ...
    def step(model, optimizer):
        ...
        loss, _ = v4.v4_sequence_loss(logits, y, content_weight=1.0)
        loss.backward()
        optimizer.step()
    ...
    if not nested_equal(continuous_model, resumed.state_dict()):
        raise RuntimeError("V4 resume self-test model state diverged")
    if not nested_equal(continuous_optimizer, resumed_opt.state_dict()):
        raise RuntimeError("V4 resume self-test optimizer state diverged")
    if not nested_equal(continuous_rng, rng_state()):
        raise RuntimeError("V4 resume self-test RNG state diverged")
    print("V4_RESUME_SELF_TEST_PASS")


def decoder_self_test():
    probs = np.zeros((16, 6, v4.NUM_CLASSES), dtype=np.float64)
    probs[..., v4.SILENCE_CLASS] = 1.0
    for t in range(3, 12):
        probs[t, 0, :] = 0.0
        probs[t, 0, v4.SILENCE_CLASS] = 0.1
        probs[t, 0, 4] = 0.9
    for t in (7, 8):
        probs[t, 0, :] = 0.0
        probs[t, 0, v4.SILENCE_CLASS] = 0.55
        probs[t, 0, 4] = 0.45
    probs[14, 0, :] = 0.0
    probs[14, 0, v4.SILENCE_CLASS] = 0.1
    probs[14, 0, 9] = 0.9

    naive = np.argmax(probs, axis=-1).astype(np.int16)
    naive[naive == v4.SILENCE_CLASS] = -1
    decoded = v4.decode_with_hysteresis(probs)
    if v4.count_active_runs(decoded) >= v4.count_active_runs(naive):
        raise RuntimeError("V4 temporal decoder did not reduce synthetic fragmentation")
    if not np.all(decoded[3:12, 0] == 4):
        raise RuntimeError("V4 temporal decoder failed to preserve/restore synthetic true run")
    if decoded[14, 0] != -1:
        raise RuntimeError("V4 temporal decoder failed to suppress synthetic singleton")
    # Diagnosis-driven confirmed-start path: admit a near-silence onset only
    # when the same fret is confirmed on the next frame.
    confirm = np.zeros((8, 6, v4.NUM_CLASSES), dtype=np.float64)
    confirm[..., v4.SILENCE_CLASS] = 1.0
    confirm[2, 0, :] = 0.0
    confirm[2, 0, v4.SILENCE_CLASS] = 0.50
    confirm[2, 0, 4] = 0.45
    confirm[2, 0, 9] = 0.05
    confirm[3, 0, :] = 0.0
    confirm[3, 0, v4.SILENCE_CLASS] = 0.45
    confirm[3, 0, 4] = 0.40
    confirm[3, 0, 9] = 0.15
    for t in range(4, 7):
        confirm[t, 0, :] = 0.0
        confirm[t, 0, v4.SILENCE_CLASS] = 0.10
        confirm[t, 0, 4] = 0.90
    confirmed = v4.decode_with_hysteresis(confirm)
    if confirmed[2, 0] != 4 or not np.all(confirmed[2:7, 0] == 4):
        raise RuntimeError("V4 confirmed-start decoder failed to recover a bounded low-confidence true onset")

    spike = np.zeros((7, 6, v4.NUM_CLASSES), dtype=np.float64)
    spike[..., v4.SILENCE_CLASS] = 1.0
    spike[3, 0, :] = 0.0
    spike[3, 0, v4.SILENCE_CLASS] = 0.50
    spike[3, 0, 4] = 0.45
    spike[3, 0, 9] = 0.05
    rejected = v4.decode_with_hysteresis(spike)
    if rejected[3, 0] != -1 or v4.count_active_runs(rejected) != 0:
        raise RuntimeError("V4 confirmed-start decoder admitted an unconfirmed low-confidence spike")

    print("V4_DECODER_SELF_TEST_PASS")


def content_balance_self_test():
    weights = v4.bounded_content_weights({
        "chords": 20,
        "scales": 20,
        "singlenotes": 4,
        "PalmMute": 1,
    ...
    if any(not (v4.CONTENT_WEIGHT_MIN <= w <= v4.CONTENT_WEIGHT_MAX) for w in weights.values()):
        raise RuntimeError("content weight escaped frozen bounds")
    print("V4_CONTENT_BALANCE_SELF_TEST_PASS")
