#!/usr/bin/env python3
"""Score the frozen V167 active-topology Guitar pruning family.

The complete manifest and every candidate hash are verified before scorer/reference
access. The exact I005 reproduction control is verified reference-blind and is never
scored. Bass is verified normalized-identical to I005 and is never scored. Exactly
the three preregistered new Guitar whole rules receive score calls. No candidate is
...
EPS = 1e-12
EXPECTED = {
    "manifestSchema": "dadrock.tabs.v167.predeclared-active-topology-guitar-manifest.v1",
    ...
    "reproductionId": "topo-repro-i005",
    "newIds": ["topo-single-or-chord", "topo-single-only", "topo-chord-only"],
...
def delta(metric: Mapping[str, Any], baseline: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        * (float(metric["primaryF1"]) - float(baseline["primaryF1"])),
        "precisionPercentagePoints": 100.0
        * (
            float(metric["primaryPrecision"])
            - float(baseline["primaryPrecision"])
        ),
        ...
        * (float(metric["primaryRecall"]) - float(baseline["primaryRecall"])),
        ...
        - int(baseline["falsePositive"]),
        "falseNegative": int(metric["falseNegative"])
        - int(baseline["falseNegative"]),
    }
...
def main() -> int:
    ...
    if args.output.exists():
        raise RuntimeError(f"topology score report already exists: {args.output}")
    if sha256_file(args.i005) != EXPECTED["i005Sha256"]:
        raise RuntimeError("frozen I005 SHA mismatch")

    # PRE-REFERENCE FREEZE BOUNDARY.
    # Verify manifest, policy, candidate hashes, reproduction equality and Bass
    # equality before importing scorer code or opening the professional reference.
    manifest = json.loads(args.manifest.read_text(encoding="utf-8"))
    if manifest.get("schema") != EXPECTED["manifestSchema"]:
        raise RuntimeError("unexpected active-topology manifest schema")
    if manifest.get("status") != "FROZEN_BEFORE_REFERENCE_SCORING":
        raise RuntimeError("active-topology manifest is not frozen pre-score")
    ...
    for key, expected in expected_policy.items():
        if policy.get(key) != expected:
            raise RuntimeError(f"active-topology manifest policy mismatch: {key}")
    promotion = policy.get("promotionEligibility") or {}
    if abs(float(promotion.get("minimumF1GainPercentagePointsVsI005")) - 0.10) > EPS:
        raise RuntimeError("topology promotion F1 threshold drift")
    if promotion.get("precisionMustBeAtLeastI005") is not True:
        raise RuntimeError("topology promotion precision guard drift")

    variants = list(manifest.get("variants") or [])
    if len(variants) != 4:
        raise RuntimeError(f"unexpected topology variant count: {len(variants)}")
    ids = [str(row["id"]) for row in variants]
    if set(ids) != {EXPECTED["reproductionId"], *EXPECTED["newIds"]}:
        raise RuntimeError(f"unexpected topology variant ids: {ids}")
    if len(set(ids)) != len(ids):
        raise RuntimeError("duplicate topology variant id")

    payloads: dict[str, dict[str, Any]] = {}
    for row in variants:
        path = args.variant_root / str(row["relativePath"])
        if not path.is_file():
            raise RuntimeError(f"missing frozen topology candidate: {row['id']}")
        if sha256_file(path) != str(row["sha256"]):
            raise RuntimeError(f"frozen topology candidate hash mismatch: {row['id']}")
        payload = json.loads(path.read_text(encoding="utf-8"))
        if payload.get("schema") != "dadrock.tabs.v167.active-topology-score-candidate.v1":
    ...
    if normalized(repro["streams"]["combinedGuitar"]) != normalized(i005_guitar):
        raise RuntimeError("topology reproduction Guitar differs from I005")
    if normalized(repro["streams"]["bass"]) != normalized(i005_bass):
        raise RuntimeError("topology reproduction Bass differs from I005")
    for variant_id, payload in payloads.items():
        if normalized(payload["streams"]["bass"]) != normalized(i005_bass):
            raise RuntimeError(f"topology Bass differs from I005: {variant_id}")
    ...
    baseline_row = {
        ...
        "config": {
            "baseline": True,
            "source": "frozen_iteration_005_inherited_without_score_call",
        },
        "generationSummary": {
    ...
    promotion_eligible = (
        ...
        and float(best["metrics"]["primaryPrecision"])
        + EPS
        >= float(I005_GUITAR["primaryPrecision"])
    )

    report = {
        "schema": "dadrock.tabs.v167.active-topology-guitar-sweep.v1",
        "version": "V167",
        "status": "REFERENCE_GRADED_COMPLETE_PREDECLARED_ACTIVE_TOPOLOGY_VARIANTS",
        ...
        json.dumps(
            {
                ...
                "newVariantsMeetingPromotionEligibility": report[
                    "newVariantsMeetingPromotionEligibility"
                ],
            },
            indent=2,
            sort_keys=True,
