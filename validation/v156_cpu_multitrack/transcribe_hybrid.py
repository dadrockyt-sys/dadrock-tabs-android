#!/usr/bin/env python3
"""Canonical V156 reference-blind CPU hybrid front-end.

V156 intentionally preserves the already-sealed V155 musical architecture without
quality tuning. It imports only the frozen reference-blind implementation engine;
V155 candidates/outputs and professional references are never inputs.
"""
...
def main() -> int:
    ...
    if args.output.exists() or args.receipt.exists():
        raise RuntimeError("V156 candidate/receipt is write-once")
    ...
    if prereg.get("canonicalContract", {}).get("generationReceiptSchema") != RECEIPT_SCHEMA:
        raise RuntimeError("V156 receipt schema contract drift")
    ...
    if env_receipt.get("v156EngineGitBlob") != ENGINE_GIT_BLOB:
    ...
    if not guitar or not bass:
        raise RuntimeError("V156 generated an empty stream")
