#!/usr/bin/env python3
"""Song-blind static coverage for the V162 JSON-native structural-QC receipt boundary."""
...
from structural_qc_v162 import json_native
...
def main() -> int:
    ...
    if not all(type(value) is bool for value in native_checks.values()):
        raise RuntimeError("synthetic V162 structural checks did not normalize to native bool")
    ...
    if contains_numpy(normalized):
        raise RuntimeError("NumPy type remained after V162 JSON-native normalization")
    expected = {
        "boolScalar": True,
        "integerScalar": 7,
        "floatingScalar": 1.25,
    ...
    if normalized != expected:
        raise RuntimeError(f"unexpected normalized V162 fixture: {normalized!r}")
    encoded = json.dumps(normalized, allow_nan=False, sort_keys=True)
    if json.loads(encoded) != normalized:
        raise RuntimeError("V162 JSON-native fixture did not round-trip exactly")
    ...
    for nonfinite in (float("nan"), float("inf"), float("-inf")):
        try:
            ...
            raise RuntimeError("V162 json_native accepted nonfinite native float")
        try:
    ...
    print(json.dumps({
        "schema": "dadrock.tabs.v162.json-native-static-test.v1",
        "validation": "PASS",
