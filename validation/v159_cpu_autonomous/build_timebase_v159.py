#!/usr/bin/env python3
"""Build the sealed V159 reference-blind timebase before any pitch inference.
...
def main() -> int:
    ...
    if args.output.exists():
        raise RuntimeError("V159 timebase is write-once")
    for path in (
        args.source_audio, args.mix, args.drums, args.bass, args.guitar,
        args.preregistration, args.implementation_contract,
    ):
        if not path.is_file():
            raise RuntimeError(f"missing required V159 timebase input: {path}")
    ...
    if prereg.get("version") != "V159" or prereg.get("status") != "PREREGISTERED_BEFORE_IMPLEMENTATION_CODE":
        raise RuntimeError("invalid V159 preregistration state")
    if contract.get("version") != "V159" or contract.get("status") != "SEALED_BEFORE_IMPLEMENTATION_CODE":
        raise RuntimeError("invalid V159 implementation-contract state")
    if contract.get("canonicalSchemas", {}).get("timebase") != SCHEMA:
        raise RuntimeError("V159 timebase schema contract drift")
    numerics = contract.get("timebaseNumerics", {})
    if numerics.get("analysisSampleRate") != SR or numerics.get("hopLength") != HOP:
        raise RuntimeError("V159 analysis sample-rate/hop contract drift")

    song = prereg.get("song", {})
    source_identity = identity(args.source_audio)
    mix_identity = identity(args.mix)
    if source_identity["sha256"] != song.get("audioSha256") or source_identity["bytes"] != song.get("audioBytes"):
        raise RuntimeError("V159 historical source-audio identity mismatch")
    if mix_identity["sha256"] != song.get("normalizedWavSha256"):
        raise RuntimeError("V159 normalized-mix identity mismatch")
    ...
    with warnings.catch_warnings(record=True) as captured:
        ...
        if n == 0:
            raise RuntimeError("empty V159 fused beat envelope")
        ...
        if not np.all(np.isfinite(fused)) or np.any(fused < 0.0) or not np.any(fused > 0.0):
            raise RuntimeError("V159 fused beat envelope domain failure")
        ...
        if tempo_raw.size != 1 or not math.isfinite(float(tempo_raw[0])) or float(tempo_raw[0]) <= 0.0:
            raise RuntimeError("V159 tracker tempo is not one finite positive scalar")
        tracker_tempo = float(tempo_raw[0])
        if len(beat_frames) < 8:
            raise RuntimeError(f"V159 insufficient accepted beats: {len(beat_frames)}")
        ...
        if not np.all(np.isfinite(detected)) or not np.all(np.diff(detected) > 0.0):
            raise RuntimeError("V159 detected beat times are not finite/strictly increasing")
    ...
    if runtime_warnings:
        raise RuntimeError("V159 RuntimeWarning captured during timebase construction: " + "; ".join(str(x.message) for x in runtime_warnings))
    ...
    if positive_early.size == 0:
        raise RuntimeError("V159 has no positive early inter-beat interval")
    ...
    if not np.all(np.diff(grid_times) > 0.0):
        raise RuntimeError("V159 raw grid is not strictly increasing")
