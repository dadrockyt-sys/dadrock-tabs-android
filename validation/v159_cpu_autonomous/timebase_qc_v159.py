#!/usr/bin/env python3
"""Independent sealed V159 timebase QC.

This is the hard pre-pitch boundary.  It only validates the already-materialized
V159 timebase and its input identities.  It never imports or invokes pitch
recognition code.
"""
...
SCHEMA = "dadrock.tabs.v159.reference-blind-timebase-qc.v1"
...
def main() -> int:
    ...
    if args.receipt.exists():
        raise RuntimeError("V159 timebase-QC receipt is write-once")
    for path in (
    ...
    checks["preregistrationState"] = prereg.get("version") == "V159" and prereg.get("status") == "PREREGISTERED_BEFORE_IMPLEMENTATION_CODE"
    checks["contractState"] = contract.get("version") == "V159" and contract.get("status") == "SEALED_BEFORE_IMPLEMENTATION_CODE"
    ...
    checks["sourceAudioIdentity"] = exact_identity(audio.get("source"), args.source_audio) and sha256_file(args.source_audio) == song.get("audioSha256") and args.source_audio.stat().st_size == song.get("audioBytes")
    checks["normalizedMixIdentity"] = exact_identity(audio.get("normalizedMix"), args.mix) and sha256_file(args.mix) == song.get("normalizedWavSha256")
    ...
    lo = float(numerics.get("minimumBpmInclusive", 30.0))
    hi = float(numerics.get("maximumBpmInclusive", 300.0))
    ratio_lo = float(numerics.get("minimumTempoConsistencyRatioInclusive", 0.5))
    ratio_hi = float(numerics.get("maximumTempoConsistencyRatioInclusive", 2.0))
    ...
    print(json.dumps({"validation": receipt["validation"], "failedChecks": [k for k, ok in checks.items() if not ok]}, sort_keys=True))
