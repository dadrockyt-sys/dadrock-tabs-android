#!/usr/bin/env python3
"""Build the sealed V160 reference-blind timebase before any pitch inference.

This module intentionally has no Basic Pitch, pYIN, professional-reference,
prior-candidate, scorer, or diagnostic-artifact inputs. It consumes only the
sealed preregistration/contract plus the source audio, normalized mix, and the
fresh Demucs stems needed by the preregistered timebase.
"""
...
HOP = 256
EPS = 1e-12
SCHEMA = "dadrock.tabs.v160.reference-blind-timebase.v1"
...
def main() -> int:
    ...
    if args.output.exists():
        raise RuntimeError("V160 timebase is write-once")
    for path in (
        args.source_audio, args.mix, args.drums, args.bass, args.guitar,
        args.preregistration, args.implementation_contract,
    ):
        if not path.is_file():
            raise RuntimeError(f"missing required V160 timebase input: {path}")
    ...
    if prereg.get("version") != "V160" or prereg.get("status") != "PREREGISTERED_BEFORE_IMPLEMENTATION_CODE":
        raise RuntimeError("invalid V160 preregistration state")
    if contract.get("version") != "V160" or contract.get("status") != "SEALED_BEFORE_IMPLEMENTATION_CODE":
        raise RuntimeError("invalid V160 implementation-contract state")
    if contract.get("canonicalSchemas", {}).get("timebase") != SCHEMA:
        raise RuntimeError("V160 timebase schema contract drift")
    numerics = (contract.get("generationNumerics") or {}).get("timebase") or {}
    if numerics.get("analysisSampleRate") != SR or numerics.get("hopLength") != HOP:
        raise RuntimeError("V160 analysis sample-rate/hop contract drift")

    source_contract = (contract.get("generationNumerics") or {}).get("sourceAudio") or {}
    source_identity = identity(args.source_audio)
    mix_identity = identity(args.mix)
    if source_identity["sha256"] != source_contract.get("sha256") or source_identity["bytes"] != source_contract.get("bytes"):
        raise RuntimeError("V160 historical source-audio identity mismatch")
    if mix_identity["sha256"] != source_contract.get("normalizedWavSha256"):
        raise RuntimeError("V160 normalized-mix identity mismatch")
    ...
    with warnings.catch_warnings(record=True) as captured:
        ...
        if n == 0:
            raise RuntimeError("empty V160 fused beat envelope")
        mix_scaled = positive_unit_scale(env_mix[:n])
        drums_scaled = positive_unit_scale(env_drums[:n])
        fused = 0.5 * mix_scaled + 0.5 * drums_scaled
        if not np.all(np.isfinite(fused)) or np.any(fused < 0.0) or not np.any(fused > 0.0):
            raise RuntimeError("V160 fused beat envelope domain failure")
        ...
        if tempo_raw.size != 1 or not math.isfinite(float(tempo_raw[0])) or float(tempo_raw[0]) <= 0.0:
            raise RuntimeError("V160 tracker tempo is not one finite positive scalar")
        tracker_tempo = float(tempo_raw[0])
        if len(beat_frames) < 8:
            raise RuntimeError(f"V160 insufficient accepted beats: {len(beat_frames)}")
        ...
        if not np.all(np.isfinite(detected)) or not np.all(np.diff(detected) > 0.0):
            raise RuntimeError("V160 detected beat times are not finite/strictly increasing")

        low_flux = low_frequency_flux(mix)
        harmonic_change = chroma_change(mix)
        features = {
            ...
            "lowFrequencyFlux": sample_feature(robust_z(low_flux), beat_frames),
            "harmonicChangeNovelty": sample_feature(robust_z(harmonic_change), beat_frames),
        }
    ...
    if runtime_warnings:
        raise RuntimeError("V160 RuntimeWarning captured during timebase construction: " + "; ".join(str(x.message) for x in runtime_warnings))
    ...
    if positive_early.size == 0:
        raise RuntimeError("V160 has no positive early inter-beat interval")
    early_period = float(np.median(positive_early))
    leading = int((-selected_phase) % 4)
    prefix = np.asarray(
        [float(detected[0] - early_period * k) for k in range(leading, 0, -1)],
        dtype=float,
    )
    ...
    if not np.all(np.diff(grid_times) > 0.0):
        raise RuntimeError("V160 raw grid is not strictly increasing")
    ...
    artifact = {
        ...
        "stemIdentities": {
            "drums": identity(args.drums),
            "bass": identity(args.bass),
            "guitar": identity(args.guitar),
        },
