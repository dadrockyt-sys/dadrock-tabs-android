#!/usr/bin/env python3
"""Song-blind static coverage for the V160 JSON-native QC receipt boundary."""
...
from structural_qc_v160 import json_native
...
def main() -> int:
    ...
    if not control_failed:
        raise RuntimeError("control fixture did not reproduce V159 numpy.bool_ JSON failure")

    raw_checks = {
        "boolTrue": np.bool_(True),
        "boolFalse": np.bool_(False),
    }
    ...
    if contains_numpy(normalized):
        raise RuntimeError("NumPy type remained after V160 JSON-native normalization")
    if normalized != {
        "boolScalar": True,
        "integerScalar": 7,
        "floatingScalar": 1.25,
    ...
    if decoded != normalized:
        raise RuntimeError("V160 JSON-native fixture did not round-trip exactly")
    ...
    print(json.dumps({
        "schema": "dadrock.tabs.v160.json-native-static-test.v1",
        "validation": "PASS",
