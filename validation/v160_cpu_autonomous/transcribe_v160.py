#!/usr/bin/env python3
"""Sealed V160 CPU transcriber consuming a frozen PASS V160 timebase.

Pitch-recognition numerics are mechanically preserved from V159/V158. This
module does not build a beat/bar timebase and cannot run unless the independent
V160 timebase-QC receipt is already frozen PASS.
"""
...
CANDIDATE_SCHEMA = "dadrock.tabs.v160.cpu-timebase-first-generated.v1"
RECEIPT_SCHEMA = "dadrock.tabs.v160.cpu-generation-receipt.v1"
TIMEBASE_SCHEMA = "dadrock.tabs.v160.reference-blind-timebase.v1"
TIMEBASE_QC_SCHEMA = "dadrock.tabs.v160.reference-blind-timebase-qc.v1"
PRE_RUN_SCHEMA = "dadrock.tabs.v160.pre-run-identity-receipt.v1"
ENV_SCHEMA = "dadrock.tabs.v160.cpu-environment-receipt.v1"
...
def guitar_events(path: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for onset_index, raw_frame in enumerate(onset_frames):
        ...
        for analysis_frame in (frame - 1, frame, frame + 1):
            ranked = top_template_midi_per_frame(cqt, freqs, analysis_frame, GUITAR_RANGE[0], GUITAR_RANGE[1], 6)
        ...
        for midi in persistent:
            ...
            if len(values) != 3:
                continue
            candidates.append((midi, float(np.mean(values)), any(frame_fundamental.get(midi, []))))
...
class FrozenGrid:
    ...
    def raw_step(self, seconds: float) -> float:
        t = float(seconds)
        if len(self.times) < 2:
            raise RuntimeError("V160 frozen grid has fewer than two beats")
        if t <= self.times[0]:
        ...
        if not math.isfinite(dt) or dt <= 0.0:
            raise RuntimeError("V160 frozen grid interpolation interval invalid")
...
def frozen_grid(timebase: dict[str, Any]) -> FrozenGrid:
    ...
    if len(times) < 2 or len(times) != len(steps):
        raise RuntimeError("V160 frozen timebase grid length invalid")
    if not np.all(np.isfinite(times)) or not np.all(np.isfinite(steps)):
        raise RuntimeError("V160 frozen timebase grid nonfinite")
    if not np.all(np.diff(times) > 0.0) or not np.all(np.diff(steps) == 4.0):
        raise RuntimeError("V160 frozen timebase grid invariant failure")
...
def validate_runtime_boundary(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if prereg.get("version") != "V160" or prereg.get("status") != "PREREGISTERED_BEFORE_IMPLEMENTATION_CODE":
        raise RuntimeError("V160 preregistration state invalid")
    if contract.get("version") != "V160" or contract.get("status") != "SEALED_BEFORE_IMPLEMENTATION_CODE":
        raise RuntimeError("V160 implementation-contract state invalid")
    schemas = contract.get("canonicalSchemas", {})
    if timebase.get("schema") != TIMEBASE_SCHEMA or schemas.get("timebase") != TIMEBASE_SCHEMA:
        raise RuntimeError("V160 timebase schema invalid")
    if schemas.get("candidate") != CANDIDATE_SCHEMA or schemas.get("generationReceipt") != RECEIPT_SCHEMA:
        raise RuntimeError("V160 candidate/generation schema contract drift")
    if qc.get("schema") != TIMEBASE_QC_SCHEMA or schemas.get("timebaseQc") != TIMEBASE_QC_SCHEMA or qc.get("validation") != "PASS":
        raise RuntimeError("V160 timebase QC is not frozen PASS")
    if qc.get("timebaseSha256") != sha256_file(args.timebase):
        raise RuntimeError("V160 timebase identity differs from PASS QC receipt")
    if pre_run.get("schema") != PRE_RUN_SCHEMA or pre_run.get("validation") != "PASS":
        raise RuntimeError("V160 pre-run identity receipt invalid")
    if environment.get("schema") != ENV_SCHEMA or environment.get("validation") != "PASS" or environment.get("device") != "cpu":
        raise RuntimeError("V160 environment receipt invalid")
    if environment.get("cudaAvailable") is not False or environment.get("torchCudaVersion") is not None:
        raise RuntimeError("V160 environment is not sealed CPU-only")
    ...
    if not (
        ...
        raise RuntimeError("V160 frozen timebase safety boundary invalid")

    stem_ids = timebase.get("stemIdentities") or {}
    expected_stems = {"guitar": args.guitar, "bass": args.bass, "drums": args.drums}
    for name, path in expected_stems.items():
        ...
        if record.get("sha256") != sha256_file(path) or record.get("bytes") != path.stat().st_size:
            raise RuntimeError(f"V160 {name} stem differs from frozen timebase identity")
    ...
    if mix_record.get("sha256") != sha256_file(args.mix) or mix_record.get("bytes") != args.mix.stat().st_size:
        raise RuntimeError("V160 normalized mix differs from frozen timebase identity")
...
def main() -> int:
    ...
    if args.output.exists() or args.receipt.exists():
        raise RuntimeError("V160 candidate/generation receipt is write-once")
    for path in (
        ...
        if not path.is_file():
            raise RuntimeError(f"missing V160 transcriber input: {path}")
    ...
    if not guitar or not bass:
        raise RuntimeError("V160 generated empty required stream")
    ...
    candidate = {
        ...
        "classification": "single-preregistered-reference-blind-v160-cpu-timebase-first-candidate",
