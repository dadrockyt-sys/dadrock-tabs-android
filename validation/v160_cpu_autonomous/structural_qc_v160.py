#!/usr/bin/env python3
"""Independent reference-blind structural QC for the single V160 candidate."""
...
QC_SCHEMA = "dadrock.tabs.v160.reference-blind-structural-qc.v1"
CANDIDATE_SCHEMA = "dadrock.tabs.v160.cpu-timebase-first-generated.v1"
GENERATION_SCHEMA = "dadrock.tabs.v160.cpu-generation-receipt.v1"
...
def json_native(value: Any) -> Any:
    """Recursively normalize supported values to deterministic JSON-native types."""
    if value is None:
        return None
    ...
    if type(value) is float:
        if not math.isfinite(value):
            raise RuntimeError("nonfinite native float is not permitted in V160 QC receipt")
        return value
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if type(key) is not str:
                raise RuntimeError("V160 QC receipt dictionary keys must be native strings")
            normalized[key] = json_native(item)
        return normalized
    raise RuntimeError(f"unsupported V160 QC receipt type: {type(value).__name__}")
...
def main() -> int:
    ...
    if args.receipt.exists():
        raise RuntimeError("V160 structural-QC receipt is write-once")
    for path in (
        ...
        if not path.is_file():
            raise RuntimeError(f"missing V160 structural-QC input: {path}")
    ...
    if not all(type(value) is bool for value in native_checks.values()):
        raise RuntimeError("V160 structural-QC check normalization failed")
    ...
    args.receipt.write_text(
        json.dumps(normalized_receipt, indent=2, sort_keys=True, allow_nan=False) + "\n"
    )
    print(json.dumps({
        "validation": normalized_receipt["validation"],
        "failedChecks": [name for name, ok in native_checks.items() if not ok],
