#!/usr/bin/env python3
"""Independent sealed V160 timebase QC.

This is the hard pre-pitch boundary. It only validates the already-materialized
V160 timebase and its input identities. It never imports or invokes pitch
recognition code.
"""
...
SCHEMA = "dadrock.tabs.v160.reference-blind-timebase-qc.v1"
...
def main() -> int:
    ...
    if args.receipt.exists():
        raise RuntimeError("V160 timebase-QC receipt is write-once")
    for path in (
        args.timebase, args.source_audio, args.mix, args.drums, args.bass,
        args.guitar, args.preregistration, args.implementation_contract,
    ):
        if not path.is_file():
            raise RuntimeError(f"missing V160 timebase-QC input: {path}")

    tb = load_json(args.timebase)
    prereg = load_json(args.preregistration)
    contract = load_json(args.implementation_contract)
    generation_numerics = contract.get("generationNumerics") or {}
    numerics = generation_numerics.get("timebase") or {}
    source_contract = generation_numerics.get("sourceAudio") or {}

    checks: dict[str, bool] = {}
    details: dict[str, Any] = {}
    checks["schema"] = tb.get("schema") == TIMEBASE_SCHEMA and contract.get("canonicalSchemas", {}).get("timebaseQc") == SCHEMA
    checks["preregistrationState"] = prereg.get("version") == "V160" and prereg.get("status") == "PREREGISTERED_BEFORE_IMPLEMENTATION_CODE"
    checks["contractState"] = contract.get("version") == "V160" and contract.get("status") == "SEALED_BEFORE_IMPLEMENTATION_CODE"
    ...
    checks["sourceAudioIdentity"] = (
        exact_identity(audio.get("source"), args.source_audio)
        and sha256_file(args.source_audio) == source_contract.get("sha256")
        and args.source_audio.stat().st_size == source_contract.get("bytes")
    )
    checks["normalizedMixIdentity"] = (
        exact_identity(audio.get("normalizedMix"), args.mix)
        and sha256_file(args.mix) == source_contract.get("normalizedWavSha256")
    )
    ...
    lo = float(bpm_range[0])
    hi = float(bpm_range[1])
    ratio_lo = float(ratio_range[0])
    ratio_hi = float(ratio_range[1])
    ...
    checks["tempoConsistencyRatio"] = math.isfinite(ratio) and ratio_lo <= ratio <= ratio_hi
    ...
    if not all(type(value) is bool for value in native_checks.values()):
        raise RuntimeError("V160 timebase-QC check normalization failed")
    passed = all(native_checks.values())
    receipt = {
        "schema": SCHEMA,
        "version": "V160",
        "validation": "PASS" if passed else "FAIL",
        "terminalForV160OnFailure": True,
