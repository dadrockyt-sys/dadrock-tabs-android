#!/usr/bin/env python3
"""Independent reference-blind structural QC for the single V157 candidate."""
...
EXPECTED_PREREG_GIT_BLOB = "0bcca0fbb53abcc8ad8736d8e5e71e32a14004f6"
EXPECTED_ENGINE_BLOB = "3357582dd8311b28f4b85f2ebfbc7acb8c9e4fb8"
EXPECTED_PREREG_STATUS = "PREREGISTERED_BEFORE_GENERATION"
...
QC_SCHEMA = "dadrock.tabs.v157.reference-blind-structural-qc.v1"
ENV_SCHEMA = "dadrock.tabs.v157.cpu-environment-receipt.v1"
...
def fail(msg: str) -> None:
    raise RuntimeError(f"V157 structural QC failure: {msg}")
...
def main() -> int:
    ...
    if prereg.get("status") != EXPECTED_PREREG_STATUS or prereg.get("version") != "V157":
        fail("preregistration identity/status")
    contract = prereg.get("canonicalContract") or {}
    if contract.get("candidateSchema") != CANDIDATE_SCHEMA or contract.get("generationReceiptSchema") != RECEIPT_SCHEMA or contract.get("structuralQcSchema") != QC_SCHEMA or contract.get("environmentReceiptSchema") != ENV_SCHEMA:
    ...
    if pre_run.get("validation") != "PASS" or pre_run.get("version") != "V157":
    ...
    if env.get("workflowRunNumber") != 1:
        fail("generation workflow run number")
    ...
    for item in model_blobs:
        if not isinstance(item, dict):
            fail("invalid model blob record")
        ...
        if not isinstance(logical, str) or not logical or not isinstance(resolved, str) or not resolved:
            fail("invalid model blob paths")
        if not isinstance(digest, str) or len(digest) != 64:
            fail("invalid model blob sha256")
        if not isinstance(size, int) or size < 1048576:
            fail("invalid model blob size")
        if resolved in seen_resolved:
            fail("duplicate resolved model blob")
