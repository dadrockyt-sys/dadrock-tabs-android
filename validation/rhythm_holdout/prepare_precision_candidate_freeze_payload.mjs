...
const inputPath = process.argv[2] || 'debug/v143-contextual-prune/precision-candidate-product.json';
...
if (failedSafetyChecks.length) {
  throw new Error(`Precision candidate is not eligible for freeze: ${failedSafetyChecks.join(', ')}`);
}
...
if (failedStructuredChecks.length) {
  throw new Error(`Precision candidate structured payload is not eligible for freeze: ${failedStructuredChecks.join(', ')}`);
}
...
const freezeInput = {
  ...
  structuredMode: 'v143-contextual-prune-precision-candidate',
...
console.log(JSON.stringify({
  gate: 'rhythm-precision-candidate-freeze-payload-preparation',
  schemaVersion: 1,
  safetyChecks,
  structuredChecks,
