#!/usr/bin/env python3
"""Independent reference-blind structural QC for the single V161 candidate."""
...
QC_SCHEMA = "dadrock.tabs.v161.reference-blind-structural-qc.v1"
CANDIDATE_SCHEMA = "dadrock.tabs.v161.cpu-event-refined-generated.v1"
GENERATION_SCHEMA = "dadrock.tabs.v161.cpu-generation-receipt.v1"
...
REQUIRED_EVENT_FIELDS = {"measure", "step", "midi", "startSeconds", "rawGridStep", "absoluteGridStep", "source", "admissionScore", "originalOnsetFrame", "refinedOnsetFrame", "onsetRefined"}
...
def json_native(value: Any) -> Any:
    ...
    if type(value) is float:
        if not math.isfinite(value):
            raise RuntimeError("nonfinite native float is not permitted in V161 QC receipt")
        return value
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if type(key) is not str:
                raise RuntimeError("V161 QC receipt dictionary keys must be native strings")
            normalized[key] = json_native(item)
        return normalized
    raise RuntimeError(f"unsupported V161 QC receipt type: {type(value).__name__}")
...
def safety_pass(safety: Any) -> bool:
    return isinstance(safety, dict) and safety.get("referenceRead") is False and safety.get("professionalReferencePathsOpened") == 0 and safety.get("referenceFacingScoreCalls") == 0 and safety.get("priorGeneratedCandidateRead") is False and safety.get("priorScoreRead") is False and safety.get("V160CandidateRead") is False and safety.get("referenceGuidedFiltering") is False and safety.get("thresholdSweep") is False and safety.get("variantSelection") is False and safety.get("humanCorrection") is False and safety.get("cudaGpuUsed") is False and safety.get("modalUsed") is False and safety.get("mainOrProductionModified") is False


def stream_check(name: str, rows: Any, times: np.ndarray, steps: np.ndarray) -> tuple[bool, list[str]]:
    errors: list[str] = []
    ...
    for index, row in enumerate(rows):
        ...
        if missing:
            ...
            continue
        for key in ("midi", "startSeconds", "rawGridStep", "absoluteGridStep", "measure", "step", "admissionScore", "originalOnsetFrame", "refinedOnsetFrame"):
            if not finite(row.get(key)):
                errors.append(f"{prefix}: nonfinite {key}")
        if any(msg.startswith(prefix + ": nonfinite") for msg in errors):
            continue
        midi = int(row["midi"])
        ...
        admission = float(row["admissionScore"])
        ...
        if not 0.0 <= admission <= 1.0:
            errors.append(f"{prefix}: admissionScore outside 0..1")
        if type(row.get("onsetRefined")) is not bool:
            errors.append(f"{prefix}: onsetRefined not native bool")
        ...
        if name == "combinedGuitar":
            if source != "basic_pitch_consolidated":
                errors.append(f"{prefix}: unexpected Guitar source {source}")
            for key in ("confidence", "templateRank", "onsetSupport", "activitySupport", "persistenceSupport"):
                if not finite(row.get(key)):
                    errors.append(f"{prefix}: missing/nonfinite Guitar evidence {key}")
        else:
            if source not in {"onset_harmonic_pyin_refined", "transition_harmonic_pyin_refined"}:
                errors.append(f"{prefix}: unexpected Bass source {source}")
            if row.get("proposalKind") not in {"detected_onset", "pitch_transition"}:
                errors.append(f"{prefix}: invalid Bass proposalKind")
            for key in ("medianPyinVoicedProbability", "templateRank", "onsetSupport", "activitySupport"):
                if not finite(row.get(key)):
                    errors.append(f"{prefix}: missing/nonfinite Bass evidence {key}")
        key = (absolute, midi)
        if key in dedupe:
            errors.append(f"{prefix}: duplicate absoluteGridStep/MIDI")
        dedupe.add(key)
        per_step[absolute] += 1
        sort_key = (absolute, midi)
    ...
    for step, count in per_step.items():
        if count > cap:
            errors.append(f"{name}: absolute step {step} has {count} events > cap {cap}")
...
def main() -> int:
    ...
    if args.receipt.exists():
        raise RuntimeError("V161 structural-QC receipt is write-once")
    for path in (args.candidate, args.generation_receipt, args.timebase, args.timebase_qc, args.preregistration, args.implementation_contract, args.pre_run_receipt, args.environment_receipt, args.event_logic, args.transcriber, args.timebase_builder, args.timebase_qc_code, args.negative_runtime_guard):
        if not path.is_file():
            raise RuntimeError(f"missing V161 structural-QC input: {path}")
    ...
    checks["codePins"] = pins.get("preregistration") == git_blob_sha(args.preregistration) and pins.get("implementationContract") == git_blob_sha(args.implementation_contract) and pins.get("eventLogic") == git_blob_sha(args.event_logic) and pins.get("timebaseBuilder") == git_blob_sha(args.timebase_builder) and pins.get("timebaseQc") == git_blob_sha(args.timebase_qc_code) and pins.get("transcriber") == git_blob_sha(args.transcriber) and pins.get("structuralQc") == git_blob_sha(Path(__file__)) and pins.get("negativeRuntimeGuard") == git_blob_sha(args.negative_runtime_guard)
    ...
    checks["generationEventLogicPin"] = generation.get("implementation", {}).get("eventLogicGitBlob") == git_blob_sha(args.event_logic)
    ...
    checks["environmentReceiptEmbeddedExactly"] = generation.get("environment") == environment
    checks["singleGenerationWorkflowRun"] = environment.get("workflowRunNumber") == 1 and environment.get("workflowRunAttempt") == 1 and generation.get("environment", {}).get("workflowRunNumber") == 1 and generation.get("environment", {}).get("workflowRunAttempt") == 1 and generation.get("environment", {}).get("workflowRunId") == environment.get("workflowRunId") and isinstance(environment.get("workflowRunId"), int) and environment.get("workflowRunId") > 0

    times = np.asarray(timebase.get("gridBeatTimesSeconds", []), dtype=float)
    steps = np.asarray(timebase.get("gridBeatSteps", []), dtype=float)
    checks["frozenGrid"] = bool(len(times) >= 2 and len(times) == len(steps) and np.all(np.isfinite(times)) and np.all(np.isfinite(steps)) and np.all(np.diff(times) > 0.0) and np.all(np.diff(steps) == 4.0))
    ...
    if checks["frozenGrid"]:
        ...
        errors.extend(guitar_errors + bass_errors)
    else:
        checks["combinedGuitarStructure"] = False
        checks["bassStructure"] = False
        errors.append("frozen grid invalid")
    ...
    metadata = generation.get("streamMetadata") or {}
    gmeta = metadata.get("combinedGuitar") or {}
    checks["guitarNoStandaloneHarmonicRecovery"] = gmeta.get("standaloneHarmonicTrackRecoveryEnabled") is False and gmeta.get("standaloneHarmonicTrackAddedCount") == 0 and all(str(row.get("source")) == "basic_pitch_consolidated" for row in streams.get("combinedGuitar", []))
    checks["guitarPolyphonyCapSix"] = all(v <= 6 for v in Counter(int(row["absoluteGridStep"]) for row in streams.get("combinedGuitar", [])).values())
    checks["bassGridMonophonyCapOne"] = all(v <= 1 for v in Counter(int(row["absoluteGridStep"]) for row in streams.get("bass", [])).values())
    checks["eventAdmissionScoresFiniteAndWithinZeroOne"] = all(finite(row.get("admissionScore")) and 0.0 <= float(row["admissionScore"]) <= 1.0 for name in ("combinedGuitar", "bass") for row in streams.get(name, []))
    checks["eventRefinementFieldsPresent"] = all(isinstance(row.get("originalOnsetFrame"), int) and isinstance(row.get("refinedOnsetFrame"), int) and type(row.get("onsetRefined")) is bool for name in ("combinedGuitar", "bass") for row in streams.get(name, []))
    checks["candidateSafety"] = safety_pass(candidate.get("safety"))
    checks["generationSafety"] = safety_pass(generation.get("safety"))
    checks["preRunReferenceBlind"] = pre_run.get("referenceReadAtSeal") is False and pre_run.get("professionalReferencePathsOpenedAtSeal") == 0 and pre_run.get("V160CandidateReadAtSeal") is False
    tb_safety = timebase.get("safety") or {}
    checks["timebaseReferenceBlind"] = tb_safety.get("referenceRead") is False and tb_safety.get("professionalReferencePathsOpened") == 0 and tb_safety.get("V160CandidateRead") is False
    ...
    receipt = {
        ...
        "errors": errors,
        "safety": {"referenceRead": False, "professionalReferencePathsOpened": 0, "referenceFacingScoreCalls": 0, "priorGeneratedCandidateRead": False, "priorScoreRead": False, "V160CandidateRead": False},
    }
    ...
    print(json.dumps({"validation": normalized["validation"], "failedChecks": [k for k, ok in native_checks.items() if not ok], "errorCount": len(errors)}, sort_keys=True, allow_nan=False))
