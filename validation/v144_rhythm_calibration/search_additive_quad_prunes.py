...
def candidate_name(signatures: list[str]) -> str:
    ...
    return "prune-additive-quad-" + hashlib.sha256(token).hexdigest()[:16]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fit-only additive quad-prune search from the accepted V144 1144-event baseline."
    )
    ...
    if (manifest.get("selectedCandidate") or {}).get("eventSha256") != ACCEPTED_EVENT_SHA256:
    ...
    baseline_events = apply_triple_prune(v5_events, ACCEPTED_BASELINE_SIGNATURES)
    ...
    if baseline_self["baselineGeneratedMeasureCount"] != 113:
        raise ValueError("accepted baseline must span 113 generated measures")
    ...
    for rule in ranked_rules:
        ...
        if name in metadata:
            raise ValueError(f"quad candidate-name collision: {name}")
        candidate_events = apply_quad_prune(baseline_events, signatures)
        ...
        metadata[name] = {
            **rule,
            "removedFromAcceptedBaselineEventCount": len(baseline_events) - len(candidate_events),
    ...
    fit_lock = lock_fit_candidate(
        candidates, config=config, baseline_name="accepted-v144-baseline"
    )
    ...
    locked_events = (
        ...
        else apply_quad_prune(baseline_events, locked_signatures)
    )
    ...
    selected_reason = "fit-no-qualified-additive-candidate"
    ...
    if locked_name != "accepted-v144-baseline":
        ...
        selected_reason = "locked-additive-candidate-failed-validation"
        ...
        if validation_gate["passed"]:
            ...
            stopped_at = "canary"
            selected_reason = "locked-additive-candidate-failed-canary"

            if canary_gate["passed"]:
                selected = locked_name
                selected_reason = "locked-additive-candidate-passed-split-gates-pending-full-invariant"
    ...
    report = {
        "schemaVersion": 14408,
        "classification": "v144-rhythm-fit-only-additive-quad-search",
        ...
        "candidateConstruction": {
            ...
            "sourceLabels": "accepted-baseline-fit-only-gross-unmatched-generated-notes",
            "runtimeRuleInputs": "reference-free-context-signature-quad-only",
            "candidateRuleShape": "additive-four-signature-conjunction-prune",
