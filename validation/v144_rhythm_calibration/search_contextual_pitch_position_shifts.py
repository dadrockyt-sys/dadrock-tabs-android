...
from v144_rhythm_pitch_shift_policy import DEFAULT_MAX_ABS_SEMITONE_SHIFT  # noqa: E402
...
def candidate_name(signatures: list[str], semitone_shift: int, string_shift: int) -> str:
    token = (
        "\n".join(sorted(str(value) for value in signatures))
        + f"\npitchShift::{int(semitone_shift)}"
        + f"\nstringShift::{int(string_shift)}"
    ).encode("utf-8")
    return "pitch-position-shift-" + hashlib.sha256(token).hexdigest()[:16]
...
def _validate_position(event: Mapping[str, Any]) -> tuple[int, int, int]:
    ...
    if string_index not in OPEN_MIDI_BY_STRING_INDEX:
        raise ValueError("pitch-position candidate has invalid string index")
    if not 0 <= fret <= 36:
        raise ValueError("pitch-position candidate has invalid fret")
    if OPEN_MIDI_BY_STRING_INDEX[string_index] + fret != midi:
        raise ValueError("pitch-position candidate violates tuning-derived pitch/fret identity")
...
def changed_event_count(
    ...
    *,
    ...
    if (expected_semitone_shift is None) != (expected_string_shift is None):
        raise ValueError("expected pitch and string shifts must be supplied together")
    ...
    if len(baseline_events) != len(candidate_events):
        raise ValueError("pitch-position candidate changed event count")

    mutable = {"midi", "fret", "stringIndex"}
    changed = 0
    for baseline, candidate in zip(baseline_events, candidate_events):
        if int(baseline["eventIndex"]) != int(candidate["eventIndex"]):
            raise ValueError("pitch-position candidate changed event ordering")
        ...
        if baseline_static != candidate_static:
            raise ValueError("pitch-position candidate changed protected event metadata")
        ...
        pitch_changed = pitch_delta != 0
        string_changed = string_delta != 0
        if pitch_changed != string_changed:
            raise ValueError("pitch-position candidate changed only pitch or only string position")
        if not pitch_changed:
            continue
        if abs(pitch_delta) > maximum_pitch:
            raise ValueError("pitch-position candidate exceeded semitone bound")
        if string_delta not in (-1, 1):
            raise ValueError("pitch-position candidate exceeded adjacent-string bound")
        if expected_pitch is not None and pitch_delta != expected_pitch:
            raise ValueError("pitch-position candidate used a non-locked semitone delta")
        if expected_string is not None and string_delta != expected_string:
            raise ValueError("pitch-position candidate used a non-locked string delta")
        changed += 1
    return changed
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fit-only contextual joint pitch+adjacent-string search from the locked V144 Rhythm baseline."
    )
    ...
    ranked_rules = rank_fit_pitch_position_rules(
    ...
    for rule in ranked_rules:
        ...
        name = candidate_name(signatures, semitone_shift, string_shift)
        if name in metadata:
            raise ValueError(f"pitch-position candidate-name collision: {name}")
        ...
        if len(candidate_events) != ACCEPTED_EVENT_COUNT:
            raise ValueError(f"pitch-position candidate {name} changed event count")
        ...
        if not measure_evidence["baselineGeneratedMeasureSetPreserved"]:
            raise ValueError(f"pitch-position candidate {name} changed generated measure set")
        if measure_evidence["candidateGeneratedMeasureCount"] != ACCEPTED_GENERATED_MEASURE_COUNT:
            raise ValueError(f"pitch-position candidate {name} changed generated measure count")
        ...
        candidates.append(
            make_candidate(
                name,
                "contextual-pitch-position-shift::"
                + " && ".join(signatures)
                + f" => pitch {semitone_shift:+d}, string {string_shift:+d}",
                fit_score,
                measure_preserved=True,
                event_count_preserved=True,
            )
    ...
    if len(locked_events) != ACCEPTED_EVENT_COUNT:
        raise ValueError("selector locked a pitch-position candidate with changed event count")
    ...
    ) if locked_rule is not None else 0
    ...
    selected_reason = "fit-no-qualified-pitch-position-candidate"
    ...
    if locked_name != "accepted-v144-baseline":
        ...
        selected_reason = "locked-pitch-position-candidate-failed-validation"
        if validation_gate["passed"]:
            ...
            stopped_at = "canary"
            selected_reason = "locked-pitch-position-candidate-failed-canary"
            if canary_gate["passed"]:
                selected = locked_name
                selected_reason = "locked-pitch-position-candidate-passed-split-gates-pending-full-invariant"
    ...
    report = {
        "schemaVersion": 14413,
        "classification": "v144-rhythm-fit-only-contextual-pitch-position-search",
        "evaluationRole": "accepted-v144-baseline-contextual-pitch-position-correction-gold-calibration",
        ...
        "candidateConstruction": {
            ...
            "runtimeRuleInputs": "reference-free-source-pitch-class-plus-structural-context-and-fixed-pitch-string-shifts-only",
            "candidateRuleShape": "contextual-joint-pitch-adjacent-string-position-shift",
            "pairingPolicy": "exact-pitch-removal-then-minimum-absolute-midi-distance-deterministic-then-adjacent-string-target-filter",
            ...
            "timingAndMeasurePreservationRequired": True,
            "protectedNonPositionMetadataPreservationRequired": True,
            "generatedMeasureSetPreservationRequired": True,
            "jointNonZeroPitchAndStringDeltaRequired": True,
