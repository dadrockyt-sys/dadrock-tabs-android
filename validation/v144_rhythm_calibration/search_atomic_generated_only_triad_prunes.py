...
def _validate_fixed_search_parameters(
    ...
    if int(minimum_false_positive_support) != DEFAULT_MIN_FALSE_POSITIVE_SUPPORT:
        raise ValueError("family #13 minimum false-positive support is preregistered at exactly 3")
    if int(maximum_candidates) != DEFAULT_MAX_CANDIDATES:
        raise ValueError("family #13 maximum candidate count is preregistered at exactly 256")
...
def _normalize_note_identity(string_index: int, pitch_class: int) -> tuple[int, int]:
    ...
    if not 0 <= string_value <= 5:
        raise ValueError("family #13 source string outside [0,5]")
    if not 0 <= pitch_value <= 11:
        raise ValueError("family #13 source pitch class outside [0,11]")
...
def _normalize_rule(
    ...
    if not context_value.startswith(_ALLOWED_CONTEXT_PREFIXES):
        raise ValueError("family #13 rule requires one structural onset context signature")
    identities = tuple(
        sorted(
            (
                ...
                _normalize_note_identity(third_string_index, third_pitch_class),
            )
        )
    )
    return context_value, identities  # type: ignore[return-value]
...
def candidate_name(
    ...
    context_value, identities = _normalize_rule(
    ...
    token = (
        ...
        f"note2::{identities[2][0]}::{identities[2][1]}"
    )
    return "generated-only-triad-prune-" + hashlib.sha256(token.encode("utf-8")).hexdigest()[:16]
...
def removed_event_count(
    ...
    *,
    ...
    expected_third_string_index: int | None = None,
    expected_third_pitch_class: int | None = None,
) -> int:
    """Prove a candidate is exactly a reference-free family #13 atomic triad prune."""
    expected_values = (
        ...
        expected_third_string_index,
        expected_third_pitch_class,
    )
    supplied = [value is not None for value in expected_values]
    if any(supplied) and not all(supplied):
        raise ValueError("all expected family #13 rule fields must be supplied together")
    if len(candidate_events) > len(baseline_events):
        raise ValueError("family #13 candidate added events")
    ...
    if candidate_indices != [value for value in baseline_indices if value in candidate_set]:
        raise ValueError("family #13 candidate changed survivor ordering or duplicated event indices")
    if len(candidate_indices) != len(candidate_set):
        raise ValueError("family #13 candidate duplicated survivor event indices")
    ...
    for row in candidate:
        ...
        if index not in baseline_by_index or row != baseline_by_index[index]:
            raise ValueError("family #13 candidate modified a surviving event")

    removed = len(baseline) - len(candidate)
    if removed % 3 != 0:
        raise ValueError("family #13 candidate removed a non-triad event count")
    ...
    if all(supplied):
        context_value, identities = _normalize_rule(
            ...
            int(expected_third_string_index),
            int(expected_third_pitch_class),
        )
        expected_candidate = apply_generated_only_triad_prune_rule(
            baseline,
            context_value,
            identities[0][0],
            identities[0][1],
        ...
        if candidate != expected_candidate:
            raise ValueError("family #13 candidate does not equal locked reference-free rule output")
        ...
        for index in baseline_indices:
            ...
            if len(onset_events) != 3:
                raise ValueError("family #13 removed a non-triad onset")
            if not onset_matches_generated_only_triad_prune_rule(
                ...
                raise ValueError("family #13 removed an onset outside the locked rule")
            removed_onsets.add(onset)
        if removed != 3 * len(removed_onsets):
            raise ValueError("family #13 did not delete each changed triad onset atomically")
    return removed
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="FIT-only family #13 atomic exact-triad generated-only whole-onset prune search."
    )
    ...
    ranked_rules = rank_fit_generated_only_triad_prune_rules(
        fit_generated,
        fit_reference,
    ...
    for rule in ranked_rules:
        ...
        third_string = int(rule["thirdSourceStringIndex"])
        third_pitch = int(rule["thirdSourcePitchClass"])
        name = candidate_name(
            context_value,
            first_string,
            first_pitch,
        ...
        candidate_events = apply_generated_only_triad_prune_rule(
            baseline_events,
            context_value,
            first_string,
            first_pitch,
        ...
        removed_count = removed_event_count(
            ...
            expected_third_string_index=third_string,
            expected_third_pitch_class=third_pitch,
        )
        if removed_count <= 0 or removed_count % 3 != 0:
            continue
        ...
        candidates.append(
            make_candidate(
                name,
                "atomic-generated-only-triad-onset-prune::"
                + context_value
                + f" | notes ({first_string},{first_pitch}) ({second_string},{second_pitch}) ({third_string},{third_pitch})",
                fit_score,
                measure_preserved=True,
        ...
        metadata[name] = {
            **rule,
            ...
            "removedOnsetCount": removed_count // 3,
    ...
    locked_third_string = None if locked_rule is None else int(locked_rule["thirdSourceStringIndex"])
    locked_third_pitch = None if locked_rule is None else int(locked_rule["thirdSourcePitchClass"])
    locked_events = baseline_events if locked_rule is None else apply_generated_only_triad_prune_rule(
        baseline_events,
        locked_context or "",
        ...
        int(locked_third_string),
        int(locked_third_pitch),
    )
    ...
    if not locked_measure_evidence["baselineGeneratedMeasureSetPreserved"]:
        raise ValueError("selector locked family #13 candidate violating measure preservation")
    if int(locked_measure_evidence["candidateGeneratedMeasureCount"]) != BASELINE_MEASURE_COUNT:
        raise ValueError("selector locked family #13 candidate changed generated measure count")
    locked_removed_count = 0 if locked_rule is None else removed_event_count(
        ...
        expected_third_string_index=locked_third_string,
        expected_third_pitch_class=locked_third_pitch,
    )
    ...
    selected_reason = "fit-no-qualified-atomic-generated-only-triad-prune-candidate"
    ...
    if locked_name != BASELINE_LABEL:
        ...
        selected_reason = "locked-atomic-generated-only-triad-prune-candidate-failed-validation"
        if validation_gate["passed"]:
            ...
            stopped_at = "canary"
            selected_reason = "locked-atomic-generated-only-triad-prune-candidate-failed-canary"
            if canary_gate["passed"]:
                selected = locked_name
                selected_reason = "locked-atomic-generated-only-triad-prune-candidate-passed-split-gates-pending-full-invariant"
        ...
        json.dumps(
            {
                ...
                "lockedThirdSourceStringIndex": locked_third_string,
                "lockedThirdSourcePitchClass": locked_third_pitch,
                "runtimeReferenceInputUsed": False,
    ...
    report = {
        "schemaVersion": REPORT_SCHEMA_VERSION,
        "classification": "v144-rhythm-fit-only-atomic-generated-only-triad-onset-prune-search",
        "evaluationRole": "accepted-singleton-baseline-atomic-generated-only-triad-whole-onset-prune-gold-calibration",
        ...
        "candidateConstruction": {
            ...
            "sourceLabels": "accepted-baseline-fit-only-generated-only-exact-three-note-onsets",
            "runtimeRuleInputs": "reference-free-exact-three-note-generated-onset-plus-structural-context-and-three-sorted-source-string-pitch-class-identities-only",
            "candidateRuleShape": "atomic-exact-three-note-generated-only-whole-onset-prune",
        ...
        "locked": {
            ...
            "thirdSourceStringIndex": locked_third_string,
            "thirdSourcePitchClass": locked_third_pitch,
            "eventCount": len(locked_events),
            "eventSha256": sha256_json(locked_events),
            "removedEventCount": locked_removed_count,
            "removedOnsetCount": locked_removed_count // 3,
            "measureSet": {
