...
REPORT_SCHEMA_VERSION = 14420


def _validate_fixed_search_parameters(
    minimum_correction_support: int,
    maximum_candidates: int,
    maximum_abs_semitone_shift: int,
) -> None:
    if int(minimum_correction_support) != DEFAULT_MIN_CORRECTION_SUPPORT:
        raise ValueError("family #10 minimum correction support is preregistered at exactly 3")
    if int(maximum_candidates) != DEFAULT_MAX_CANDIDATES:
        raise ValueError("family #10 maximum candidate count is preregistered at exactly 256")
    if int(maximum_abs_semitone_shift) != DEFAULT_MAX_ABS_SEMITONE_SHIFT:
        raise ValueError("family #10 maximum absolute semitone shift is preregistered at exactly 12")
...
def _normalize_rule(
    ...
    if not context_value.startswith(allowed_prefixes):
        raise ValueError("singleton candidate requires one structural onset context signature")
    ...
    if source_string not in OPEN_MIDI_BY_STRING_INDEX or target_string not in OPEN_MIDI_BY_STRING_INDEX:
        raise ValueError("singleton candidate contains invalid string index")
    if source_string == target_string:
        raise ValueError("singleton candidate requires explicit string replacement")
    if not 0 <= source_pitch <= 11:
        raise ValueError("singleton candidate source pitch class outside [0,11]")
    if shift == 0 or abs(shift) > DEFAULT_MAX_ABS_SEMITONE_SHIFT:
        raise ValueError("singleton candidate semitone shift outside preregistered non-zero bound")
    return context_value, source_string, source_pitch, target_string, shift
...
def candidate_name(
    ...
    context_value, source_string, source_pitch, target_string, shift = _normalize_rule(
    ...
    return "singleton-onset-replace-" + hashlib.sha256(token.encode("utf-8")).hexdigest()[:16]
...
def _validate_position(event: Mapping[str, Any]) -> tuple[int, int, int]:
    ...
    if string_index not in OPEN_MIDI_BY_STRING_INDEX:
        raise ValueError("singleton candidate has invalid string index")
    if not 0 <= fret <= 36:
        raise ValueError("singleton candidate has invalid fret")
    if OPEN_MIDI_BY_STRING_INDEX[string_index] + fret != midi:
        raise ValueError("singleton candidate violates tuning-derived pitch/fret identity")
...
def changed_event_count(
    ...
    *,
    ...
    expected_target_string_index: int | None = None,
    expected_semitone_shift: int | None = None,
) -> int:
    """Count changed events while proving atomic singleton replacement invariants."""
    maximum = int(maximum_abs_semitone_shift)
    if maximum != DEFAULT_MAX_ABS_SEMITONE_SHIFT:
        raise ValueError("singleton search semitone bound must remain preregistered at 12")

    expected_values = (
        expected_context_signature,
        expected_source_string_index,
        expected_source_pitch_class,
        expected_target_string_index,
        expected_semitone_shift,
    )
    supplied = [value is not None for value in expected_values]
    if any(supplied) and not all(supplied):
        raise ValueError("all expected singleton rule fields must be supplied together")
    normalized_expected = None
    if all(supplied):
        normalized_expected = _normalize_rule(
            str(expected_context_signature),
            int(expected_source_string_index),
            int(expected_source_pitch_class),
            int(expected_target_string_index),
            int(expected_semitone_shift),
        )

    if len(baseline_events) != len(candidate_events):
        raise ValueError("singleton candidate changed event count")

    mutable_fields = {"midi", "fret", "stringIndex"}
    ...
    for index, (baseline, candidate) in enumerate(zip(baseline_events, candidate_events)):
        if int(baseline["eventIndex"]) != int(candidate["eventIndex"]):
            raise ValueError("singleton candidate changed event ordering")
        ...
        if baseline_static != candidate_static:
            raise ValueError("singleton candidate changed protected metadata")

        base_string, base_fret, base_midi = _validate_position(baseline)
        cand_string, cand_fret, cand_midi = _validate_position(candidate)
        onset = (int(baseline["measure"]), int(baseline["step"]))
        baseline_by_onset[onset].append(baseline)

        changed_fields = (
            base_string != cand_string,
            base_fret != cand_fret,
            base_midi != cand_midi,
        )
        ...
        if base_string == cand_string:
            raise ValueError("singleton candidate changed pitch without changing string")
        if base_midi == cand_midi:
            raise ValueError("singleton candidate changed string without changing pitch")
        midi_delta = cand_midi - base_midi
        if abs(midi_delta) > maximum:
            raise ValueError("singleton candidate exceeded preregistered semitone bound")
        if cand_fret != cand_midi - OPEN_MIDI_BY_STRING_INDEX[cand_string]:
            raise ValueError("singleton candidate target fret is not tuning-derived")
        changed_indices.append(index)

    for index in changed_indices:
        baseline = baseline_events[index]
        candidate = candidate_events[index]
        onset = (int(baseline["measure"]), int(baseline["step"]))
        baseline_onset = baseline_by_onset[onset]
        if len(baseline_onset) != 1:
            raise ValueError("singleton candidate changed an onset that was not exactly one generated note")

        if normalized_expected is not None:
            context_value, source_string, source_pitch, target_string, shift = normalized_expected
            if not onset_matches_singleton_replacement_rule(
                ...
                raise ValueError("singleton candidate changed an onset outside the locked rule")
            if int(baseline["stringIndex"]) != source_string:
                raise ValueError("singleton candidate used a non-locked source string")
            if int(baseline["midi"]) % 12 != source_pitch:
                raise ValueError("singleton candidate used a non-locked source pitch class")
            if int(candidate["stringIndex"]) != target_string:
                raise ValueError("singleton candidate used a non-locked target string")
            if int(candidate["midi"]) - int(baseline["midi"]) != shift:
                raise ValueError("singleton candidate used a non-locked semitone shift")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="FIT-only atomic singleton-onset pitch+explicit-string replacement search from locked V144 Rhythm baseline."
    )
    ...
    _validate_fixed_search_parameters(
        args.minimum_correction_support,
        args.maximum_candidates,
        args.maximum_abs_semitone_shift,
    )
    ...
    if int(selected_manifest.get("generatedMeasureCount") or 0) != ACCEPTED_GENERATED_MEASURE_COUNT:
        raise ValueError("accepted V144 baseline generated measure count changed")
    ...
    ranked_rules = rank_fit_singleton_onset_replacement_rules(
    ...
    for rule in ranked_rules:
        ...
        target_string = int(rule["targetStringIndex"])
        shift = int(rule["semitoneShift"])
        name = candidate_name(context_value, source_string, source_pitch, target_string, shift)
        if name in metadata:
            raise ValueError(f"singleton candidate-name collision: {name}")
        ...
        if len(candidate_events) != ACCEPTED_EVENT_COUNT:
            raise ValueError(f"singleton candidate {name} changed event count")
        ...
        if not measure_evidence["baselineGeneratedMeasureSetPreserved"]:
            raise ValueError(f"singleton candidate {name} changed generated measure set")
        if measure_evidence["candidateGeneratedMeasureCount"] != ACCEPTED_GENERATED_MEASURE_COUNT:
            raise ValueError(f"singleton candidate {name} changed generated measure count")
        changed_count = changed_event_count(
            ...
            expected_target_string_index=target_string,
            expected_semitone_shift=shift,
        )
        if changed_count <= 0:
            continue
        ...
        policy_text = (
            "atomic-singleton-onset-replacement::"
            + context_value
            + f" | source string {source_string} pc {source_pitch}"
            + f" => target string {target_string}, pitch {shift:+d}"
        )
        candidates.append(
            make_candidate(
                name,
        ...
        metadata[name] = {
            **rule,
            ...
            "changedOnsetCount": changed_count,
    ...
    locked_target_string = None if locked_rule is None else int(locked_rule["targetStringIndex"])
    ...
    locked_events = baseline_events if locked_rule is None else canonical_events(
        apply_singleton_onset_replacement_rule(
            ...
            int(locked_target_string),
            int(locked_shift),
            maximum_abs_semitone_shift=args.maximum_abs_semitone_shift,
        )
    )
    if len(locked_events) != ACCEPTED_EVENT_COUNT:
        raise ValueError("selector locked singleton candidate with changed event count")
    ...
    if not locked_measure_evidence["baselineGeneratedMeasureSetPreserved"]:
        raise ValueError("selector locked singleton candidate violating measure preservation")
    locked_changed_count = 0 if locked_rule is None else changed_event_count(
        ...
        expected_target_string_index=locked_target_string,
        expected_semitone_shift=locked_shift,
    )
    ...
    selected_reason = "fit-no-qualified-atomic-singleton-onset-replacement-candidate"
    ...
    if locked_name != "accepted-v144-baseline":
        ...
        selected_reason = "locked-atomic-singleton-onset-replacement-candidate-failed-validation"
        if validation_gate["passed"]:
            ...
            stopped_at = "canary"
            selected_reason = "locked-atomic-singleton-onset-replacement-candidate-failed-canary"
            if canary_gate["passed"]:
                selected = locked_name
                selected_reason = "locked-atomic-singleton-onset-replacement-candidate-passed-split-gates-pending-full-invariant"
        ...
        json.dumps(
            {
                ...
                "lockedTargetStringIndex": locked_target_string,
    ...
    report = {
        "schemaVersion": REPORT_SCHEMA_VERSION,
        "classification": "v144-rhythm-fit-only-atomic-singleton-onset-replacement-search",
        "evaluationRole": "accepted-v144-baseline-atomic-singleton-onset-pitch-explicit-string-replacement-gold-calibration",
        ...
        "candidateConstruction": {
            ...
            "sourceLabels": "accepted-baseline-fit-only-shared-singleton-onset-reference-pairs",
            "runtimeRuleInputs": "reference-free-exact-singleton-generated-onset-plus-structural-context-source-string-source-pitch-class-explicit-target-string-and-fixed-semitone-shift-only",
            "candidateRuleShape": "atomic-exact-singleton-onset-pitch-explicit-string-replacement",
            "pairingPolicy": "generated-and-reference-onsets-both-exactly-one-note-both-pitch-and-string-change-tuning-derived-target",
            ...
            "atomicOneEventChangeRequired": True,
            "pitchChangeRequired": True,
            "stringChangeRequired": True,
            "explicitTargetStringRequired": True,
            "adjacentStringOnlyRequired": False,
            "timingPreservationRequired": True,
            "durationTechniqueAndOtherMetadataPreservationRequired": True,
            "tuningDerivedTargetFretRequired": True,
        ...
        "locked": {
            ...
            "targetStringIndex": locked_target_string,
            ...
            "changedOnsetCount": locked_changed_count,
            "measureSet": {
