...
def _validate_fixed_search_parameters(
    ...
    if int(minimum_false_positive_support) != DEFAULT_MIN_FALSE_POSITIVE_SUPPORT:
        raise ValueError("family #12 minimum false-positive support is preregistered at exactly 3")
    if int(maximum_candidates) != DEFAULT_MAX_CANDIDATES:
        raise ValueError("family #12 maximum candidate count is preregistered at exactly 256")
...
def _normalize_note_identity(string_index: int, pitch_class: int) -> tuple[int, int]:
    ...
    if not 0 <= string_value <= 5:
        raise ValueError("family #12 source string outside [0,5]")
    if not 0 <= pitch_value <= 11:
        raise ValueError("family #12 source pitch class outside [0,11]")
...
def _normalize_rule(
    ...
    if not context_value.startswith(_ALLOWED_CONTEXT_PREFIXES):
        raise ValueError("family #12 rule requires one structural onset context signature")
    identities = tuple(
        sorted(
            (
...
def candidate_name(
    ...
    return "generated-only-dyad-prune-" + hashlib.sha256(token.encode("utf-8")).hexdigest()[:16]
...
def removed_event_count(
    ...
    *,
    ...
    """Prove a candidate is exactly a reference-free family #12 atomic dyad prune."""
    expected_values = (
    ...
    if any(supplied) and not all(supplied):
        raise ValueError("all expected family #12 rule fields must be supplied together")
    if len(candidate_events) > len(baseline_events):
        raise ValueError("family #12 candidate added events")
    ...
    if candidate_indices != [value for value in baseline_indices if value in candidate_set]:
        raise ValueError("family #12 candidate changed survivor ordering or duplicated event indices")
    if len(candidate_indices) != len(candidate_set):
        raise ValueError("family #12 candidate duplicated survivor event indices")
    ...
    for row in candidate:
        ...
        if index not in baseline_by_index or row != baseline_by_index[index]:
            raise ValueError("family #12 candidate modified a surviving event")

    removed = len(baseline) - len(candidate)
    if removed % 2 != 0:
        raise ValueError("family #12 candidate removed a non-dyad event count")
    ...
    if all(supplied):
        ...
        expected_candidate = apply_generated_only_dyad_prune_rule(
            baseline,
            context_value,
            identities[0][0],
            identities[0][1],
        ...
        if candidate != expected_candidate:
            raise ValueError("family #12 candidate does not equal locked reference-free rule output")
        ...
        for index in baseline_indices:
            ...
            if len(onset_events) != 2:
                raise ValueError("family #12 removed a non-dyad onset")
            if not onset_matches_generated_only_dyad_prune_rule(
                onset_events,
                context_value,
                identities[0][0],
                identities[0][1],
                ...
                raise ValueError("family #12 removed an onset outside the locked rule")
            removed_onsets.add(onset)
        if removed != 2 * len(removed_onsets):
            raise ValueError("family #12 did not delete each changed dyad onset atomically")
    return removed
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="FIT-only family #12 atomic exact-dyad generated-only whole-onset prune search."
    )
    ...
    ranked_rules = rank_fit_generated_only_dyad_prune_rules(
        fit_generated,
        fit_reference,
    ...
    for rule in ranked_rules:
        ...
        candidate_events = apply_generated_only_dyad_prune_rule(
            baseline_events,
            context_value,
            first_string,
            first_pitch,
        ...
        if removed_count <= 0 or removed_count % 2 != 0:
            continue
        ...
        candidates.append(
            make_candidate(
                name,
                "atomic-generated-only-dyad-onset-prune::"
                + context_value
                + f" | notes ({first_string},{first_pitch}) ({second_string},{second_pitch})",
                fit_score,
                measure_preserved=True,
        ...
        metadata[name] = {
            **rule,
            ...
            "removedOnsetCount": removed_count // 2,
    ...
    locked_events = baseline_events if locked_rule is None else apply_generated_only_dyad_prune_rule(
        baseline_events,
        locked_context or "",
    ...
    if not locked_measure_evidence["baselineGeneratedMeasureSetPreserved"]:
        raise ValueError("selector locked family #12 candidate violating measure preservation")
    ...
    selected_reason = "fit-no-qualified-atomic-generated-only-dyad-prune-candidate"
    ...
    if locked_name != BASELINE_LABEL:
        ...
        selected_reason = "locked-atomic-generated-only-dyad-prune-candidate-failed-validation"
        if validation_gate["passed"]:
            ...
            stopped_at = "canary"
            selected_reason = "locked-atomic-generated-only-dyad-prune-candidate-failed-canary"
            if canary_gate["passed"]:
                selected = locked_name
                selected_reason = "locked-atomic-generated-only-dyad-prune-candidate-passed-split-gates-pending-full-invariant"
    ...
    report = {
        "schemaVersion": REPORT_SCHEMA_VERSION,
        "classification": "v144-rhythm-fit-only-atomic-generated-only-dyad-onset-prune-search",
        "evaluationRole": "accepted-singleton-baseline-atomic-generated-only-dyad-whole-onset-prune-gold-calibration",
        ...
        "candidateConstruction": {
            ...
            "sourceLabels": "accepted-baseline-fit-only-generated-only-exact-two-note-onsets",
            "runtimeRuleInputs": "reference-free-exact-two-note-generated-onset-plus-structural-context-and-two-sorted-source-string-pitch-class-identities-only",
            "candidateRuleShape": "atomic-exact-two-note-generated-only-whole-onset-prune",
            ...
            "removedEventsPerChangedOnset": 2,
        ...
        "locked": {
            ...
            "removedOnsetCount": locked_removed_count // 2,
            "measureSet": {
